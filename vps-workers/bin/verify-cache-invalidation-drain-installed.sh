...
if [ ! -f "$remote_dir/jobs/run-web-cron.mjs" ]; then
  echo "Cache-invalidation drain runner is not installed at $remote_dir/jobs/run-web-cron.mjs." >&2
  exit 1
fi
...
drain_count="$(
  printf '%s\n' "$installed_crontab" | awk -v remote_dir="$remote_dir" '
    ...
    index($0, remote_dir "/jobs/run-web-cron.mjs /api/cron/drain-cache-invalidations") &&
