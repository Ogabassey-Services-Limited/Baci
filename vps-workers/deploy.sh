...
22 4   * * * flock -n $REMOTE_DIR/locks/remediation-codex-canary.lock flock -n $REMOTE_DIR/locks/error-remediator-global.lock bash -lc 'export BACI_CODEX_DOCKER_IMAGE=$CODEX_REMEDIATOR_IMAGE BACI_CODEX_CONTAINER_BIN=$CODEX_CONTAINER_BIN && cd $REMOTE_DIR && $NODE_BIN $REMOTE_DIR/jobs/remediation-codex-canary.mjs' >> $REMOTE_DIR/logs/remediation-codex-canary.log 2>&1
