...
0,30 * * * * cd $REMOTE_DIR && $NODE_BIN $REMOTE_DIR/jobs/push-receipts.mjs >> $REMOTE_DIR/logs/push-receipts.log 2>&1
0 0    * * * cd $REMOTE_DIR && $NODE_BIN $REMOTE_DIR/jobs/cleanup-push-tokens.mjs >> $REMOTE_DIR/logs/cleanup-push-tokens.log 2>&1
0 3    * * * cd $REMOTE_DIR && $NODE_BIN $REMOTE_DIR/jobs/cleanup-import-uploads.mjs >> $REMOTE_DIR/logs/cleanup-import-uploads.log 2>&1
EOF
