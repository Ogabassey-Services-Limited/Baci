...
*/15 * * * * flock -n $REMOTE_DIR/locks/vercel-error-remediator.lock flock -n $REMOTE_DIR/locks/error-remediator-global.lock bash -lc 'export BACI_CODEX_DOCKER_IMAGE=$CODEX_REMEDIATOR_IMAGE && cd $REMOTE_DIR && $NODE_BIN $REMOTE_DIR/jobs/vercel-error-remediator.mjs' >> $REMOTE_DIR/logs/vercel-error-remediator.log 2>&1
*/5 *  * * * flock -n $REMOTE_DIR/locks/sentry-mobile-error-remediator.lock flock -n $REMOTE_DIR/locks/error-remediator-global.lock bash -lc 'export BACI_CODEX_DOCKER_IMAGE=$CODEX_REMEDIATOR_IMAGE && cd $REMOTE_DIR && $NODE_BIN $REMOTE_DIR/jobs/sentry-mobile-error-remediator.mjs' >> $REMOTE_DIR/logs/sentry-mobile-error-remediator.log 2>&1
