...
## 2026-06-25 — Missing returnKeyType on IMEI/Serial Input Fields
**Learning:** React Native `TextInput.returnKeyType` controls how the keyboard return key appears; `done` is a current cross-platform value for fields that complete a data-entry step.
**Action:** For standalone or final `TextInput` fields, set `returnKeyType="done"`; use context-specific alternatives such as `next`, `search`, `send`, or `go` when the return key advances focus or submits a different action.
**Source:** React Native TextInput docs / Improving User Experience docs
