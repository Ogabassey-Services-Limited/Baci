...
**Learning:** In Expo/React Native, `FlatList` components rendering predictable items without an explicit `getItemLayout` prop force the framework to calculate item dimensions asynchronously, causing significant UI thread overhead and skipped frames during scroll/mount.
**Action:** Always provide the `getItemLayout` prop alongside predictable minimum sizing (like `minHeight: N`) in item containers to optimize performance by bypassing asynchronous measurement cycles. Also, ensure unused callback parameters (like `data` in `(_, index) => ...`) are replaced with `_` to prevent strict TypeScript/linting failures.
