...
## 2024-05-18 - VerificationCard Hardcoded Colors
**Learning:** Found several hardcoded colors in `VerificationCard.tsx` (like `#ECFDF5`, `#059669`, `#FEF2F2`) that were used to style success and error states. These colors do not adapt to dark mode and violate the theming constraints. Additionally, using raw values for radius instead of `RADIUS` constant violates the layout styling rules.
**Action:** Replace hardcoded hex values with `colors.*` (such as `colors.success`, `colors.error`, etc.) from `useTheme()`. Use the `withAlpha` utility to apply background opacities to semantic colors instead of hardcoding `rgba` strings, allowing them to adapt gracefully across modes. Replaced raw layout values with theme constants (e.g., `borderRadius: 12` to `borderRadius: RADIUS.xl`). Always ensure to import and use layout and typography constants from `@/constants/Colors` rather than raw numbers.
## 2026-06-03 - Removed hardcoded hex values in Expense form
**Learning:** Hardcoded white text colors (`#FFF`) inside `StyleSheet.create` for button text and overlay text in image picker badges prevents them from correctly inheriting the dynamic primary text color (`colors.textOnPrimary`).
**Action:** Remove hardcoded text colors from `StyleSheet.create` definitions and replace them by passing dynamic theme tokens from `useTheme()` via inline style arrays where the text components are rendered.
