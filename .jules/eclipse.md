...
## 2026-05-15 - Hardcoded Translucent White Text on Primary Cards
**Learning:** Hardcoding `#ffffffcc` (white with ~80% opacity) for secondary text and icons on a card with a `colors.primary` background is problematic because it violates the design system's architecture. Hardcoded styles cannot adapt dynamically if the primary color definition changes or requires a specific foreground token in different themes.
**Action:** When a card's background uses a theme token (like `colors.primary`), its children must use the matching foreground token (like `colors.textOnPrimary`) passed dynamically via inline styles. To achieve translucency, combine the dynamic theme token with the `opacity` style property instead of hardcoding an RGBA/Hex8 string in the stylesheet.
