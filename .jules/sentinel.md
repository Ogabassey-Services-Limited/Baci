...
## 2025-02-18 — Sentinel: Missing tenant isolation on staff update endpoint
**Vulnerability:** Broken Access Control / Insecure Direct Object Reference (OWASP A01:2021, CWE-862, CWE-367)
**Learning:** The application successfully authenticated and authorized the user, and verified that the staff `id` belonged to the merchant prior to the update. However, the actual database `update` query lacked the `.eq('merchant_id', merchantId)` filter. In a concurrent environment, this creates a TOCTOU (Time-of-Check to Time-of-Use) race condition, where the staff record could theoretically change ownership between the select check and the update execution, breaking tenant isolation.
**Prevention:** Always append `.eq('merchant_id', ...)` to mutation queries (e.g., `update`, `delete`), even if an upstream `select` check has already validated ownership. This acts as a defense-in-depth measure.
**Source:** Standard mitigation for IDOR/BOLA and race conditions.
