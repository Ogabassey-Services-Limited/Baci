...
## 2026-03-25 - Add CSRF Protection to Storefront API Routes
**Vulnerability:** Several storefront POST endpoints (`/api/storefront/negotiate`, `/api/storefront/imei-check`, `/api/storefront/loyalty/redeem`, `/api/storefront/loyalty/enroll`, `/api/storefront/discount/validate`) were missing CSRF protection, leaving the application vulnerable to cross-site request forgery attacks on state-changing operations or those incurring external service costs (e.g. sickw API).
**Learning:** In the Baci monorepo, ALL authenticated, mutating, or cost-incurring API routes (POST, PUT, PATCH, DELETE) must explicitly call `checkCsrfProtection()` at the start of the handler, including storefront APIs unless explicitly exempted and handled by other strong authentication mechanisms like webhooks with signature validation.
**Prevention:** Ensure new API routes implement CSRF protection. Review existing routes periodically, especially those under public or storefront namespaces to ensure protection is consistently applied.
