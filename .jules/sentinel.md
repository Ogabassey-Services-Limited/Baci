...
## 2026-02-05 - Missing CSRF Protection on Analytics Cache Deletion
**Vulnerability:** The `DELETE /api/analytics/cache` endpoint was accessible to authenticated users without CSRF protection. This allowed a malicious site to trick an authenticated user's browser into sending a `DELETE` request, forcing an unintended invalidation of the merchant's analytics cache and potentially causing a Denial of Service via cache stampede when data is re-fetched.
**Learning:** Even internal caching endpoints that modify state (like `DELETE` requests) require CSRF protection. It's easy to overlook CSRF checks on endpoints that don't modify database state directly, but cache manipulation can still be abused to degrade application performance.
**Prevention:** Always use the `checkCsrfProtection(request)` utility on ALL state-changing API endpoints (POST, PUT, PATCH, DELETE) in the Next.js application, regardless of whether they modify a database or just an in-memory/Redis cache.
