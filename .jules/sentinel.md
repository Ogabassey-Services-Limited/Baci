...
## 2026-05-22 - Fix Timing Attack in MyCover Webhook Signature Verification
**Vulnerability:** The MyCover webhook handler (`apps/web/src/app/api/webhooks/mycover/route.ts`) verified the signature by converting it to an array of bytes and XORing the arrays in a loop using `result |= charA ^ charB`. While it attempted to be constant-time, it had an early exit `if (!signature)` and the implementation was error-prone, failing to guarantee true constant-time performance across engines because of V8 optimization edge cases on JS loops. Additionally, `crypto.subtle` is completely unnecessary in the Node environment where the standard `crypto` module securely exposes underlying optimized C bindings for HMAC.
**Learning:** Manual implementations of cryptographic equality operations (e.g. byte XOR loops) in JavaScript should be avoided. Node's `crypto.timingSafeEqual` (via our centralized `@/lib/constant-time-equal`) abstracts away V8 optimization pitfalls and is guaranteed to be constant time.
**Prevention:** Always use `@/lib/constant-time-equal` (`constantTimeEqual`) when comparing webhook signatures or other sensitive authentication digests.
