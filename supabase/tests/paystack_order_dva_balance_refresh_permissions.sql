...
-- Change the order balance and refresh again. Only the active snapshot may
-- receive the new payable amount; expired snapshots must retain their
-- assignment-time values for delayed transfer matching.
SET LOCAL ROLE service_role;
SELECT pg_catalog.set_config('request.jwt.claim.role', 'service_role', true);
UPDATE public.orders
SET total = 6000
WHERE id = '3d7a0200-0000-4000-8000-000000000001';
...
BEGIN
  ...
  IF v_payable_amount IS DISTINCT FROM 5000::numeric THEN
    RAISE EXCEPTION
      'second refresh expected payable amount 5000, got %',
      v_payable_amount;
  END IF;
