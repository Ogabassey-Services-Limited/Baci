...
function availableUnitPredicatesMatch(source, variantVariable, branchVariable) {
  ...
  const contradictoryStatus = new RegExp(
    `(?:${branchQualifier}status\\s*=\\s*'(?!available')[^']+'|${branchQualifier}status\\s*(?:<>|!=)\\s*'available'|${branchQualifier}status\\s+NOT\\s+IN\\s*\\([^)]*'available'|NOT\\s*\\(\\s*${branchQualifier}status\\s*=\\s*'available'\\s*\\)|\\(\\s*${branchQualifier}status\\s*=\\s*'available'\\s*\\)\\s+IS\\s+FALSE)`,
    'i'
  );
  return (
    query !== null &&
