    ...
    for (const tx of stuckTransactions ?? []) {
        ...
        try {
            ...
            if (tx.gateway === "paystack") {
                ...
                console.warn(`Gateway ${tx.gateway} not supported for automated verification.`);
                continue;
            }
            ...
            if (actualStatus === "success") {
                ...
                results.push({ id: tx.id, gateway_reference: tx.gateway_reference, status: "success" });
                ...
                results.push({ id: tx.id, gateway_reference: tx.gateway_reference, status: "failed" });
            } else {
                results.push({ id: tx.id, gateway_reference: tx.gateway_reference, status: actualStatus });
            }
        } catch (err) {
            console.error(`Error verifying transaction ${tx.id}:`, err);
            results.push({ id: tx.id, gateway_reference: tx.gateway_reference, error: err.message });
        }
    }

    return new Response(JSON.stringify({ processed: stuckTransactions?.length ?? 0, details: results }), {
...
async function verifyJuicyway(reference: string): Promise<any> {
    ...
    const baseUrl = Deno.env.get("JUICYWAY_BASE_URL") || "https://api.spendjuice.com";
...
async function verifyKorapay(reference: string): Promise<any> {
    ...
    const res = await fetch(`https://api.korapay.com/merchant/api/v1/charges/${reference}/verify`, {
        headers: { Authorization: `Bearer ${secret}` },
    });
    if (res.status === 404) return "failed";
    const data = await res.json();
    ...
    if (data.status && data.data.status === "success") return "success";
    if (data.status && (data.data.status === "failed" || data.data.status === "expired")) return "failed";
    return "pending";
}
