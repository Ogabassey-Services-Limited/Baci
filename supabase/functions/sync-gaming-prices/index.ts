...
const GOOGLE_SHEET_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQNLSf8iIWXHWqn_SPYRAr-oGVjZdf8PyEpKQ1P2TwHxNkaZPyfBhl1ncs_Fp_8tFE5mx1A-4e6WQYJ/pub?output=csv";
...
serve(async (req) => {
    try {
        ...
        for (let i = startIdx; i < lines.length; i++) {
            ...
            const costPrice = parseFloat(priceStr.replace(/["',]/g, "")) || 0;
            if (costPrice === 0) continue;

            const externalId = rawName.toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-");
...
async function sendNotification(newProducts: any[], priceChanges: any[]) {
    ...
    if (priceChanges.length > 0) {
        ...
        <ul>
            ${priceChanges.slice(0, 50).map(p => `<li><b>${p.name}</b>: ₦${p.oldPrice?.toLocaleString()} ➡️ ₦${p.newPrice.toLocaleString()}</li>`).join("")}
