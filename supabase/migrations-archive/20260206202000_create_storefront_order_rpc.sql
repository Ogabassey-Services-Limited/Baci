-- Migration: Secure storefront order creation RPC
-- Created: 2026-02-06
-- Description: Allow guest checkout order creation without service role
...
BEGIN
  ...
  IF v_user_id IS NOT NULL AND p_user_id IS NOT NULL AND p_user_id <> v_user_id THEN
    RAISE EXCEPTION 'user_id_mismatch';
  END IF;

  -- Ensure merchant exists
  PERFORM 1 FROM merchants WHERE id = p_merchant_id LIMIT 1;
  IF NOT FOUND THEN
  ...
  SELECT
    COUNT(*) FILTER (WHERE product_id IS NULL OR product_name IS NULL) AS invalid_item_count,
    COUNT(*) FILTER (WHERE quantity IS NULL OR quantity <= 0) AS invalid_quantity_count,
    COUNT(*) FILTER (
      WHERE variant_id IS NOT NULL AND variant_stock IS NULL
  ...
  SELECT COALESCE(SUM((COALESCE(price_override, base_price) * quantity) + assurance_fee), 0)
    INTO v_subtotal
  FROM tmp_storefront_order_items;
  ...
  ELSE
    ...
    SET
      ...
      user_id = COALESCE(user_id, p_user_id, v_user_id),
  ...
  -- Stock adjustments (skip if manage_stock = false)
  FOR item_rec IN
    SELECT product_id, variant_id, quantity, manage_stock
    FROM tmp_storefront_order_items
  LOOP
    IF COALESCE(item_rec.manage_stock, true) THEN
