...
--
-- 1. CRITICAL — merchants.user_id has NO index.
--    Every RLS policy check (WHERE user_id = auth.uid()) and every
--    onboarding/login merchant lookup performs a sequential scan.
--
-- 2. HIGH — upsert_customer_on_auth uses SELECT-then-INSERT (race condition).
--    Two concurrent logins for the same email can both see "no customer"
--    and attempt to INSERT, causing a duplicate key error.
--    Fix: Use INSERT ... ON CONFLICT for atomic upsert.
