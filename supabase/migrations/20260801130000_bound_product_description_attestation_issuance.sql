-- Boundary C1 abuse-control follow-up: cap new attestation grants per
-- merchant in a rolling window, including rows already archived as evidence.
-- Idempotent replays are handled before the budget check in the RPC.

CREATE INDEX IF NOT EXISTS product_description_attestation_grants_merchant_created_idx
  ON private.product_description_attestation_grants (merchant_id, created_at);

CREATE INDEX IF NOT EXISTS product_description_attestation_grant_evidence_merchant_created_idx
...
COMMENT ON FUNCTION public.request_product_description_attestation_grant(uuid, uuid, uuid, text, text, text, text, boolean, text) IS
  'Authenticated attestation grants are idempotent and capped at 1000 new grants per merchant per rolling hour; active and archived evidence rows count toward the cap.';
