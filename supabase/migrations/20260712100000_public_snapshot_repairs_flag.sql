-- Extend the anonymous storefront snapshot wrapper's feature-settings
-- allowlist with repairs_catalog_enabled.
--
-- Why: the storefront merchant path now sources feature_settings from
-- resolve_storefront_public_snapshot_v2. The repairs catalogue's public
-- surfaces (repair pages, PDP repair link, repairs sitemap) gate on
-- merchant.feature_settings.repairs_catalog_enabled, so omitting the key
-- from the allowlist silently disables the public repairs catalogue for
-- merchants that enabled it. The flag is a benign presentation boolean in
-- the same class as the other *_enabled page toggles already allowlisted;
-- no payment, plan, or credential data is affected.
--
