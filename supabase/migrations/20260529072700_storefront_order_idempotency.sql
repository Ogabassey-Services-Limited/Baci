-- Storefront checkout idempotency for BNPL provider switching.
-- The key is scoped per merchant and the request hash excludes payment provider,
-- allowing Credit Direct -> Klump/card retries to reuse one pending order row.
...
-- DROP the prior create_storefront_order signatures: the 19-arg B3
-- overload, the intermediate 21-arg initial B3.5 overload, and the
-- stale 22-arg parity-check overload. The first B3.5 push introduced
-- 21 args; reviewer feedback (Codex P1) required moving the parity
-- check into the RPC, which added `p_expected_total`. Without dropping
-- all historical overloads here, callers that route via positional args
-- could hit stale implementations.
...
AS $$
  ...
  -- Keep pathological client arrays bounded; requests above this limit
  -- intentionally return no overrides instead of scanning unbounded input.
...
COMMENT ON FUNCTION public.create_storefront_order_with_savings(
  ...
  text, jsonb, text, text, jsonb, uuid, text, text, uuid, text, numeric,
  numeric, uuid, numeric, text, text, text
) IS 'Atomically creates or replays a storefront order and redeems an eligible customer savings goal in the same database transaction.';
...
COMMENT ON FUNCTION public.prepare_storefront_order_for_checkout(
  UUID, UUID, TEXT, TEXT, TEXT, TEXT, UUID
) IS 'Prepares an existing pending storefront order for a new checkout attempt by validating merchant, tracking token, and customer email, checking payment and shipping status reusability, and updating payment-related fields for pending-order reuse.';

REVOKE ALL ON FUNCTION public.prepare_storefront_order_for_checkout(
  UUID, UUID, TEXT, TEXT, TEXT, TEXT, UUID
) FROM PUBLIC;
GRANT ALL ON FUNCTION public.prepare_storefront_order_for_checkout(
  UUID, UUID, TEXT, TEXT, TEXT, TEXT, UUID
) TO anon;
GRANT ALL ON FUNCTION public.prepare_storefront_order_for_checkout(
  UUID, UUID, TEXT, TEXT, TEXT, TEXT, UUID
...
REVOKE ALL ON FUNCTION public.set_credit_direct_session(
  uuid, text, uuid, text, numeric
) FROM PUBLIC;
GRANT ALL ON FUNCTION public.set_credit_direct_session(
  uuid, text, uuid, text, numeric
) TO anon;
GRANT ALL ON FUNCTION public.set_credit_direct_session(
  uuid, text, uuid, text, numeric
