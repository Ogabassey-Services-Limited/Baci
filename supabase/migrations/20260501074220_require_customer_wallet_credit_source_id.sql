-- Require source ids for customer wallet credits that rely on source-based
-- idempotency. Without a source id, retries cannot take the advisory lock or
-- find an existing ledger entry before mutating the wallet balance.
