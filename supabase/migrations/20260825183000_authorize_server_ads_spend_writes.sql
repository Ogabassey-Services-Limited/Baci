...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.merchant_ad_connections
  ...
  LOOP
    IF pg_catalog.jsonb_typeof(v_row) <> 'object'
      ...
      OR (v_row ->> 'provider_customer_id') IS DISTINCT FROM pg_catalog.btrim(p_provider_customer_id) THEN
  ...
  RETURN public.upsert_merchant_ads_spend_daily(
    p_merchant_id, p_provider, COALESCE(p_rows, '[]'::pg_catalog.jsonb)
  );
END;
$$;
...
BEGIN
  ...
  WHERE c.merchant_id = p_merchant_id
    ...
    AND c.status = 'active';
  IF v_selected_customer_id IS NULL OR v_selected_customer_id <> p_provider_customer_id THEN
  ...
  LOOP
    INSERT INTO public.merchant_ad_spend_daily (
      ...
      p_merchant_id, 'google_ads', p_provider_customer_id,
      (v_row ->> 'spend_date')::pg_catalog.date,
      v_row ->> 'currency_code',
      (v_row ->> 'spend_micros')::pg_catalog.int8,
      (v_row ->> 'impressions')::pg_catalog.int8,
...
GRANT EXECUTE ON FUNCTION public.upsert_merchant_ads_spend_daily(
  uuid, text, jsonb
) TO service_role;
