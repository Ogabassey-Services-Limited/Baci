-- Follow-up hardening for the storefront shipping policy and public repair
-- booking limiter. Existing migrations are immutable, so live definitions are
-- replaced here rather than rewriting historical rollout sources.

-- This helper is a literal immutable catalog with no object lookups. Resetting
-- its inherited search_path setting lets PostgreSQL inline and constant-fold
-- the catalog at call sites.
...
-- v_provider is validated against the supported catalog before this lookup,
-- so comparing the stored setting directly avoids a redundant helper call for
-- every JSON array entry.
...
-- Serialize valid bookings for a merchant before either rolling rate-limit
-- count. A merchant-level transaction lock covers both the merchant-wide and
-- normalized-email thresholds without making invalid requests contend.
