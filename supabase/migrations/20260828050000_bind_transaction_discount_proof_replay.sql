-- A valid storefront transaction-discount proof must be consumable by only one
-- order. Without a private replay ledger, a caller could copy a signed marker
-- into another direct public RPC call for the same merchant.
CREATE TABLE IF NOT EXISTS private.transaction_discount_proof_replay (
...
COMMENT ON TABLE private.transaction_discount_proof_replay IS
  'Private one-time consumption ledger for storefront transaction discount proofs.';
