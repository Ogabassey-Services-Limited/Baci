-- Count orders across every recorded currency while keeping monetary totals
-- strictly NGN-only. Currency scope must never suppress non-monetary activity.
...
AS $$
  ...
  current_orders AS MATERIALIZED (
    ...
    WHERE o.created_at >= p_start_at AND o.created_at < p_now
  ),
  current_stats AS (
    SELECT COUNT(*)::bigint AS gross_orders,
      ...
      COUNT(*) FILTER (
        WHERE currency = 'NGN' AND payment_status = 'paid'
      )::bigint AS ngn_paid_orders,
      COALESCE(SUM(total) FILTER (
        WHERE currency = 'NGN' AND payment_status = 'paid'
      ), 0)::numeric AS paid_gmv,
      COUNT(DISTINCT merchant_id) FILTER (
        WHERE currency = 'NGN' AND payment_status = 'paid'
      )::bigint AS selling_merchants,
      COUNT(*) FILTER (WHERE currency IS DISTINCT FROM 'NGN')::bigint
        AS excluded_gross_orders,
      COUNT(*) FILTER (
        WHERE currency IS DISTINCT FROM 'NGN' AND payment_status = 'paid'
      )::bigint AS excluded_paid_orders
    FROM current_orders
  ),
  ...
  previous_stats AS (
    SELECT COUNT(*) FILTER (WHERE payment_status = 'paid')::bigint AS paid_orders,
      COUNT(*) FILTER (
        WHERE currency = 'NGN' AND payment_status = 'paid'
      )::bigint AS ngn_paid_orders,
      COALESCE(SUM(total) FILTER (
  ...
  merchant_users AS MATERIALIZED (
    SELECT m.id AS merchant_id, m.user_id FROM public.merchants AS m
    WHERE m.user_id IS NOT NULL
    UNION
    SELECT sm.merchant_id, sm.user_id FROM public.staff_members AS sm
    WHERE sm.user_id IS NOT NULL AND sm.status = 'active'
  ),
  active_stats AS (
    SELECT COUNT(DISTINCT mu.merchant_id) FILTER (
        WHERE a.created_at >= p_start_at AND a.created_at < p_now
      )::bigint AS current_active,
      COUNT(DISTINCT mu.merchant_id) FILTER (
        WHERE p_period <> 'all' AND a.created_at >= p_previous_start_at
          AND a.created_at < p_previous_end_at
      )::bigint AS previous_active
    FROM auth.audit_log_entries AS a
    INNER JOIN merchant_users AS mu
  ...
  SELECT jsonb_build_object(
    'totalGmv', cs.paid_gmv, 'grossGmv', cs.gross_gmv,
    ...
    'gmvChange', CASE WHEN p_period = 'all' THEN 0
      WHEN ps.paid_gmv > 0 THEN (cs.paid_gmv - ps.paid_gmv) / ps.paid_gmv * 100
      WHEN cs.paid_gmv > 0 THEN 100 ELSE 0 END,
    'orderChange', CASE WHEN p_period = 'all' THEN 0
      WHEN ps.paid_orders > 0
        THEN (cs.paid_orders - ps.paid_orders)::numeric / ps.paid_orders * 100
      WHEN cs.paid_orders > 0 THEN 100 ELSE 0 END,
    'activeMerchants', COALESCE(ast.current_active, 0),
    'activeMerchantChange', CASE WHEN p_period = 'all' THEN 0
      WHEN COALESCE(ast.previous_active, 0) > 0
        THEN (COALESCE(ast.current_active, 0) - ast.previous_active)::numeric
          / ast.previous_active * 100
    ...
    'avgOrderValue', CASE WHEN cs.ngn_paid_orders > 0
      THEN cs.paid_gmv / cs.ngn_paid_orders ELSE 0 END,
    'aovChange', CASE WHEN p_period = 'all' THEN 0
      WHEN ps.ngn_paid_orders > 0 AND ps.paid_gmv > 0 THEN (
        (CASE WHEN cs.ngn_paid_orders > 0
          THEN cs.paid_gmv / cs.ngn_paid_orders ELSE 0 END)
        - (ps.paid_gmv / ps.ngn_paid_orders)
      ) / (ps.paid_gmv / ps.ngn_paid_orders) * 100
      WHEN cs.ngn_paid_orders > 0 THEN 100 ELSE 0 END,
    'avgGmvPerMerchant', CASE WHEN cs.selling_merchants > 0
      THEN cs.paid_gmv / cs.selling_merchants ELSE 0 END,
    'recordedPlatformFees', NULL, 'recordedProcessorFees', NULL,
    'recordedMerchantNet', NULL
  )
  FROM current_stats AS cs CROSS JOIN previous_stats AS ps
  CROSS JOIN active_stats AS ast CROSS JOIN merchant_counts AS mc;
$$;
