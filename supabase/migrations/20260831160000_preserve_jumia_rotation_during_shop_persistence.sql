-- Keep shop persistence from overwriting a newer rotating Self Authorization
-- grant. The caller supplies the authorization rotation version it observed
-- before exchanging the token. A row lock serializes this persistence with
-- workers that rotate the same grant; when the version is stale, the current
-- ciphertext and expiry metadata are used instead of the stale request.

DROP FUNCTION IF EXISTS public.persist_jumia_self_authorization_ordered(
  uuid, text, text, timestamptz, timestamptz,
  text[], text[], text[], text[], text[]
);

CREATE FUNCTION public.persist_jumia_self_authorization_ordered(
  p_merchant_id uuid,
