-- Follow-up hardening for direct /rest/v1/rpc/create_repair_booking callers.
--
-- App routes validate repair booking input with Zod before calling the RPC, but
-- anon can also execute the public wrapper directly through Supabase REST. The
-- SECURITY DEFINER function must therefore reject malformed customer/device
-- fields itself and store normalized values.
...
BEGIN
  ...
  VALUES (
    ...
    v_device_type, v_device_model, v_issue_description, p_preferred_date,
