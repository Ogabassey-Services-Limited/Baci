-- Manual retry hardening: when a legacy manual shipped/delivered endpoint is
-- allowed to retry after a skipped/failed outbox row, record the successful
-- retry back into the outbox so later manual calls are blocked by a sent marker.
...
BEGIN
  ...
  WITH candidate AS (
    ...
    WHERE outbox.order_id = p_order_id
      ...
      AND outbox.status IN ('pending', 'skipped', 'failed')
    ORDER BY
      CASE outbox.status
        WHEN 'pending' THEN 1
        WHEN 'failed' THEN 2
...
COMMENT ON FUNCTION public.complete_order_notification_outbox_manual_result(
  ...
  'Records manual shipped/delivered notification outcomes. Updates pending/skipped/failed retry rows or inserts a terminal audit row when no outbox row exists; processing rows remain owned by cron.';
