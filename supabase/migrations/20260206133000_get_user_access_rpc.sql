-- Create function to return the authenticated user's merchant access and permissions
-- This avoids service-role usage in API routes and keeps logic centralized.
...
BEGIN
  ...
  -- Staff access
  SELECT merchant_id, role, permissions INTO v_staff
  FROM staff_members
  WHERE user_id = v_user_id
    AND status = 'active'
  LIMIT 1;
