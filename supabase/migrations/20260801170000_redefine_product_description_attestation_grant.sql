...
COMMENT ON FUNCTION public.request_product_description_attestation_grant(uuid, uuid, uuid, text, text, text, text, boolean, text) IS
  'Authenticated attestation grants use merchant-scoped operation keys, are idempotent, and are capped at 1000 new grants per merchant per rolling hour; active and archived evidence rows count toward the cap.';
