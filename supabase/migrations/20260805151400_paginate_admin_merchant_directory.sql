-- Bound platform-admin merchant-directory results at the database boundary.
...
BEGIN
  ...
  FROM filtered
  ORDER BY CASE WHEN p_sort_by = 'gmv' THEN total_gmv END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'orders' THEN total_orders END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'joined' THEN joined_at END DESC NULLS LAST, merchant_id ASC
