-- PR-C follow-up: expose merchants.updated_at to mobile-admin so store settings
-- can enforce optimistic concurrency instead of silently overwriting newer edits.
...
COMMENT ON FUNCTION public.get_user_merchant_context() IS
  'Returns merchant and primary domain context, including merchants.updated_at for mobile optimistic concurrency.';
