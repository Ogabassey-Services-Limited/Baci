-- Processing settlements are still pending merchant funds and must be counted
-- with pending settlements in the currency-less settlement summary.
...
AS $$
  ...
  settlement_counts AS (
    SELECT
      COUNT(*) FILTER (
        WHERE settlement.status IN ('pending', 'processing')
...
COMMENT ON FUNCTION public.get_admin_merchant_360_v2(uuid) IS
  'Platform-admin Merchant 360 v2. Pending settlement count includes pending and processing; settlement amounts are withheld because currency is not recorded.';
