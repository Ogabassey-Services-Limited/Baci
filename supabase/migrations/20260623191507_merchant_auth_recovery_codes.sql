...
-- ---------------------------------------------------------------------------
-- Attempt ledger: lockout is computed per user / code-set and per IP from
-- here, so wrong codes that match no row still count toward the throttle.
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS merchant_auth_recovery_attempts_account_lockout_idx
  ON public.merchant_auth_recovery_attempts (user_id, code_set_id, created_at DESC)
  WHERE succeeded = false;

DROP FUNCTION IF EXISTS public.claim_merchant_auth_recovery_code(uuid, uuid, uuid, text);
DROP FUNCTION IF EXISTS public.claim_merchant_auth_recovery_code(uuid, uuid, uuid, uuid, text);
...
BEGIN
  ...
  -- Serialize lockout accounting for this user / code-set. This prevents
  -- concurrent wrong-code bursts from rotating IPs and all passing a stale
  -- count-before-insert check against the same subscriber account.
  PERFORM pg_advisory_xact_lock(
    hashtextextended(
      p_user_id::text || ':' || p_code_set_id::text,
      0
    )
  );

  SELECT COUNT(*)::integer
  INTO account_failure_count
  ...
  IF account_failure_count >= p_max_failures THEN
    RETURN NULL;
  END IF;
  ...
  IF ip_failure_count >= p_max_failures THEN
    RETURN NULL;
  END IF;
...
BEGIN
  ...
  IF NULLIF(BTRIM(p_replacement_code_hash), '') IS NULL THEN
    RAISE EXCEPTION 'replacement_code_hash_required'
      USING ERRCODE = 'P0001';
  END IF;
  ...
  IF claimed THEN
    ...
    INSERT INTO public.merchant_auth_recovery_codes (
      user_id,
      code_set_id,
      code_hash
    )
...
REVOKE ALL ON FUNCTION public.claim_merchant_auth_recovery_code(uuid, uuid, uuid, uuid, text, text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.claim_merchant_auth_recovery_code(uuid, uuid, uuid, uuid, text, text)
  TO service_role;
