-- Give resume-triggered timeout answers the same table-first ordering as
-- explicit answer submissions and quiz starts.
...
BEGIN
  ...
  -- Resume may insert a timeout answer. Acquire the compatible writer lock
  -- before event and attempt rows so score repair cannot form a lock cycle.
...
ALTER FUNCTION public.resume_quiz_attempt_v2(uuid, text) OWNER TO postgres;
