-- Permit staff who can view orders to refresh the authoritative, derived
-- payable amount before reusing an existing Paystack invoice account. Account
-- reservation and expired-account release remain restricted to order editors.
...
COMMENT ON FUNCTION public.refresh_paystack_order_payable_amount(uuid) IS
  'Refreshes an order Paystack account balance for authenticated order viewers.';
