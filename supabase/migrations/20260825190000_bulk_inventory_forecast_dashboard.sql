-- Replace the dashboard's browser-side page walk and per-product forecast RPCs
-- with one bounded, tenant-authorized server-side aggregation.
...
BEGIN
  ...
  WITH sales AS MATERIALIZED (
    ...
    SELECT
      ...
      COALESCE(NULLIF(p.low_stock_threshold, 0), 5)::integer
    ...
    SELECT *
    FROM visible
    ORDER BY days_of_stock ASC, product_id ASC
    LIMIT p_limit
    OFFSET p_offset
  )
  SELECT jsonb_build_object(
    'forecasts', COALESCE((
      SELECT jsonb_agg(
        ...
        ) ORDER BY r.days_of_stock ASC, r.product_id ASC
      )
      FROM page_rows r
    ), '[]'::jsonb),
