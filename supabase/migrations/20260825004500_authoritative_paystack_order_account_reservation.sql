-- Make Paystack order-account balance calculation and reservation fully
-- authoritative. Neither RPC accepts caller-provided matching amounts.

DROP FUNCTION IF EXISTS public.refresh_paystack_order_payable_amount(uuid, numeric);
DROP FUNCTION IF EXISTS public.reserve_paystack_order_payment_account(
  uuid, text, text, text, numeric, timestamptz, timestamptz
);

CREATE FUNCTION public.refresh_paystack_order_payable_amount(p_order_id uuid)
RETURNS numeric
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
