-- Prioritize every actionable inventory status before applying the dashboard
-- page limit so no critical or warning product is hidden by a healthy row.
-- This replay is append-only and retains the effective stock fallback.
