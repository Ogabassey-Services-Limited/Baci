-- Include wallet top-up credits when rebuilding customer wallet balances.
--
...
-- merchant wallets so stale aggregate rows do not hide credited cashback,
-- wallet top-ups, or merchant commissions.
...
BEGIN
  ...
  -- This locks customer_wallet_transactions, customer_wallets,
  -- wallet_transactions, merchant_wallets, and merchant_settlements in SHARE
  -- ROW EXCLUSIVE MODE, so run manually during low-traffic windows if the
  -- ledger grows large enough for the backfill to take more than a few seconds.
  LOCK TABLE
