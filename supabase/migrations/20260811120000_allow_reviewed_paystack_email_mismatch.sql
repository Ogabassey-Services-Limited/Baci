-- Permit a deliberately approved operator override when the provider email
-- differs from the canonical order email. The zero-candidate review keeps the
-- provider email as evidence; the override is explicit, service-role only,
-- and is copied into both the transaction and audit trail.
...
BEGIN
  ...
  -- The existing strict RPC remains the single completion implementation. It
  -- receives the canonical order email for its identity guard; the provider
  -- email is restored in transaction/review metadata immediately afterward.
...
COMMENT ON FUNCTION public.reconcile_paystack_unmatched_partial_payment(
  ...
  'Applies a verified unmatched Paystack payment, with an explicit audited operator override for provider/order email mismatches.';
