-- Fix ambiguous column reference 'total' and 'tax_amount' in create_storefront_order.
--
-- Inside the create_storefront_order function, the SELECT query:
--   SELECT total, tax_amount INTO v_total, v_tax_amount FROM orders WHERE id = v_order_id;
-- causes PostgreSQL compiler ambiguity since 'total' and 'tax_amount' match both
-- the RETURNS TABLE output columns and the orders table columns.
--
-- We qualify both fields using the table alias 'o'.
