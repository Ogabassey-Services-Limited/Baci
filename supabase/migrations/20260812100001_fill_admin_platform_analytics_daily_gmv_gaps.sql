-- Emit every Africa/Lagos calendar day in the selected analytics window so
-- charts distinguish a genuine zero-sales day from missing data.
...
AS $$
  ...
  current_orders AS MATERIALIZED (
    SELECT o.merchant_id, o.created_at,
    ...
    WHERE o.created_at >= p_start_at AND o.created_at < p_end_at
  ),
  ngn_current_orders AS MATERIALIZED (
    ...
    FROM current_orders WHERE currency = 'NGN'
  ),
  paid_current AS MATERIALIZED (
    SELECT merchant_id, created_at, total, payment_method, source
    FROM ngn_current_orders WHERE payment_status = 'paid'
  ),
  paid_all_current AS MATERIALIZED (
    SELECT merchant_id, created_at, total, currency
    FROM current_orders WHERE payment_status = 'paid'
  ),
  analytics_days AS MATERIALIZED (
    SELECT day::date AS sale_date
    FROM pg_catalog.generate_series(
      (p_start_at AT TIME ZONE 'Africa/Lagos')::date,
      ((p_end_at - interval '1 microsecond') AT TIME ZONE 'Africa/Lagos')::date,
      interval '1 day'
    ) AS day
  ),
  daily_paid_orders AS MATERIALIZED (
    SELECT (created_at AT TIME ZONE 'Africa/Lagos')::date AS sale_date,
      COALESCE(SUM(total) FILTER (WHERE currency = 'NGN'), 0)::numeric AS gmv,
      COUNT(*)::bigint AS orders, COUNT(DISTINCT merchant_id)::bigint AS merchants
    FROM paid_all_current GROUP BY 1
  )
  SELECT jsonb_build_object(
    'dailyGmv', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'date', d.sale_date, 'gmv', COALESCE(p.gmv, 0),
        'orders', COALESCE(p.orders, 0), 'merchants', COALESCE(p.merchants, 0)
      ) ORDER BY d.sale_date)
      FROM analytics_days d
      LEFT JOIN daily_paid_orders p USING (sale_date)
    ), '[]'::jsonb),
    ...
    'paymentStatuses', COALESCE((
      ...
      FROM (
        SELECT payment_status AS status, COUNT(*)::bigint AS orders,
          COALESCE(SUM(total) FILTER (WHERE currency = 'NGN'), 0)::numeric AS amount,
          SUM(COUNT(*)) OVER ()::bigint AS all_orders,
          SUM(SUM(total) FILTER (WHERE currency = 'NGN')) OVER ()::numeric AS all_amount,
        ...
        FROM current_orders GROUP BY payment_status
      ) s
    ), '[]'::jsonb),
    'shippingStatuses', COALESCE((
      ...
      FROM (
        SELECT shipping_status AS status, COUNT(*)::bigint AS orders,
          COALESCE(SUM(total) FILTER (WHERE currency = 'NGN'), 0)::numeric AS amount,
          SUM(COUNT(*)) OVER ()::bigint AS all_orders,
          SUM(SUM(total) FILTER (WHERE currency = 'NGN')) OVER ()::numeric AS all_amount,
        ...
        FROM current_orders GROUP BY shipping_status
      ) s
    ), '[]'::jsonb),
...
ALTER FUNCTION private.get_admin_platform_analytics_breakdowns_v1(timestamptz, timestamptz)
  OWNER TO postgres;
REVOKE ALL ON FUNCTION private.get_admin_platform_analytics_breakdowns_v1(timestamptz, timestamptz)
