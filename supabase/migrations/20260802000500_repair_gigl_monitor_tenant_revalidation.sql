-- Repair GIGL monitor ownership transitions that occur across two updates.
-- The marker makes tenant-driven deactivation distinguishable from an
-- intentionally inactive monitor, so the second update can safely revalidate it.
...
BEGIN
  ...
  IF NEW.order_id IS NULL
     ...
     OR NEW.merchant_id IS DISTINCT FROM v_order_merchant_id THEN
...
WHERE monitor.shipment_id = shipment.id
  ...
  AND (
    ...
    OR monitor.last_error IS DISTINCT FROM 'tracking_tenant_changed'
  );
