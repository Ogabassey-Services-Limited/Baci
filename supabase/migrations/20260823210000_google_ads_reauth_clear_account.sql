-- Clear the selected Google Ads account whenever the stored grant is marked
-- for reauthorization. The OAuth identity may now point at a different
-- manager or customer, so the old selection must be confirmed again.
