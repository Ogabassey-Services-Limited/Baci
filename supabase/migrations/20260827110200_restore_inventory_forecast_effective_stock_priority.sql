-- Restore the effective product stock fallback and status-priority ordering
-- after the zero-threshold replay so final forecast output stays consistent
-- with product updates and does not hide actionable rows behind the page limit.
-- This corrective replay is append-only.
      ...
      GREATEST(
        CASE
          ...
          WHEN COALESCE(p.stock_quantity, 0) = 0
            AND COALESCE(p.stock, 0) > 0 THEN p.stock
