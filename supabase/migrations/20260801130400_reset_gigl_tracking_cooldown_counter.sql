-- Reset the unchanged-poll counter before a paused monitor is polled again.
-- The cooldown claim is a forward repair for the resume RPC introduced by
-- 20260801120000; migrations remain append-only.
...
BEGIN
  ...
  WITH due_monitors AS (
    ...
    SET locked_at = now(),
      ...
      unchanged_poll_count = CASE
        WHEN monitor.state = 'paused' THEN 0
        ELSE monitor.unchanged_poll_count
      END,
      updated_at = now()
    FROM due_monitors
