-- Only suppress pending milestones on manual terminal status changes. The
-- tracking result RPC updates last_tracked_at with its carrier snapshot, so
-- its newly queued terminal notification must remain deliverable.
