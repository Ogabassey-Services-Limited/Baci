-- Per-slug storefront product resolution for proxy-level legacy redirects.
--
-- The proxy must distinguish three URL states before React/PPR streams a 200:
--   * active product slug       -> render canonical PDP
--   * archived redirect alias   -> real HTTP 308 to the active parent PDP
--   * absent/non-resolvable slug -> real HTTP 404
--
-- This keeps the public request path least-privilege: the app calls this through
-- an anon public client, while the SECURITY DEFINER function exposes only a
-- boolean verdict and the minimal parent fields needed to build a redirect path.
...
AS $$
  WITH normalized_input AS (
    SELECT lower(btrim(COALESCE(p_product_slug, ''))) AS slug
  ),
  merchant_ok AS (
    SELECT 1
