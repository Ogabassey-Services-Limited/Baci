-- Applies one validated provider observation only while its lease and epoch
-- still match, preventing delayed workers from reviving old shipment timelines.
...
BEGIN
  ...
  SET status = p_status,
    ...
    tracking_events = (
      SELECT coalesce(jsonb_agg(event ORDER BY event->>'occurred_at'), '[]'::jsonb)
      FROM (
        SELECT DISTINCT ON (coalesce(event->>'provider_event_key', event->>'provider_event_id', event->>'occurred_at')) event
        FROM jsonb_array_elements(coalesce(shipment.tracking_events, '[]'::jsonb) || p_events) AS event
        ORDER BY coalesce(event->>'provider_event_key', event->>'provider_event_id', event->>'occurred_at'), event->>'occurred_at'
      ) AS merged_events
    ),
    last_tracked_at = now(),
    updated_at = now()
...
CREATE OR REPLACE FUNCTION public.record_gigl_tracking_failure(
  p_shipment_id uuid, p_tracking_epoch_id uuid, p_worker_id text, p_error text
)
RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE v_failure_count integer;
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'GIGL tracking failure recording requires service role' USING ERRCODE = '42501';
  END IF;
  UPDATE public.shipment_tracking_monitors AS monitor
  SET consecutive_failures = monitor.consecutive_failures + 1,
    last_error = left(nullif(btrim(p_error), ''), 512), last_polled_at = now(),
    next_poll_at = now() + least(interval '60 minutes', interval '5 minutes' * power(2, least(monitor.consecutive_failures, 3))),
    locked_at = NULL, locked_by = NULL, updated_at = now()
  WHERE monitor.shipment_id = p_shipment_id
    AND monitor.tracking_epoch_id = p_tracking_epoch_id
    AND monitor.locked_by = btrim(p_worker_id)
  RETURNING consecutive_failures INTO v_failure_count;
  RETURN v_failure_count IS NOT NULL;
END;
$$;

ALTER FUNCTION public.apply_gigl_tracking_result(uuid, uuid, text, text, text, timestamptz, jsonb) OWNER TO postgres;
ALTER FUNCTION public.record_gigl_tracking_failure(uuid, uuid, text, text) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.apply_gigl_tracking_result(uuid, uuid, text, text, text, timestamptz, jsonb) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.record_gigl_tracking_failure(uuid, uuid, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.apply_gigl_tracking_result(uuid, uuid, text, text, text, timestamptz, jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.record_gigl_tracking_failure(uuid, uuid, text, text) TO service_role;
