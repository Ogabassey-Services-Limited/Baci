-- Extend the guarded merchant identity settings boundary with authored storefront copy.
-- Contact fields retain recent-auth and audit behavior; site_description is non-sensitive.
...
BEGIN
  ...
  IF p_settings ? 'site_description'
    AND pg_catalog.length(pg_catalog.btrim(p_settings ->> 'site_description')) > 320 THEN
    RAISE EXCEPTION 'invalid_site_description' USING ERRCODE = '22023';
  END IF;
  ...
  UPDATE public.merchants AS m
     SET business_name = CASE
         ...
         site_description = CASE
           WHEN p_settings ? 'site_description'
             THEN NULLIF(pg_catalog.btrim(p_settings ->> 'site_description'), '')
           ELSE m.site_description
         END,
...
COMMENT ON FUNCTION public.update_merchant_identity_settings(uuid, jsonb, timestamptz)
  IS 'Updates guarded merchant identity, contact, and storefront profile settings with optimistic concurrency.';
