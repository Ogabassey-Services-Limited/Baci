...
COMMENT ON COLUMN public.order_items.image_url IS
  'Selected product image snapshot captured at order time';

CREATE INDEX IF NOT EXISTS idx_orders_merchant_customer_email_lower
ON public.orders (merchant_id, lower(customer_email));
