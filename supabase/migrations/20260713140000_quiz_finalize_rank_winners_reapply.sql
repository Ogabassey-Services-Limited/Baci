-- Re-apply of 20260713130000_quiz_finalize_rank_winners.sql (Phase A "A3").
--
-- WHY THIS FILE EXISTS
-- The A3 migration was authored as 20260713130000_quiz_finalize_rank_winners.sql,
-- but that version was ALREADY claimed by
-- 20260713130000_add_storefront_paystack_subaccount_configured_rpc.sql, which
-- merged first. .github/scripts/apply-pending-migrations.sh keys on the version
-- prefix ALONE, so once the paystack file recorded version 20260713130000, the
-- quiz file was reported as "already applied" and its DDL never ran. Verified on
-- prod: supabase_migrations.schema_migrations has exactly one 20260713130000 row,
-- name='add_storefront_paystack_subaccount_configured_rpc', and neither
-- mint_quiz_event_ranked_awards nor finalize_due_quiz_events exists.
--
-- Because the version is recorded, the applier will never retry it. Migrations are
-- append-only, so the original file is left untouched and this forward migration
-- re-applies the identical DDL under a free version. Every statement below is
-- idempotent (DROP CONSTRAINT IF EXISTS + ADD, CREATE UNIQUE INDEX IF NOT EXISTS,
-- CREATE OR REPLACE FUNCTION), so it is a no-op on any database that somehow
