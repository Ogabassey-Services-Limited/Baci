-- Harden fulfillment notification enqueueing for real production paths:
--   1. Treat completed as a delivered notification event.
--   2. Do not emit customer-facing emails for imported/external order syncs.
--   3. Let authenticated manual send endpoints consume matching active outbox
--      rows after they send/skip, preventing duplicate cron emails for legacy
--      clients that still call /shipped or /delivered after status updates.
...
COMMENT ON FUNCTION public.complete_order_notification_outbox_manual_result(
  ...
  'Marks active fulfillment-notification outbox rows consumed by legacy manual shipped/delivered email endpoints.';
