...
ALTER TABLE public.agentic_request_records
  DROP CONSTRAINT IF EXISTS agentic_request_records_status_code_check;
ALTER TABLE public.agentic_request_records
  ADD CONSTRAINT agentic_request_records_status_code_check
  CHECK (status_code IS NULL OR status_code BETWEEN 100 AND 599);

COMMENT ON COLUMN public.agentic_request_records.status_code IS
  'HTTP outcome status for recorded signed requests, used for redacted merchant health reporting.';
  ...
  RETURN jsonb_build_object(
    ...
    COALESCE(
      (
        ...
        FROM (
          SELECT agent_id, api_version, route, status_code, created_at, expires_at
