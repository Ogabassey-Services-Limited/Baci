-- Remove stale orders.payment_reference writes from storefront order RPCs.
--
-- The orders table intentionally does not have payment_reference; gateway
-- references live in transactions/checkout_sessions. Keeping the RPCs from
-- writing that absent column preserves pending-order reuse without opening a
-- new orders-table contract.
...
COMMENT ON FUNCTION public.create_storefront_order(
  ...
  TEXT, JSONB, TEXT, TEXT, JSONB, UUID, TEXT, TEXT, UUID, TEXT, NUMERIC,
  NUMERIC, TEXT, TEXT
) IS 'Creates storefront orders. Fix: Qualified o.total and o.tax_amount re-read to resolve PostgreSQL scoping ambiguity.';
