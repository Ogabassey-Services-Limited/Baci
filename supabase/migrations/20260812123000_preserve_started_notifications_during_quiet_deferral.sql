-- Quiet-hour rescheduling must never make an already-started broadcast editable.
BEGIN;

CREATE OR REPLACE FUNCTION public.finalize_scheduled_admin_notification_v1(
  p_notification_id uuid, p_claim_token uuid, p_outcome text,
...
BEGIN
  ...
  ELSIF p_outcome = 'deferred' THEN
    UPDATE public.notifications SET delivery_state = 'pending', delivery_claimed_at = NULL, delivery_claim_token = NULL, delivery_last_error = NULL, scheduled_for = statement_timestamp() + interval '15 minutes' WHERE id = p_notification_id AND delivery_state = 'processing' AND delivery_claim_token = p_claim_token AND sent_at IS NULL;
  ELSE
...
REVOKE ALL ON FUNCTION public.finalize_scheduled_admin_notification_v1(uuid,uuid,text,text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.finalize_scheduled_admin_notification_v1(uuid,uuid,text,text) TO service_role;
COMMIT;
