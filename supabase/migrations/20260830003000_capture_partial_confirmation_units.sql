-- Confirm only inventory units whose reservation durability changed on this call.
-- Replayed confirmations must not emit duplicate hold_confirmed events.
-- Capture and clear those units in one row-locking UPDATE so a concurrent sale
-- cannot turn a stale pre-update snapshot into a false hold_confirmed event.
-- Partial reservations use the same status recheck before their expiry is cleared.
...
DECLARE
  ...
  v_newly_confirmed_unit_ids uuid[] := ARRAY[]::uuid[];
BEGIN
  ...
  LOOP
    ...
    IF v_reserved_count = v_item.quantity THEN
      ...
      INTO v_newly_confirmed_unit_ids
      FROM confirmed_units;

      FOR v_unit IN
        SELECT id
        FROM public.variant_inventory
        WHERE id = ANY(v_newly_confirmed_unit_ids)
        ORDER BY id
      LOOP
      ...
      END LOOP;
    ELSE
      WITH partially_confirmed_units AS (
        UPDATE public.variant_inventory
        SET reservation_expires_at = NULL,
            updated_at = now()
        WHERE order_item_id = v_item.id
      ...
      SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::uuid[])
      INTO v_newly_confirmed_unit_ids
      FROM partially_confirmed_units;
