-- Phase B — B3 (Δ-9 follow-up). Fail-closed for missing shipping quote.
--
-- Pre-B3, `create_storefront_order` silently accepted requests with
-- `p_shipping_provider` set but `p_selected_quote_id` missing — the
-- order was created with `shipping_provider` populated but no quote
-- linkage, and downstream label-printing / tracking integrations had
-- no way to look up the actual carrier rate. The legacy storefront
-- checkout (`apps/web/src/app/checkout/page.tsx:1217`) defaulted
-- `shipping_provider` to `'GIGL'` when no quote was selected, so the
-- silent-drop bug was easy to trigger.
--
-- B3 (per plan §5 B3):
--   (a) RPC raises `shipping_quote_required` when shipping_provider
--       is supplied but no quote id is present. Storefront callers
--       MUST attach a quote id (pickup/airport callers send
--       `shipping_provider: null` so they bypass the check).
--   (b) Client callers updated in the same PR: pickup/airport stop
--       sending `shipping_provider` so this check doesn't trip; the
--       legacy `|| 'GIGL'` fallback in `apps/web/src/app/checkout/page.tsx`
--       is dropped so callers without a quote send null shipping
--       provider and the check is a no-op.
--
-- Everything else in the function body is unchanged from
-- 20260512120000_fix_storefront_order_customer_upsert.sql (B2).
...
BEGIN
  ...
  -- B3 (plan §5 B3): fail-closed when a shipping provider is supplied
  -- without a corresponding quote id. Pre-B3 the legacy checkout fell
  -- back to `shipping_provider: 'GIGL'` even with no quote, persisting
  -- the order with shipping_provider populated but no carrier rate
  -- linkage — label printing / tracking integrations had nothing to
  -- query. Pickup/airport flows MUST send `p_shipping_provider: NULL`
  -- (they're delivery-method labels, not third-party providers) so
  -- this check is a no-op for them. Storefront callers updated in
  -- the same PR to satisfy this contract.
  ...
  -- AND concurrent same-identity inserts.
  -- ====================================================================

  -- Existing phone advisory lock (preserved from prior migration):
  -- serializes same-phone-but-different-email retries.
  ...
  -- B2: additional advisory locks on (merchant_id, user_id) and
  -- (merchant_id, email). Two concurrent same-identity calls serialize
  -- here so the SELECT below sees the winner's row instead of both
  -- landing in the INSERT branch and 23505'ing on the same constraint.
  --
  -- The second argument to pg_advisory_xact_lock disambiguates the
  -- lock namespace from the phone lock above (which uses
  -- (hashtext(merchant), hashtext(phone))). hashtextextended takes a
  -- salt as second arg so we vary it per identity-dimension to avoid
  -- collisions across user_id/email locks.
  ...
  -- Existing primary lookup: by user_id (if present), then by phone.
  -- Both use SELECT FOR UPDATE so a concurrent same-row UPDATE blocks
  -- until we're done.
  IF p_user_id IS NOT NULL THEN
    SELECT c.id
      INTO v_customer_id
  ...
  -- B2: fall back to email lookup so the retry loop's SELECT path
  -- catches "same email different user_id" races BEFORE the INSERT.
  ...
  IF v_customer_id IS NOT NULL THEN
    -- Found-path: defensive UPDATE that respects the four unique
    -- indexes. Each conditional COALESCE checks for cross-row
    -- conflicts before overwriting fields.
    UPDATE customers c
    SET
      email = CASE
  ...
  ELSE
    -- Insert-path: phone-conflict-skip preflight (preserved) — if
    -- another customer in this merchant already has this phone,
    -- insert with NULL phone so we don't trip
    -- `customers_merchant_phone_unique`. The retry loop below would
    -- otherwise spin until exhausted because re-SELECT by user_id
    -- OR email finds nothing (different customer owns the phone).
    ...
    -- B2 (Δ-9): retry loop. Catches ANY of the customer unique
    -- indexes — `customers_merchant_email_unique`,
    -- `idx_customers_merchant_email`, `idx_customers_merchant_user`,
    -- `customers_merchant_phone_unique` — by re-SELECTing on
    -- unique_violation. The advisory locks above usually mean the
    -- second iteration's SELECT finds the winner; the 3-iteration
    ...
    LOOP
      ...
      EXCEPTION WHEN unique_violation THEN
        -- A peer transaction won the race. Re-SELECT by user_id OR
        -- email — whichever index we tripped, the winner's row will
        -- be visible now (the peer has committed by the time our
        -- INSERT raised). UPDATE that row defensively, same pattern
        -- as the found-path above.
        v_customer_id := NULL;
        ...
        IF v_customer_id IS NOT NULL THEN
          ...
          EXIT;
        END IF;
        -- Race-with-no-visible-winner: a unique index other than
        -- user_id/email tripped (e.g., phone). Loop and try again
        -- with the phone-conflict preflight already in effect.
      END;
    END LOOP;
  END IF;

  -- ====================================================================
  -- End B2 customer block; rest of the RPC is unchanged.
  -- ====================================================================
...
COMMENT ON FUNCTION public.create_storefront_order(
  ...
  'Creates storefront orders. B2 (Δ-9): customer upsert resilient to all customer unique indexes (email/user_id/phone) and concurrent same-identity inserts via advisory locks + 3-iteration retry-on-unique_violation loop. B3: raises `shipping_quote_required` when p_shipping_provider IS NOT NULL AND p_selected_quote_id IS NULL.';
