...
DECLARE
  ...
  v_registered_is_empty boolean;
BEGIN
  v_formatted_address := public.format_merchant_address(NEW.registered_address);

  -- A legacy free-text write may only survive when there is NO structured
  -- address at all (registered_address null or `{}`). A non-empty structured
  -- object that merely lacks displayable parts (e.g. `{"country":"Nigeria"}`
  -- from a partial tax save) is NOT a legacy free-text row: business_address is
  -- then DERIVED (NULL), so a direct business_address write can never repopulate
  -- it while the structured address stays semantically empty (drift).
  v_registered_is_empty := NEW.registered_address IS NULL
    OR NEW.registered_address = '{}'::jsonb;

  IF v_formatted_address IS NOT NULL THEN
    NEW.business_address := v_formatted_address;
  ELSIF TG_OP = 'INSERT' THEN
    IF v_registered_is_empty THEN
