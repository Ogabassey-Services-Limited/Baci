...
COMMENT ON FUNCTION public.replace_order_items(UUID, JSONB, UUID, BOOLEAN, JSONB) IS
  'Atomically updates one merchant-owned order and replaces its order_items rows from JSONB payloads. Import callers may override order and item timestamps, preserve rounded marketplace line totals, and pass receipt display snapshots such as condition, variant_name, and image_url.';
