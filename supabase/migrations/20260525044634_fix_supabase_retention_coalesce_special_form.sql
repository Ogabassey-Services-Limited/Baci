...
BEGIN
  ...
  WHERE events.event_type IN ('page_view', 'search')
    AND COALESCE(events.event_timestamp, events.created_at) < pg_catalog.clock_timestamp() - p_analytics_low_value_retention;
  GET DIAGNOSTICS analytics_events_deleted = ROW_COUNT;

  DELETE FROM cron.job_run_details AS runs
  WHERE COALESCE(runs.end_time, runs.start_time) < pg_catalog.clock_timestamp() - p_cron_retention;
  GET DIAGNOSTICS cron_job_run_details_deleted = ROW_COUNT;

  DELETE FROM net._http_response AS responses
  WHERE responses.created < pg_catalog.clock_timestamp() - p_pg_net_retention;
