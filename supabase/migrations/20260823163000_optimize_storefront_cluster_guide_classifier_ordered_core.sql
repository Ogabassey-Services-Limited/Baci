...
CREATE OR REPLACE FUNCTION private.classify_storefront_cluster_guide_candidates_v2(
  p_merchant_id uuid,
  p_cluster_rules jsonb,
  p_search_query pg_catalog.tsquery,
  p_requested_category_slug text,
...
requested_terms AS MATERIALIZED (
  SELECT
    'category'::text AS term_kind,
    pg_catalog.lower(pg_catalog.btrim(category_name.value)) AS term
  FROM cluster_rules AS rule
  CROSS JOIN LATERAL pg_catalog.unnest(rule.category_names)
    AS category_name(value)
  WHERE rule.category_slug = p_requested_category_slug
  UNION ALL
  SELECT
    'semantic'::text,
    pg_catalog.lower(pg_catalog.btrim(article_token.value))
  FROM cluster_rules AS rule
  CROSS JOIN LATERAL pg_catalog.unnest(rule.article_tokens)
    AS article_token(value)
  WHERE rule.category_slug = p_requested_category_slug
),
searchable_posts AS MATERIALIZED (
  SELECT
...
matching_posts AS MATERIALIZED (
  SELECT
    candidate.post_slug, candidate.post_title, candidate.post_excerpt,
    candidate.post_category, candidate.post_tags, candidate.post_keywords,
    candidate.post_featured_image_url, candidate.post_published_at,
    candidate.post_reading_time_minutes, candidate.explicit_category,
    candidate.semantic_haystack, candidate.search_rank
  FROM searchable_posts AS candidate
  WHERE EXISTS (
    SELECT 1
    FROM requested_terms AS requested
    WHERE pg_catalog.strpos(
      CASE requested.term_kind
        WHEN 'category' THEN candidate.explicit_category
        ELSE candidate.semantic_haystack
      END,
      requested.term
    ) > 0
  )
...
COMMENT ON FUNCTION private.classify_storefront_cluster_guide_candidates_v2(uuid, jsonb, pg_catalog.tsquery, text, integer) IS
  'Internal bounded classifier for one storefront guide category. The requested-category candidate filter, ranking, and cap stay inside the definer boundary; exact category values use a hash join and only unmatched posts use the substring fallback.';

ALTER FUNCTION private.classify_storefront_cluster_guide_candidates_v2(uuid, jsonb, pg_catalog.tsquery, text, integer) OWNER TO postgres;

REVOKE ALL ON FUNCTION private.classify_storefront_cluster_guide_candidates_v2(uuid, jsonb, pg_catalog.tsquery, text, integer) FROM PUBLIC;

GRANT USAGE ON SCHEMA private TO postgres, service_role;

GRANT EXECUTE ON FUNCTION private.classify_storefront_cluster_guide_candidates_v2(uuid, jsonb, pg_catalog.tsquery, text, integer) TO postgres, service_role;
