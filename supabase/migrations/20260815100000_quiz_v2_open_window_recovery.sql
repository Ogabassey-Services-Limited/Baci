-- Recover a submitted attempt before the universal event window closes. The
-- player may finish early and restart the app while rankings are still being
-- finalized, so losing the attempt id would strand result polling and prize
-- claims. A started attempt is always preferred, preventing an older terminal
-- row from masking a currently active start.
...
BEGIN
  ...
  -- Prefer a live attempt above. If none exists, a submitted attempt is still
  -- recoverable while the event is open because its result may be pending.
...
COMMENT ON FUNCTION public.resume_quiz_attempt_v2(uuid, text) IS
  'Owner-scoped v2 quiz recovery, including open-window terminal attempts.';
