-- Keep provider-only match context in the durable envelope without copying it
-- into the product analytics/platform-event tables or their public dashboards.
...
BEGIN
  IF COALESCE((SELECT auth.role()), '') <> 'service_role' THEN
    RAISE EXCEPTION 'forbidden: record_analytics_domain_event_v1 requires service_role' USING ERRCODE = '42501';
  END IF;
  ...
  IF jsonb_typeof(COALESCE(p_event_data, '{}'::jsonb)) <> 'object'
    ...
    OR jsonb_typeof(COALESCE(p_delivery_data, '{}'::jsonb)) <> 'object' THEN
...
GRANT EXECUTE ON FUNCTION public.record_analytics_domain_event_v1(uuid, text, text, jsonb, jsonb, jsonb, text, text, text, text, timestamptz, jsonb) TO service_role;
...
BEGIN
  IF COALESCE((SELECT auth.role()), '') <> 'service_role' THEN
    RAISE EXCEPTION 'forbidden: record_platform_domain_event_v1 requires service_role' USING ERRCODE = '42501';
  END IF;
  IF p_external_event_id IS NULL OR length(p_external_event_id) = 0 THEN
    RAISE EXCEPTION 'platform_external_event_id_required' USING ERRCODE = '22023';
  END IF;
  IF jsonb_typeof(COALESCE(p_delivery_data, '{}'::jsonb)) <> 'object' THEN
    RAISE EXCEPTION 'platform_delivery_data_object_required' USING ERRCODE = '22023';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.record_platform_domain_event_v1(text, text, jsonb, jsonb, text, uuid, text, text, text, text, text, timestamptz, jsonb) TO service_role;
