-- Harden provider-neutral ads RPCs after the Google-only baseline. Google keeps
-- using its immutable v1 envelope and Google-specific RPCs; these shared RPCs
-- are exclusively for social providers with v2 provider-bound ciphertext.
...
BEGIN
  IF NOT public.check_staff_permission(
    (SELECT auth.uid()), p_merchant_id, 'integrations', 'manage'
  ) OR p_provider NOT IN ('meta_ads', 'tiktok_ads', 'snapchat_ads') THEN
    RETURN false;
  END IF;
  UPDATE public.merchant_ad_connections
