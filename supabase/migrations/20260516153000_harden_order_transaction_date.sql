...
CREATE OR REPLACE FUNCTION public.set_order_transaction_date_from_created_at()
RETURNS trigger
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  IF NEW.transaction_date IS NULL THEN
    NEW.transaction_date := COALESCE(NEW.created_at, now());
  END IF;
...
DROP TRIGGER IF EXISTS set_order_transaction_date_from_created_at ON public.orders;

CREATE TRIGGER set_order_transaction_date_from_created_at
  BEFORE INSERT ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.set_order_transaction_date_from_created_at();
...
BEGIN
  ...
  IF (p_transaction_date AT TIME ZONE 'Africa/Lagos')::date >
