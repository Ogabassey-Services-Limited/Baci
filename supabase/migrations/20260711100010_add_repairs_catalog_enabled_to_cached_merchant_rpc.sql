-- Wire repairs_catalog_enabled into the storefront merchant-resolution RPC.
--
-- main's resolve_storefront_cached_merchant (20260707211507) builds the public
-- feature_settings jsonb from an explicit allowlist of jsonb_build_object pairs
-- and predates the repairs stack, so it never surfaced repairs_catalog_enabled.
-- Under that RPC architecture the storefront merchant shell (getCachedMerchant /
-- getCachedMerchantByDomain in apps/web/src/lib/cached-data.ts) would therefore
-- never receive the repairs flag.
--
-- This follow-up re-creates the function verbatim from 20260707211507 and adds
...
--
-- The three jsonb_build_object sub-objects stay concatenated with `||` so no
-- single call exceeds Postgres' 100-argument (50-pair) limit; sub-object two now
-- holds 25 pairs (50 arguments), still within the cap.
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      -- jsonb_build_object caps at 100 arguments (50 pairs); the 61 public
      -- feature-settings fields are built in three sub-objects concatenated
      -- with || so no single call exceeds the limit. This mirrors the live
      -- production function definition exactly (verified via pg_get_functiondef).
      ELSE (
        jsonb_build_object(
        ...
        jsonb_build_object(
          ...
          'repairs_catalog_enabled', fs.repairs_catalog_enabled,
