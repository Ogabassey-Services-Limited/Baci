-- Fix: refresh_product_variant_media_projection regressed in
-- 20260425012000_canonicalize_variant_media_color_keys.sql which replaced
-- min(id::text) with bare min(id). Postgres has no min(uuid) aggregate, so
-- every INSERT/UPDATE/DELETE on product_variants raises:
--   ERROR 42883: function min(uuid) does not exist
--
-- Restore the ::text cast so the BEFORE/AFTER trigger stops aborting writes.
-- The cast preserves a stable per-color tiebreak ordering when the same
-- created_at appears across variants.
