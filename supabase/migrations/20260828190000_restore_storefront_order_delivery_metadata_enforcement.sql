-- Restore the full delivery-metadata validator after the rollout-only
-- persistence bridge in 20260828151100. This append-only repair keeps normal
-- filename-order replays and production postdeploys on the same enforcement
-- semantics; the bridge must not be the final definition of this trigger.
...
BEGIN
  ...
  -- Preserve legacy airport labels without inferring from fee amounts.
...
COMMENT ON FUNCTION private.validate_storefront_order_delivery_metadata() IS
  'Restores server-owned storefront delivery metadata persistence and local airport fee and quote enforcement after the rollout compatibility bridge.';
