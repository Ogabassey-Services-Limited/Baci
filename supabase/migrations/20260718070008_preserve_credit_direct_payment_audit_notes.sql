-- Credit Direct client completion and signed webhooks can overlap. A webhook
-- update is built from an earlier notes snapshot, so preserve the latest
-- database-owned audit fields while still allowing provider confirmation
-- fields from the webhook to win.
...
BEGIN
  ...
  -- Retain unrelated fields added after the webhook read. New webhook-owned
  -- values still replace the corresponding old values below.
  v_merged_notes := v_old_notes || v_new_notes;
...
DROP TRIGGER IF EXISTS preserve_credit_direct_payment_audit_notes
  ON public.orders;
CREATE TRIGGER preserve_credit_direct_payment_audit_notes
BEFORE UPDATE OF notes ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.preserve_credit_direct_payment_audit_notes();
