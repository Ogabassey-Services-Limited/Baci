-- Restore paid-repair GIGL tracking hardenings dropped by the orderless
-- repair-pickup apply/activate rewrite: failed-event recovery, delivered_at
-- only when delivery wins, and manual-failed monitor terminality.
...
DECLARE
  ...
  v_manual_terminal_failed boolean := false;
...
BEGIN
  ...
  v_effective_status := CASE
    ...
    WHEN v_current_status IS NOT NULL
      ...
      AND v_latest_status_event_at < v_latest_persisted_status_event_at
      THEN v_current_status
    WHEN v_current_status = 'failed'
      AND p_status IN ('picked_up', 'in_transit', 'out_for_delivery')
      AND v_latest_status_event_at IS NOT NULL
      AND (
        v_latest_persisted_status_event_at IS NULL
        OR v_latest_status_event_at >= v_latest_persisted_status_event_at
      )
      THEN p_status
  ...
  v_manual_terminal_failed := v_effective_status = 'failed'
    AND v_manual_terminal_override_at IS NOT NULL
    AND (
      v_latest_incoming_event_at IS NULL
