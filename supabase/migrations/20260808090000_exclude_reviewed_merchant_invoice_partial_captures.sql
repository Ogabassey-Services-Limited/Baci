-- Reviewed merchant-invoice partial captures are reconciliation records, not
-- invoice payments. Keep them out of both the wrapper and the locked delegate.
...
COMMENT ON FUNCTION public.complete_merchant_invoice_partial_payment(
  ...
  'Excludes reviewed merchant-invoice partial captures from locked invoice-payment totals while allowing exact-completion retries to heal standard payment side effects.';
