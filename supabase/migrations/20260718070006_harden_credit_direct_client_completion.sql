-- Preserve the previously deployed implementation as a non-callable internal
-- worker, then put a signed-session and idempotency guard in front of it.
ALTER FUNCTION public.record_credit_direct_client_completion(uuid, text, text, text)
  RENAME TO record_credit_direct_client_completion_v1;

REVOKE ALL ON FUNCTION public.record_credit_direct_client_completion_v1(uuid, text, text, text)
...
BEGIN
  ...
  IF v_request_role <> 'service_role' THEN
    IF NOT (
      ...
      OR (
        p_email IS NOT NULL
        AND lower(p_email) = lower(trim(v_order.customer_email))
      )
      OR (
  ...
  -- Browser completion evidence is accepted only for the currently signed
  -- checkout session. The service role retains its repair capability.
  ...
  -- Return before the legacy UPDATE so a duplicate callback cannot fire the
  -- orders updated_at trigger and keep an abandoned order alive indefinitely.
  IF v_completion_unchanged AND v_order.payment_status <> 'paid' THEN
    RETURN jsonb_build_object('status', 'pending_confirmation');
  END IF;

  -- The revoked v1 worker still performs its legacy tracking-token check.
  -- After this wrapper validates an email-only guest, pass the order's
  -- server-held token into that non-callable worker so it does not discard the
  ...
  RETURN public.record_credit_direct_client_completion_v1(
    ...
    CASE
      WHEN p_email IS NOT NULL
        AND lower(p_email) = lower(trim(v_order.customer_email))
      THEN v_order.tracking_token
      ELSE p_tracking_token
    END
  );
END;
$$;
...
COMMENT ON FUNCTION public.record_credit_direct_client_completion(uuid, text, text, text, text) IS
  'Records untrusted Credit Direct SDK completion only for the current signed session and ignores unchanged retries.';
