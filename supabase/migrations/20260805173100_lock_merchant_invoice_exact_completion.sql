-- Keep the exact remaining-balance check and completion under one lock.

ALTER FUNCTION public.complete_order_gateway_payment(
  uuid, uuid, jsonb, text
) RENAME TO complete_order_gateway_payment_v1;

REVOKE ALL ON FUNCTION public.complete_order_gateway_payment_v1(
  uuid, uuid, jsonb, text
...
COMMENT ON FUNCTION public.complete_order_gateway_payment(
  ...
  'Rechecks marked merchant-invoice final balances under the order payment lock, files stale allocations for review, and marks successful exact completions idempotently.';
