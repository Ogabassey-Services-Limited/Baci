...
-- definer (owner/active-staff via has_merchant_access, or published storefront
-- for customer payment routing) so it never widens access beyond the raw table.
--
-- has_merchant_access(uuid) is an existing SECURITY DEFINER helper (owner OR
-- active staff) — it also breaks the merchants<->staff_members policy recursion.
...
-- READ: Paystack subaccount code. Owner/staff (any publish state) OR a customer
-- of a published storefront (needed to route split payments at checkout).
...
AS $$
  ...
  WHERE m.id = p_merchant_id
    AND (m.is_published IS TRUE OR public.has_merchant_access(m.id));
$$;

-- READ: Virtual terminal code. Owner/staff only (a merchant-management field;
-- not part of any customer payment flow).
...
-- null)` whose WHERE filter needs SELECT on the revoked column). Owner/staff.
...
AS $$
  ...
  WHERE id = p_merchant_id
    ...
    AND public.has_merchant_access(p_merchant_id);
$$;
...
-- 'virtual_terminal_code', code)` whose WHERE filter needs SELECT). Owner/staff.
