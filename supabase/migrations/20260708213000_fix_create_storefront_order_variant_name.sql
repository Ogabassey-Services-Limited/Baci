-- Preserve explicit storefront order item variant labels through the
-- create_storefront_order RPC. The private function already writes
-- order_items.variant_name, but older definitions only derived it from
-- variant_attributes and dropped payload variant_name / variantName values.
