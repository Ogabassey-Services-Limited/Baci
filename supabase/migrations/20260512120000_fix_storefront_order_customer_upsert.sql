-- Phase B — B2 (Δ-9). Hardens `create_storefront_order`'s customer
-- upsert against ALL `customers` unique-violation paths AND concurrent
-- inserts.
--
-- The Efosa incident hit `idx_customers_merchant_user` (UNIQUE on
-- (merchant_id, user_id) WHERE user_id IS NOT NULL) — TWO `/api/orders`
-- POSTs landed in flight at the same instant for the same authed
-- buyer; the existing `ON CONFLICT ON CONSTRAINT
-- customers_merchant_id_email_key` only catches the email path, so
-- the second call 500'd with Postgres 23505.
--
-- Strategy (Δ-9 combined defense-in-depth):
--   (a) Acquire two transaction-scoped advisory locks BEFORE the SELECT:
--       one on (merchant_id, user_id) and one on (merchant_id, email).
--       Concurrent same-identity calls serialize here.
--   (b) Replace the single `INSERT ... ON CONFLICT email` with a
--       3-iteration retry loop: SELECT (covering both user_id and
--       email lookup); UPDATE if found; else INSERT inside an
--       EXCEPTION WHEN unique_violation block that re-loops the SELECT
--       on conflict. After 3 failed iterations raise
--       `customer_upsert_failed`.
--   (c) Preserve the existing phone-conflict-skip preflight (NULL out
--       the phone when another customer already has it) so the
--       `customers_merchant_phone_unique` index doesn't churn the
--       retry loop.
--
-- Everything outside the customer block (item validation, totals,
-- stock decrement, order INSERT, RETURN QUERY) is unchanged from
-- 20260509083500_resolve_storefront_order_customer_phone_conflicts.sql.
...
COMMENT ON FUNCTION public.create_storefront_order(
  ...
  'Creates storefront orders. B2 (Δ-9): customer upsert is now resilient to all customer unique indexes (email/user_id/phone) and concurrent same-identity inserts via advisory locks + 3-iteration retry-on-unique_violation loop.';
