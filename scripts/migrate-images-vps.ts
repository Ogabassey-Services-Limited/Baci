    ...
    for (const item of oldImages) {
        ...
        if (!match) {
            ...
            for (const p of newProducts) {
                ...
                let intersection = 0;
                newTokens.forEach(t => { if (oldTokens.has(t) && t.length > 2) intersection++; });
        ...
        if (match) {
            ...
            commands.push(`[ -f "/var/www/cdn/products/${filename}" ] && mv "/var/www/cdn/products/${filename}" "/var/www/cdn/products/${newFilename}"`);
            ...
            const cdnUrl = `https://cdn.ogabassey.com/products/${newFilename}`;
...
`;
    ...
    fs.writeFileSync('vps-migration.sh', vpsScript);
    ...
    fs.writeFileSync('supabase-updates.json', JSON.stringify(Object.values(updatePayloads), null, 2));
    console.log("📄 Generated supabase-updates.json");

    // Log unmatched validation
    fs.writeFileSync('unmatched_images.json', JSON.stringify(unmatched, null, 2));
}
