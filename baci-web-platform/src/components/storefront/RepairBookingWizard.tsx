...
import { repairBookingSchema, type RepairBookingInput } from '@/lib/validations/repair';
import { createRepair, calculateRepairShipping, type ShippingCalculationResult } from '@/app/actions/repair';
import { PlaceDetails } from '@/components/address-autocomplete';
...
export function RepairBookingWizard({ merchantId, merchantName }: RepairBookingWizardProps) {
    const [currentStep, setCurrentStep] = useState(0);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [isSuccess, setIsSuccess] = useState(false);
    const [shippingQuote, setShippingQuote] = useState<ShippingCalculationResult | null>(null);
    ...
    const nextStep = async () => {
        ...
        if (currentStep === 0) {
            ...
            fieldsToValidate = ['customerName', 'customerEmail', 'customerPhone', 'preferredDate'];
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                toast({
                    ...
                    description: 'We have received your repair request. We will contact you shortly.',
                });
            } else {
                toast({
                    variant: 'destructive',
        ...
        return (
            ...
            >
                ...
                <p className="text-muted-foreground mb-8">
                    Thanks for booking a repair with {merchantName}. Your ticket number will be sent to your email. We'll be in touch shortly to confirm the details.
                </p>
                <Button asChild>
                ...
                <div className="flex justify-between mb-2">
                    {steps.map((step, index) => (
                        <div
                            ...
                            style={index <= currentStep ? { color: 'var(--theme-primary, #dc2626)' } : undefined}
                        >
                            {step.title}
                        </div>
                    ))}
                            ...
                            >
                                <div className="space-y-4">
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel className="text-base">What device needs repair?</FormLabel>
                                                <FormControl>
                                                    <RadioGroup
                        ...
                        {currentStep === 1 && (
                            ...
                            >
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-base">How would you like to proceed?</FormLabel>
                                            <FormControl>
                                                <RadioGroup
                                                    onValueChange={field.onChange}
                                                    defaultValue={field.value}
                                                ...
                                                >
                                                    <FormItem>
                                                        <FormControl>
                                                            <RadioGroupItem value="dropoff" className="peer sr-only" />
                                                        </FormControl>
                                                        <FormLabel className="flex flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-current [&:has([data-state=checked])]:border-current cursor-pointer transition-all" style={{ borderColor: field.value === 'dropoff' ? 'var(--theme-primary, #dc2626)' : undefined }}>
                                                            <MapPin className="mb-2 h-6 w-6" />
                                                            <span className="font-medium">Drop-off</span>
                                                            <span className="text-xs text-muted-foreground text-center">I'll bring it to you</span>
                                                        </FormLabel>
                                                    </FormItem>
                                                    <FormItem>
                                                        <FormControl>
                                                        ...
                                                        <FormLabel className="flex flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-current [&:has([data-state=checked])]:border-current cursor-pointer transition-all" style={{ borderColor: field.value === 'pickup' ? 'var(--theme-primary, #dc2626)' : undefined }}>
                                                            <Truck className="mb-2 h-6 w-6" />
                                                            <span className="font-medium">Pickup</span>
                                                            <span className="text-xs text-muted-foreground text-center">Pick up from me</span>
                                                        </FormLabel>
                                                    </FormItem>
                                                </RadioGroup>
                                            </FormControl>
                                            ...
                                            <FormItem>
                                                ...
                                                {isCalculatingShipping && (
                                                    <div className="flex items-center text-sm text-muted-foreground mt-2">
                                                        <Loader2 className="w-3 h-3 mr-2 animate-spin" /> Calculating pickup fee...
                                                    </div>
                                                )}
                        ...
                        {currentStep === 2 && (
                            ...
                            >
                                <div className="bg-muted/50 rounded-lg p-6 space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div>
                                            ...
                                            <p className="font-medium">{formData.deviceType} - {formData.deviceModel}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">{formData.customerPhone}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div className="col-span-2">
                                            ...
                                            <div className="flex justify-between items-center">
                                                <p className="font-medium capitalize flex items-center">
                                                    ...
                                                    {formData.serviceType === 'pickup' && formData.pickupAddress && (
                                                ...
                                                {formData.serviceType === 'pickup' && shippingQuote && (
                                                    <span className={cn(
                                                        ...
                                                        shippingQuote.isFree ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"
                                                    )}>
                                ...
                                <div className="bg-blue-50 text-blue-800 p-4 rounded-md flex gap-3 items-start text-sm">
                                    ...
                                    <p>
                                        Submitting this request does not guarantee an appointment slot. The merchant will review and confirm availability.
                                    </p>
                                </div>
                            </motion.div>
                        )}
