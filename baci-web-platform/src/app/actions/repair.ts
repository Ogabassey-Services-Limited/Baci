...
export async function createRepair(
    ...
    try {
        ...
        if (error) {
            ...
            return { success: false, error: 'Failed to submit repair request. Please try again.' };
        }
...
export async function calculateRepairShipping(place: PlaceDetails): Promise<ShippingCalculationResult> {
    try {
        ...
        const validQuotes = quotes.filter((q) => q.price > 0).sort((a, b) => a.price - b.price);
