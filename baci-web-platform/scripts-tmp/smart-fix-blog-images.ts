...
const FALLBACK_IMAGE = 'https://cdn.ogabassey.com/blog/2025/07/Apple_MacBook-Pro_14-16-inch_10182021_big.jpg.large_2x-1024x1024.avif';
...
async function smartFixBlogImages() {
    ...
    for (const p of posts) {
        ...
        if (!p.featured_image_url) {
            ...
            try {
                const res = await fetch(p.featured_image_url, { method: 'HEAD', timeout: 5000 });
        ...
        if (isBroken) {
            ...
            const keywords = p.title.split(' ').slice(0, 3).join(' ').replace(/[:,]/g, '');
            ...
            if (products && products.length > 0) {
                ...
                for (const prod of products) {
                    if (prod.images && prod.images.length > 0) {
                        ...
                        try {
                            const res = await fetch(prodImg, { method: 'HEAD', timeout: 3000 });
                            if (res.status === 200) {
                                newImageUrl = prodImg;
                ...
                console.log(`   ⚠️ No matching product found for keywords "${keywords}". Using fallback.`);
            }
