...
compose_env_file_refs() {
  awk '
    ...
    function flow_scalars(line, body, i, c, sq, dq, esc, item, count) {
      if (line !~ /^\[[[:space:]]*.*[[:space:]]*\]$/) return 0
      body=line; sub(/^\[[[:space:]]*/, "", body); sub(/[[:space:]]*\]$/, "", body); if (body == "") return 0
...
compose_build_refs() { awk '
  ...
  function field(line, key, value) { key=line; sub(/[[:space:]]*:.*$/, "", key); value=line; sub(/^[^:]*:/, "", value); value=trim(value); if (value == "") return 0; if (key == "context") { if (context != "") return 0; context=value; return 1 }; if (key == "dockerfile") { if (dockerfile != "") return 0; dockerfile=value; return 1 }; return 0 }
  {
    ...
    if (line ~ /^[[:space:]]*build:[[:space:]]*/) { value=line; sub(/^[[:space:]]*build:[[:space:]]*/, "", value); level=n; if (value == "") { inside=1; field_level=-1; context=""; dockerfile=""; next }; if (value ~ /^[\\[{]/) { bad=1; next }; print value "\tDockerfile"; next }
...
scan_compose_definitions() { list=$(temp_path); tab=$(printf '\t'); for configured_root in $COMPOSE_ROOTS; do [ ! -e "$configured_root" ] && continue; root=$(compose_project_root "$configured_root") || { rm -f "$list"; return 2; }; found=$(temp_path); find "$root" -maxdepth 5 -type f \( -name 'docker-compose*.yml' -o -name 'docker-compose*.yaml' -o -name 'compose*.yml' -o -name 'compose*.yaml' -o -name 'Containerfile' \) >"$found" || { rm -f "$list" "$found"; return 2; }; while IFS= read -r path || [ -n "$path" ]; do printf '%s\t%s\n' "$root" "$path" >>"$list" || { rm -f "$list" "$found"; return 2; }; done <"$found"; rm -f "$found"; done; while IFS="$tab" read -r root path || [ -n "$root$path" ]; do [ -n "$root" ] && [ -n "$path" ] || { rm -f "$list"; return 2; }; if record=$(consumer_matched_fingerprint "$path"); then printf '%s\n' "$record"; else status=$?; [ "$status" -eq 1 ] || { rm -f "$list"; return "$status"; }; fi; case "$path" in *Containerfile) ;; *) scan_compose_environment_files "$path" || { status=$?; rm -f "$list"; return "$status"; }; scan_compose_build_files "$path" "$root" || { status=$?; rm -f "$list"; return "$status"; };; esac; done <"$list"; rm -f "$list"; }
...
systemd_linked_definitions() {
  ...
  while IFS= read -r candidate || [ -n "$candidate" ]; do
    ...
    if grep -H -E '^[[:space:]]*EnvironmentFile=' "$definition" >>"$environment_files"; then :; else status=$?; [ "$status" -eq 1 ] || { rm -f "$inventory" "$seen"; return "$status"; }; fi
  done <"$inventory"
  rm -f "$inventory" "$seen"
}
