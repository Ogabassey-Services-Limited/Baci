...
scan_compose_definitions() { list=$(temp_path); for root in $COMPOSE_ROOTS; do [ ! -e "$root" ] && continue; [ -d "$root" ] && [ ! -L "$root" ] || { rm -f "$list"; return 2; }; find "$root" -maxdepth 5 -type f \( -name 'docker-compose*.yml' -o -name 'docker-compose*.yaml' -o -name 'compose*.yml' -o -name 'compose*.yaml' -o -name 'Containerfile' \) >>"$list" || { rm -f "$list"; return 2; }; done; while IFS= read -r path || [ -n "$path" ]; do if record=$(consumer_matched_fingerprint "$path"); then printf '%s\n' "$record"; else status=$?; [ "$status" -eq 1 ] || { rm -f "$list"; return "$status"; }; fi; case "$path" in *Containerfile) ;; *) scan_compose_environment_files "$path" || { status=$?; rm -f "$list"; return "$status"; };; esac; done <"$list"; rm -f "$list"; }
...
systemd_linked_definitions() {
  ...
  while IFS= read -r candidate || [ -n "$candidate" ]; do
    ...
    target=$(readlink -- "$candidate") || { rm -f "$inventory" "$seen"; return 2; }; case "$target" in /*) :;; *'..'*) rm -f "$inventory" "$seen"; return 2;; *) continue;; esac; definition=$(consumer_canonical_regular "$target") || { rm -f "$inventory" "$seen"; return 2; }
