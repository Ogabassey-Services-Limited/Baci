...
compose_cron_cli_tokens() {
  unwrapped=$(temp_path); awk 'NR==1{if($0!="/usr/bin/flock"){plain=1;print;next};flock=1;next}plain{print;next}NR==2{if($0!="-n")bad=1;next}NR==3{if($0!~/^\/run\/[A-Za-z0-9._\/-]+$/||$0~/(^|\/)\.\.?($|\/)/)bad=1;next}NR==4{if($0!~/^\/[A-Za-z0-9._\/-]+$/||$0~/(^|\/)\.\.?($|\/)/)bad=1;else print;next}{print}END{if(flock&&NR<4)bad=1;exit bad?2:0}' "$1" >"$unwrapped" || { rm -f "$unwrapped"; return 2; }
  awk 'NR==1&&$0=="/usr/bin/env"{env=1;next}!env{print;next}!command{utility=$0;sub(/^.*\//,"",utility);if(($0!="docker"&&$0!="docker-compose"&&$0!~/^\/[A-Za-z0-9._\/-]+$/)||$0~/(^|\/)\.\.?($|\/)/||(utility!="docker"&&utility!="docker-compose")){bad=1;next};command=1;print;next}{print}END{if(env&&!command)bad=1;exit bad?2:0}' "$unwrapped"; status=$?; rm -f "$unwrapped"; return "$status"
}
...
container_wrapper_exec_paths() {
  normalized=$(temp_path)
  awk '{line=$0;if(continuing)line=joined line;slashes=0;for(i=length(line);i>0&&substr(line,i,1)=="\\";i--)slashes++;if(slashes%2){sub(/\\$/,"",line);joined=line;continuing=1;next}joined="";continuing=0;print line}END{if(continuing)exit 2}' "$1" >"$normalized" || { rm -f "$normalized"; return 2; }
  if grep -q -Ei 'ollama|11434' "$normalized"; then wrapper_bound=1; else status=$?; [ "$status" -eq 1 ] || { rm -f "$normalized"; return "$status"; }; wrapper_bound=0; fi
  awk -v wrapper_bound="$wrapper_bound" 'function trim(value){sub(/^[[:space:]]+/,"",value);sub(/[[:space:]]+$/,"",value);return value}function safe(path){return path~/^\/[A-Za-z0-9._\/-]+$/&&path!~/(^|\/)\.\.?($|\/)/}function builtin(line){return line~/^(exit([[:space:]]+[0-9]+)?|true|false|:|set[[:space:]]+-[A-Za-z]+)$/}{line=trim($0);if(line==""||line~/^#/||line~/^(\.|source)[[:space:]]+/)next;sub(/[[:space:]]+#.*$/,"",line);count=split(line,parts,/[[:space:]]+/);candidate=(parts[1]~/^[A-Za-z_][A-Za-z0-9_]*=/||parts[1]=="exec"||parts[1]~/^\//);if(!candidate){if(builtin(line))next;if(!wrapper_bound)bad=1;if(line~/^[\047"`$]/||line~/(^|;)[[:space:]]*exec[[:space:]]/)bad=1;next}if(line~/[\\\047"`$|&;<>(){}]/){bad=1;next};at=1;while(at<=count&&parts[at]~/^[A-Za-z_][A-Za-z0-9_]*=/){if(parts[at]!~/^[A-Za-z_][A-Za-z0-9_]*=[A-Za-z0-9._:+,\/-]*$/){bad=1;next};value=parts[at];sub(/^[^=]*=/,"",value);if(value~/^\//){if(!safe(value))bad=1;else print value}else if(value~/\//)bad=1;at++}if(at>1){if(parts[at]!="exec"){bad=1;next};at++;explicit=1}else{explicit=(parts[at]=="exec");if(explicit)at++}if(at>count||!safe(parts[at])){bad=1;next};for(i=at;i<=count;i++)if(parts[i]~/^\//){if(!safe(parts[i]))bad=1;else print parts[i]}else if(parts[i]~/^--?[A-Za-z0-9][A-Za-z0-9_.-]*=/){path=parts[i];sub(/^[^=]*=/,"",path);if(path~/^\//){if(!safe(path))bad=1;else print path}else if(path~/\//)bad=1}else if(parts[i]~/^https?:\/\/[A-Za-z0-9._:-]+(\/[A-Za-z0-9._~%+,:@-]*)?$/){}else if(parts[i]~/\//)bad=1}END{exit bad?2:0}' "$normalized"
  status=$?; rm -f "$normalized"; return "$status"
}
...
container_argument_consumers_run() {
  argument_id=$1; argument_configuration=$2; argument_rest=${argument_configuration#* }; argument_rest=${argument_rest#* }; argument_path=${argument_rest%% *}; [ "$argument_path" != "$argument_rest" ] || return 2; argument_rest=${argument_rest#* }; argument_json=$(temp_path); argument_again=$(temp_path); argument_roots=$(temp_path); argument_emitted=$(temp_path); case "$argument_path" in /*) :;; ./*) working_first=$(temp_path); working_second=$(temp_path); docker --host "unix://$CANONICAL_DOCKER_SOCKET" inspect -f '{{json .Config.WorkingDir}}' "$argument_id" >"$working_first" && docker --host "unix://$CANONICAL_DOCKER_SOCKET" inspect -f '{{json .Config.WorkingDir}}' "$argument_id" >"$working_second" && cmp -s "$working_first" "$working_second" || { rm -f "$argument_json" "$argument_again" "$argument_roots" "$argument_emitted" "$working_first" "$working_second"; return 2; }; working=$(/usr/bin/jq -er 'if type!="string" then error("invalid working directory") elif .=="" then "/" elif test("^/[A-Za-z0-9._/-]+$") and (test("(^|/)\\.\\.?(/|$)")|not) then . else error("invalid working directory") end' "$working_first") || { rm -f "$argument_json" "$argument_again" "$argument_roots" "$argument_emitted" "$working_first" "$working_second"; return 2; }; rm -f "$working_first" "$working_second"; [ "$working" = / ] && argument_path=/${argument_path#./} || argument_path=$working/${argument_path#./};; *) rm -f "$argument_json" "$argument_again" "$argument_roots" "$argument_emitted"; return 2;; esac; printf '%s\n' "$argument_path" | grep -Eq '^/[A-Za-z0-9._/-]+$' && ! printf '%s\n' "$argument_path" | grep -Eq '(^|/)\.\.?($|/)' || { rm -f "$argument_json" "$argument_again" "$argument_roots" "$argument_emitted"; return 2; }; printf '%s\n' "$argument_path" >>"$argument_roots"
