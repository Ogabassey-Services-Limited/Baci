...
recovery_scan() { root; init_temp_root; trap 'cleanup_temp' EXIT HUP INT TERM; assert_docker_socket; RECOVERY_SELF_PID=$$; RECOVERY_CONTAINER_PORTS_FILE=''
  ...
  recovery_terminal_process_snapshot "$processes" "$process_json"; recovery_terminal_container_snapshot; recovery_terminal_mutable_consumers "$mutable"; recovery_terminal_container_consumers "$containers"; recovery_write_receipt "$snapshot"; rm -f "$snapshot" "$cron" "$processes" "$container" "$mutable" "$containers" "${RECOVERY_EXTERNAL_CRON_SOURCES:-}"; [ -z "${RECOVERY_CONTAINER_PORTS_FILE:-}" ] || rm -f "$RECOVERY_CONTAINER_PORTS_FILE"; }
