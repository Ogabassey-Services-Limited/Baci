...
recovery_process_executable() {
  ...
  /usr/bin/jq -cn --arg path "$observed" --arg real "$real" --arg digest "$digest" --arg identity "$(hash_text "$stat_value")" --arg uid "$uid_value" --arg start "$start_value" --arg expected "$expected" --arg kind "$kind" '{path:$path,realPath:$real,sha256:$digest,identitySha256:$identity,uid:$uid,startTime:$start,expected:$expected,kind:$kind}'
}
