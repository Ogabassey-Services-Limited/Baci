...
recovery_scan() {
  ...
  recovery_terminal_process_snapshot "$processes" "$process_json"; recovery_terminal_container_snapshot; recovery_write_receipt "$snapshot"; rm -f "$snapshot" "$cron" "$processes" "$container" "${RECOVERY_EXTERNAL_CRON_SOURCES:-}"; [ -z "${RECOVERY_CONTAINER_PORTS_FILE:-}" ] || rm -f "$RECOVERY_CONTAINER_PORTS_FILE"
}
