...
RECOVERY_CONSUMERS_HELPER="$SCRIPT_DIR/retire-ollama-consumers.sh"; RECOVERY_CONSUMER_MOUNTS_HELPER="$SCRIPT_DIR/retire-ollama-container-mounts.sh"; RECOVERY_RUNNING_CONTAINER_HELPER="$SCRIPT_DIR/retire-ollama-running-container.sh"; RECOVERY_RUNNING_ARCHIVE_HELPER="$SCRIPT_DIR/retire-ollama-running-archive.sh"; RECOVERY_CONSUMER_CLOSURE_HELPER="$SCRIPT_DIR/retire-ollama-consumer-closure.sh"; RECOVERY_IMAGE_FILESYSTEM_HELPER="$SCRIPT_DIR/retire-ollama-image-filesystem.pl"; RECOVERY_PROJECTOR_AUTH_HELPER="$SCRIPT_DIR/retire-ollama-projector-auth.sh"
[ -f "$RECOVERY_CONSUMERS_HELPER" ] && [ ! -L "$RECOVERY_CONSUMERS_HELPER" ] && [ -f "$RECOVERY_CONSUMER_MOUNTS_HELPER" ] && [ ! -L "$RECOVERY_CONSUMER_MOUNTS_HELPER" ] && [ -f "$RECOVERY_RUNNING_CONTAINER_HELPER" ] && [ ! -L "$RECOVERY_RUNNING_CONTAINER_HELPER" ] && [ -f "$RECOVERY_RUNNING_ARCHIVE_HELPER" ] && [ ! -L "$RECOVERY_RUNNING_ARCHIVE_HELPER" ] && [ -f "$RECOVERY_CONSUMER_CLOSURE_HELPER" ] && [ ! -L "$RECOVERY_CONSUMER_CLOSURE_HELPER" ] && [ -f "$RECOVERY_IMAGE_FILESYSTEM_HELPER" ] && [ ! -L "$RECOVERY_IMAGE_FILESYSTEM_HELPER" ] && [ -f "$RECOVERY_PROJECTOR_AUTH_HELPER" ] && [ ! -L "$RECOVERY_PROJECTOR_AUTH_HELPER" ] || review_required 'recovery consumer scanner helper missing'
