...
recovery_validate_json() {
  ...
  /usr/bin/jq -e -s --arg source "$RECOVERY_SOURCE_SHA" --arg script "$RECOVERY_SCRIPT_SHA" --arg helper "$RECOVERY_HELPER_SHA" --arg receipts "$RECOVERY_RECEIPTS_SHA" --arg consumers "$RECOVERY_CONSUMERS_SHA" --arg consumer_mounts "$RECOVERY_CONSUMER_MOUNTS_SHA" --arg image_filesystem "$RECOVERY_IMAGE_FILESYSTEM_SHA" --arg temp_root "$RECOVERY_TEMP_ROOT_SHA" --arg running_container "$RECOVERY_RUNNING_CONTAINER_SHA" --arg running_archive "$RECOVERY_RUNNING_ARCHIVE_SHA" --arg consumer_closure "$RECOVERY_CONSUMER_CLOSURE_SHA" --arg process_files "$RECOVERY_PROCESS_FILES_SHA" --arg cron_inventory "$RECOVERY_CRON_INVENTORY_SHA" --arg at_quiescence "$RECOVERY_AT_QUIESCENCE_SHA" --arg projector_auth "$RECOVERY_PROJECTOR_AUTH_SHA" '
    length == 1 and (.[0] |
      ...
      (.sourceBinding | type == "object" and (keys_unsorted | sort) == ["atQuiescenceSha256","consumerClosureSha256","consumerMountsSha256","consumersSha256","cronInventorySha256","helperSha256","imageFilesystemSha256","processFilesSha256","projectorAuthSha256","receiptsSha256","runningArchiveSha256","runningContainerSha256","scriptSha256","sourceSha","tempRootSha256"] and (.sourceSha | type == "string" and test("^[0-9a-f]{40}$") and . == $source) and
        .scriptSha256 == $script and .helperSha256 == $helper and .receiptsSha256 == $receipts and .consumersSha256 == $consumers and .consumerMountsSha256 == $consumer_mounts and .imageFilesystemSha256 == $image_filesystem and .tempRootSha256 == $temp_root and .runningContainerSha256 == $running_container and .runningArchiveSha256 == $running_archive and .consumerClosureSha256 == $consumer_closure and .processFilesSha256 == $process_files and .cronInventorySha256 == $cron_inventory and .atQuiescenceSha256 == $at_quiescence and .projectorAuthSha256 == $projector_auth and
        ([.scriptSha256,.helperSha256,.receiptsSha256,.consumersSha256,.consumerMountsSha256,.imageFilesystemSha256,.tempRootSha256,.runningContainerSha256,.runningArchiveSha256,.consumerClosureSha256,.processFilesSha256,.cronInventorySha256,.atQuiescenceSha256,.projectorAuthSha256] | all(.[]; type == "string" and test("^[0-9a-f]{64}$")))) and
      (.scan | type == "object")
    )
  ' "$candidate" >/dev/null
}
...
recovery_write_receipt() {
  ...
  /usr/bin/jq -S -c -n --slurpfile scan "$snapshot" --arg source "$RECOVERY_SOURCE_SHA" --arg script "$RECOVERY_SCRIPT_SHA" --arg helper "$RECOVERY_HELPER_SHA" --arg receipts "$RECOVERY_RECEIPTS_SHA" --arg consumers "$RECOVERY_CONSUMERS_SHA" --arg consumer_mounts "$RECOVERY_CONSUMER_MOUNTS_SHA" --arg image_filesystem "$RECOVERY_IMAGE_FILESYSTEM_SHA" --arg temp_root "$RECOVERY_TEMP_ROOT_SHA" --arg running_container "$RECOVERY_RUNNING_CONTAINER_SHA" --arg running_archive "$RECOVERY_RUNNING_ARCHIVE_SHA" --arg consumer_closure "$RECOVERY_CONSUMER_CLOSURE_SHA" --arg process_files "$RECOVERY_PROCESS_FILES_SHA" --arg cron_inventory "$RECOVERY_CRON_INVENTORY_SHA" --arg at_quiescence "$RECOVERY_AT_QUIESCENCE_SHA" --arg projector_auth "$RECOVERY_PROJECTOR_AUTH_SHA" '{schemaVersion:3,mode:"recovery-scan",destructiveAuthority:false,inventoryBinding:{requiresSeparateReview:true},sourceBinding:{sourceSha:$source,scriptSha256:$script,helperSha256:$helper,receiptsSha256:$receipts,consumersSha256:$consumers,consumerMountsSha256:$consumer_mounts,imageFilesystemSha256:$image_filesystem,tempRootSha256:$temp_root,runningContainerSha256:$running_container,runningArchiveSha256:$running_archive,consumerClosureSha256:$consumer_closure,processFilesSha256:$process_files,cronInventorySha256:$cron_inventory,atQuiescenceSha256:$at_quiescence,projectorAuthSha256:$projector_auth},scan:$scan[0]}' >"$pending" || die 'recovery receipt serialization failed'
