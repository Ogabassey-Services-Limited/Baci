...
recovery_source_digests() {
  ...
  actual_running_container=$(sha "$RECOVERY_RUNNING_CONTAINER_HELPER") || return 1
  actual_running_archive=$(sha "$RECOVERY_RUNNING_ARCHIVE_HELPER") || return 1
...
recovery_validate_json() {
  ...
  /usr/bin/jq -e --arg source "$RECOVERY_SOURCE_SHA" --arg script "$RECOVERY_SCRIPT_SHA" --arg helper "$RECOVERY_HELPER_SHA" --arg receipts "$RECOVERY_RECEIPTS_SHA" --arg consumers "$RECOVERY_CONSUMERS_SHA" --arg running_container "$RECOVERY_RUNNING_CONTAINER_SHA" --arg running_archive "$RECOVERY_RUNNING_ARCHIVE_SHA" --arg consumer_closure "$RECOVERY_CONSUMER_CLOSURE_SHA" --arg process_files "$RECOVERY_PROCESS_FILES_SHA" --arg cron_inventory "$RECOVERY_CRON_INVENTORY_SHA" --arg at_quiescence "$RECOVERY_AT_QUIESCENCE_SHA" '
    ...
    (.sourceBinding | type == "object" and .sourceSha == $source and
      .scriptSha256 == $script and .helperSha256 == $helper and .receiptsSha256 == $receipts and .consumersSha256 == $consumers and .runningContainerSha256 == $running_container and .runningArchiveSha256 == $running_archive and .consumerClosureSha256 == $consumer_closure and .processFilesSha256 == $process_files and .cronInventorySha256 == $cron_inventory and .atQuiescenceSha256 == $at_quiescence) and
    (.scan | type == "object")
  ' "$candidate" >/dev/null
}
