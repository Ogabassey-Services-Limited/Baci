...
function shell(env = {}, options = {}) {
  return execFileAsync(
    ...
    [
      '-c',
      '. "$1"; SCRIPT_DIR=$(dirname "$1"); RECOVERY_HELPER="$SCRIPT_DIR/retire-ollama-recovery.sh"; . "$RECOVERY_HELPER"; id() { printf "%s\\n" "$RETIRE_OLLAMA_TEST_FAKE_UID"; }; sha() { /usr/bin/shasum -a 256 "$1" | /usr/bin/awk "{print \\$1}"; }; init_temp_root; trap cleanup_temp EXIT; recovery_source_digests; printf "%s:%s:%s:%s:%s:%s\\n" "$RECOVERY_SCRIPT_SHA" "$RECOVERY_HELPER_SHA" "$RECOVERY_RECEIPTS_SHA" "$RECOVERY_CONSUMERS_SHA" "$RECOVERY_CONSUMER_CLOSURE_SHA" "$RECOVERY_CRON_INVENTORY_SHA"',
