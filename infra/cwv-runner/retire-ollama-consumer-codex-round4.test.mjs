...
test('scans a stopped user unit when the owner user manager is unavailable', async () => {
  ...
  try {
    ...
    const { stdout } = await execFileAsync('sh', [
      '-c',
      `${prelude}home=$3; marker=$4; getent() { printf 'bassey:x:1001:1001::%s:/bin/sh\\n' "$home"; }; systemctl() { case "$1" in --user) : >"$marker"; return 1;; list-unit-files|list-units) return 0;; esac; }; . "$1"; SCRIPT_DIR=$(dirname "$1"); SYSTEMD_ROOTS="$2"; init_temp_root; trap cleanup_temp EXIT; scan_systemd_consumers; test ! -e "$marker" && printf stable`,
