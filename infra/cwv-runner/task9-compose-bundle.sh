...
[ ! -e "$helper_copy" ] && [ ! -L "$helper_copy" ] || refuse; /bin/cp -p -- "$helper" "$helper_copy"; /bin/chmod 0500 "$helper_copy"; regular "$helper_copy"; [ "$(sha256 "$helper_copy")" = "$reviewed_helper" ] || refuse
