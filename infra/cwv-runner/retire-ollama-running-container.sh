#!/bin/sh
# Running-container validation is metadata-only.  Stopped containers retain
# the docker-cp closure in retire-ollama-consumer-closure.sh.
...
running_container_image_save_bounded() {
  running_save_id=$1
  running_save_output=$2
  running_save_fifo=$3
  running_save_status=$4
  running_save_deadline=$5
  ...
  docker --host "unix://$CANONICAL_DOCKER_SOCKET" image save "$running_save_id" >"$running_save_fifo" 2>/dev/null &
  running_save_pid=$!
  /usr/bin/perl -e '
...
running_container_image_hash_stream() {
  running_hash_id=$1
  running_hash_output=$2
  running_hash_fifo=$3
  running_hash_status_file=$4
  running_hash_deadline=$5
  ...
  docker --host "unix://$CANONICAL_DOCKER_SOCKET" image save "$running_hash_id" >"$running_hash_fifo" 2>/dev/null &
  running_hash_pid=$!
...
running_container_validate_json() {
  ...
  case "$running_json_kind" in
    ...
    args)
      ...
      ;;
    env)
      /usr/bin/jq -e 'type == "array" and length <= 256 and all(.[]; type == "string" and (test("[\u0000-\u001f]") | not) and test("^[A-Za-z_][A-Za-z0-9_]*="))' "$running_json_file" >/dev/null || return 2
      ;;
    health)
...
running_container_validate() {
  ...
  if [ -e "$running_image_cache" ] || [ -L "$running_image_cache" ]; then
    ...
    running_container_image_save_bounded "$running_image_id" "$running_image_save_first" "$running_image_save_fifo" "$running_image_save_status" "$running_image_deadline" || { running_cleanup; return 2; }
    running_image_sha=$(sha "$running_image_save_first") || { running_cleanup; return 2; }
    running_image_second_sha=$(running_container_image_hash_stream "$running_image_id" "$running_image_hash" "$running_image_hash_fifo" "$running_image_hash_status" "$running_image_deadline") || { running_cleanup; return 2; }
