...
darwinTest('verifies archive, checksum row, binary, and version before rebound exec', async () => {
  ...
  assert.match(variableLog, /--method GET .*\/rulesets\/18/); assert.match(variableLog, /variable set H0_RUNNER_RULESET_ID --repo ogabasseyy\/Baci --body -\nstdin:18/); assert.match(variableLog, new RegExp(`variable set H0_RUNNER_RULESET_SHA256 --repo ogabasseyy/Baci --body -\\nstdin:${requestDigest}`));
