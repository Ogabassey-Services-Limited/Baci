...
async function fixture(additionalChecksumRows = '') {
  ...
  const manifestBytes = Buffer.from(canonical({
      ...
      sourceArchive: {
        entries: [{ blobSha256: digest('unrelated-before'), mode: '100644', path: 'infra/cwv-runner/Dockerfile' }, { blobSha256: digest(await readFile(dispatcher)), mode: '100755', path: 'infra/cwv-runner/owner-dispatch.sh' }, { blobSha256: digest(policyBytes), mode: '100644', path: 'infra/cwv-runner/policy.json' }, { blobSha256: digest(await readFile(verifier)), mode: '100755', path: 'infra/cwv-runner/verify-owner-cli.sh' }, { blobSha256: digest('unrelated-after'), mode: '100644', path: 'infra/cwv-runner/verify-owner-cli.test.mjs' }],
      },
    }));
