...
test('fails closed on a bare wrapper command with a file argument', async () => {
  const directory = await realpath(
    await mkdtemp(join(tmpdir(), 'baci-systemd-wrapper-bare-command-'))
  );
  const units = join(directory, 'units');
  const root = join(directory, 'root');
  try {
    ...
    await Promise.all([
      ...
      writeFile(
        join(root, 'usr/bin/wrapper'),
        '#!/bin/sh\nworker --config=/etc/application.conf\n',
        { mode: 0o755 }
      ),
      writeFile(join(root, 'usr/bin/worker'), '#!/bin/sh\nexit 0\n', {
        mode: 0o755,
      }),
      writeFile(
        join(root, 'etc/application.conf'),
    ...
    await assert.rejects(
      execFileAsync('sh', [
        '-c',
        `${prelude}getent() { return 2; }; systemctl() { return 0; }; . "$1"; SCRIPT_DIR=$(dirname "$1"); SYSTEMD_ROOTS="$2"; init_temp_root; trap cleanup_temp EXIT; scan_systemd_consumers`,
        'retire-ollama-systemd-wrapper-bare-command-test',
        script.pathname,
        units,
      ]),
      (error) => error.code === 2
