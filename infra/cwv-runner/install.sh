...
render_watchdog() {
  ...
  rendered_sha=$(/usr/bin/sed "s/@BACI_CWV_SOURCE_SHA@/$source_sha/g" "$source" | /usr/bin/sha256sum | /usr/bin/awk '{print $1}'); destination_sha=$(/usr/bin/printf '%s' "$target" | /usr/bin/sha256sum | /usr/bin/awk '{print $1}'); if ! is_sha "$rendered_sha" || ! is_sha "$destination_sha"; then die 'watchdog render authority refused'; fi; bound_watchdog=; residue_patterns='/etc/systemd/system/.baci-cwv-watchdog.??????'; if [ "${BACI_CWV_BOOTSTRAP_REPLACEMENT-}" = 1 ] && regular "$BOOTSTRAP_DIRECTORY/replacement-intent.json" && /usr/bin/jq -e --arg path "$target" '.transitionPaths | index($path) != null' "$BOOTSTRAP_DIRECTORY/replacement-intent.json" >/dev/null; then bound_watchdog=1; residue_patterns="$residue_patterns /etc/systemd/system/.baci-cwv-watchdog-v1-${destination_sha}-${rendered_sha}-??????"; fi
  residue_removed=; for residue in $residue_patterns; do [ -e "$residue" ] || [ -L "$residue" ] || continue; if ! regular "$residue" || [ "$(/usr/bin/stat -c %h -- "$residue")" != 1 ] || { ! root_mode "$residue" root:root:0600 && ! root_mode "$residue" root:root:0644; }; then die 'watchdog render temporary drift'; fi; residue_size=$(/usr/bin/stat -c %s -- "$residue") || die 'watchdog render temporary drift'; case "$residue_size" in *[!0-9]*|'') die 'watchdog render temporary drift';; esac; if [ "$residue_size" -le 1048576 ] && /usr/bin/sed "s/@BACI_CWV_SOURCE_SHA@/$source_sha/g" "$source" | /usr/bin/head -c "$residue_size" | /usr/bin/cmp -s "$residue" -; then :; else die 'watchdog render temporary drift'; fi; /bin/rm -- "$residue" || die 'watchdog render temporary cleanup failed'; residue_removed=1; done; if [ "$residue_removed" = 1 ]; then /usr/bin/sync -f /etc/systemd/system || die 'watchdog render temporary fsync failed'; fi; if [ "$bound_watchdog" = 1 ]; then /usr/bin/node "$SCRIPT_DIR/install-bootstrap-watchdog-residue.mjs" "$BOOTSTRAP_DIRECTORY" "$target" || die 'watchdog render temporary drift'; temporary=$(/usr/bin/mktemp "/etc/systemd/system/.baci-cwv-watchdog-v1-${destination_sha}-${rendered_sha}-XXXXXX") || die 'temporary render failed'; else temporary=$(/usr/bin/mktemp /etc/systemd/system/.baci-cwv-watchdog.XXXXXX) || die 'temporary render failed'; fi
