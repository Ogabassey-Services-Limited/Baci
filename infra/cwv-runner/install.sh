...
render_watchdog() {
  ...
  /bin/chown root:root "$temporary"; /bin/chmod 0644 "$temporary"; reconciled=; if [ "${BACI_CWV_BOOTSTRAP_REPLACEMENT-}" = 1 ] && regular "$BOOTSTRAP_DIRECTORY/replacement-intent.json" && /usr/bin/jq -e --arg path "$target" '.transitionPaths | index($path) != null' "$BOOTSTRAP_DIRECTORY/replacement-intent.json" >/dev/null; then /usr/bin/node "$SCRIPT_DIR/install-bootstrap-replacement-file.mjs" source "$BOOTSTRAP_DIRECTORY" "$target" "$temporary" >/dev/null || { /bin/rm -f -- "$temporary"; die 'watchdog unit drift'; }; /bin/rm -f -- "$temporary"; reconciled=1; fi
