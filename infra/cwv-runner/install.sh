...
render_watchdog() {
  ...
  /bin/chown root:root "$temporary"; /bin/chmod 0644 "$temporary"; residue_removed=; for residue in /etc/systemd/system/.baci-cwv-watchdog.??????; do [ "$residue" != "$temporary" ] || continue; [ -e "$residue" ] || [ -L "$residue" ] || continue; if ! regular "$residue" || ! root_mode "$residue" root:root:0644 || [ "$(/usr/bin/stat -c %h -- "$residue")" != 1 ] || ! /usr/bin/cmp -s "$residue" "$temporary"; then /bin/rm -f -- "$temporary"; die 'watchdog render temporary drift'; fi; /bin/rm -- "$residue" || { /bin/rm -f -- "$temporary"; die 'watchdog render temporary cleanup failed'; }; residue_removed=1; done; if [ "$residue_removed" = 1 ]; then /usr/bin/sync -f /etc/systemd/system || { /bin/rm -f -- "$temporary"; die 'watchdog render temporary fsync failed'; }; fi; reconciled=; if [ "${BACI_CWV_BOOTSTRAP_REPLACEMENT-}" = 1 ] && regular "$BOOTSTRAP_DIRECTORY/replacement-intent.json" && /usr/bin/jq -e --arg path "$target" '.transitionPaths | index($path) != null' "$BOOTSTRAP_DIRECTORY/replacement-intent.json" >/dev/null; then /usr/bin/node "$SCRIPT_DIR/install-bootstrap-replacement-file.mjs" source "$BOOTSTRAP_DIRECTORY" "$target" "$temporary" >/dev/null || { /bin/rm -f -- "$temporary"; die 'watchdog unit drift'; }; /bin/rm -f -- "$temporary"; reconciled=1; fi
