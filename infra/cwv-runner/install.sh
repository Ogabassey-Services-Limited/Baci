...
render_watchdog() {
  ...
  bound_watchdog=; if [ "${BACI_CWV_BOOTSTRAP_REPLACEMENT-}" = 1 ] && regular "$BOOTSTRAP_DIRECTORY/replacement-intent.json" && /usr/bin/jq -e --arg path "$target" '.transitionPaths | index($path) != null' "$BOOTSTRAP_DIRECTORY/replacement-intent.json" >/dev/null; then /usr/bin/node "$SCRIPT_DIR/install-bootstrap-watchdog-residue.mjs" "$BOOTSTRAP_DIRECTORY" "$target" || die 'watchdog render temporary drift'; rendered_sha=$(/usr/bin/sed "s/@BACI_CWV_SOURCE_SHA@/$source_sha/g" "$source" | /usr/bin/sha256sum | /usr/bin/awk '{print $1}'); destination_sha=$(/usr/bin/printf '%s' "$target" | /usr/bin/sha256sum | /usr/bin/awk '{print $1}'); if ! is_sha "$rendered_sha" || ! is_sha "$destination_sha"; then die 'watchdog render authority refused'; fi; temporary=$(/usr/bin/mktemp "/etc/systemd/system/.baci-cwv-watchdog-v1-${destination_sha}-${rendered_sha}-XXXXXX") || die 'temporary render failed'; bound_watchdog=1; else temporary=$(/usr/bin/mktemp /etc/systemd/system/.baci-cwv-watchdog.XXXXXX) || die 'temporary render failed'; fi
  /bin/chown root:root "$temporary"; /bin/chmod 0644 "$temporary"
  /usr/bin/sed "s/@BACI_CWV_SOURCE_SHA@/$source_sha/g" "$source" >"$temporary"
  /usr/bin/grep -Fq '@BACI_CWV_SOURCE_SHA@' "$temporary" && die 'unrendered watchdog token'; if [ "$bound_watchdog" = 1 ] && [ "$(sha256 "$temporary")" != "$rendered_sha" ]; then /bin/rm -f -- "$temporary"; die 'watchdog render authority refused'; fi
  residue_removed=; for residue in /etc/systemd/system/.baci-cwv-watchdog.??????; do [ "$residue" != "$temporary" ] || continue; [ -e "$residue" ] || [ -L "$residue" ] || continue; if ! regular "$residue" || [ "$(/usr/bin/stat -c %h -- "$residue")" != 1 ]; then /bin/rm -f -- "$temporary"; die 'watchdog render temporary drift'; fi; if root_mode "$residue" root:root:0644; then /usr/bin/cmp -s "$residue" "$temporary" || { /bin/rm -f -- "$temporary"; die 'watchdog render temporary drift'; }; elif root_mode "$residue" root:root:0600; then residue_size=$(/usr/bin/stat -c %s -- "$residue") || { /bin/rm -f -- "$temporary"; die 'watchdog render temporary drift'; }; case "$residue_size" in *[!0-9]*|'') /bin/rm -f -- "$temporary"; die 'watchdog render temporary drift';; esac; if [ "$residue_size" -le 1048576 ] && /usr/bin/head -c "$residue_size" -- "$temporary" | /usr/bin/cmp -s "$residue" -; then :; else /bin/rm -f -- "$temporary"; die 'watchdog render temporary drift'; fi; /bin/chmod 0644 "$residue" || { /bin/rm -f -- "$temporary"; die 'watchdog render temporary cleanup failed'; }; else /bin/rm -f -- "$temporary"; die 'watchdog render temporary drift'; fi; /bin/rm -- "$residue" || { /bin/rm -f -- "$temporary"; die 'watchdog render temporary cleanup failed'; }; residue_removed=1; done; if [ "$residue_removed" = 1 ]; then /usr/bin/sync -f /etc/systemd/system || { /bin/rm -f -- "$temporary"; die 'watchdog render temporary fsync failed'; }; fi; reconciled=; if [ "${BACI_CWV_BOOTSTRAP_REPLACEMENT-}" = 1 ] && regular "$BOOTSTRAP_DIRECTORY/replacement-intent.json" && /usr/bin/jq -e --arg path "$target" '.transitionPaths | index($path) != null' "$BOOTSTRAP_DIRECTORY/replacement-intent.json" >/dev/null; then /usr/bin/node "$SCRIPT_DIR/install-bootstrap-replacement-file.mjs" source "$BOOTSTRAP_DIRECTORY" "$target" "$temporary" >/dev/null || { /bin/rm -f -- "$temporary"; die 'watchdog unit drift'; }; /bin/rm -f -- "$temporary"; reconciled=1; fi
