...
render_watchdog() {
  ...
  temporary=$(/usr/bin/mktemp /etc/systemd/system/.baci-cwv-watchdog.XXXXXX) || die 'temporary render failed'
  /usr/bin/sed "s/@BACI_CWV_SOURCE_SHA@/$source_sha/g" "$source" >"$temporary"
  /usr/bin/grep -Fq '@BACI_CWV_SOURCE_SHA@' "$temporary" && die 'unrendered watchdog token'
  ...
  if [ -e "$target" ] || [ -L "$target" ]; then
    ...
    if /usr/bin/cmp -s "$temporary" "$target"; then /bin/rm -f -- "$temporary"; else previous_sha=$(/usr/bin/sed -n 's#^ExecStart=/srv/baci-cwv/source/\([a-f0-9]\{40\}\)/campaign-watchdog\.sh %i$#\1#p' "$target"); git_sha "$previous_sha" || die 'watchdog unit drift'; previous_source="$SOURCE_ROOT/$previous_sha/baci-cwv-campaign-watchdog@.service"; if ! regular "$previous_source" || ! /usr/bin/cmp -s "$source" "$previous_source"; then die 'watchdog unit drift'; fi; previous=$(/usr/bin/mktemp /etc/systemd/system/.baci-cwv-watchdog-prior.XXXXXX) || die 'prior render failed'; /usr/bin/sed "s/@BACI_CWV_SOURCE_SHA@/$previous_sha/g" "$previous_source" >"$previous"; if ! /usr/bin/cmp -s "$previous" "$target"; then /bin/rm -f -- "$previous" "$temporary"; die 'watchdog unit drift'; fi; /bin/rm -f -- "$previous"; /usr/bin/sync -f "$temporary" || die 'watchdog unit fsync refused'; /bin/mv -T -- "$temporary" "$target" || die 'watchdog unit replacement refused'; fi
