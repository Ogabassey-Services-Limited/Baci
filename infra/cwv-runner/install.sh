...
render_watchdog() {
  ...
  if [ -e "$target" ] || [ -L "$target" ]; then
    if ! regular "$target" || ! root_mode "$target" root:root:0644 || ! /usr/bin/cmp -s "$temporary" "$target"; then die 'watchdog unit drift'; fi
    /bin/rm -f -- "$temporary"
...
install_units() {
  ...
  UNIT_STATES=$(/usr/bin/jq -cn '{}'); for name in baci-cwv-containerd.service baci-cwv-docker.service baci-cwv-measurement.service baci-cwv-host-sampler.service baci-cwv-host-sampler.timer 'baci-cwv-campaign-watchdog@.service'; do expected_unit_file_state=static; case "$name" in baci-cwv-campaign-watchdog@.service) expected_unit_file_state=disabled;; esac
    /bin/systemctl disable --now "$name"; state=$(/bin/systemctl show "$name" --property=LoadState --property=ActiveState --property=UnitFileState --value --no-pager); [ "$state" = "$(/usr/bin/printf 'loaded\ninactive\n%s' "$expected_unit_file_state")" ] || die 'unit did not become inactive with the expected file state'
    UNIT_STATES=$(printf '%s' "$UNIT_STATES" | /usr/bin/jq -c --arg name "$name" --arg state "$state" '. + {($name):($state + "\\n")}'); /usr/bin/node "$SCRIPT_DIR/install-bootstrap-controller.mjs" journal "$BOOTSTRAP_DIRECTORY" disable-unit "/etc/systemd/system/$name" "$(sha256 "/etc/systemd/system/$name")"
  done
}
