...
UNIT=ollama.service; TIMER=ollama-watchdog.timer; CONTAINER=ollama-loopback; AT_JOB_DIR=/var/spool/cron/atjobs
...
write_receipt() {
  snapshot=$1; ensure_receipt_dir; assert_no_pending_receipts; receipt_pending=$(pending_for "$RECEIPT"); jq -S -n --slurpfile snapshot "$snapshot" '{schemaVersion:2,scan:$snapshot[0],rollbackNeeds:["restore at submission spool mode 01770","reinstall Ollama package","redownload models"],prePostDeltas:{preDestructive:null,postDestructive:null}}' >"$receipt_pending" || { discard_pending "$receipt_pending"; die 'receipt serialization failed'; }
...
at_submission_state() { at_dir=$AT_JOB_DIR; at_sequence=$at_dir/.SEQ; [ -d "$at_dir" ] && [ ! -L "$at_dir" ] && [ "$(CDPATH='' cd -- "$at_dir" && pwd -P)" = "$at_dir" ] || die 'unsafe at submission spool'; [ -f "$at_sequence" ] && [ ! -L "$at_sequence" ] || die 'unsafe at sequence file'; at_state=$(stat -c '%d:%i:%u:%g:%a' "$at_dir") || die 'at submission spool identity failed'; at_sequence_state=$(stat -c '%d:%i:%u:%g:%a' "$at_sequence") || die 'at sequence identity failed'; old_ifs=$IFS; IFS=:; set -- $at_state; IFS=$old_ifs; [ "$#" -eq 5 ] && [ "$5" = 1770 ] && [ "${at_sequence_state#*:*:}" = "$3:$4:600" ] || die 'at submission spool contract drift'; jq -cn --arg path "$at_dir" --arg identity "$1:$2:$3:$4" --arg sequenceIdentity "${at_sequence_state%:600}" '{path:$path,identity:$identity,sequenceIdentity:$sequenceIdentity,originalMode:"1770",quiescedMode:"1550"}'; }
assert_at_submissions_quiesced() { at_expected=$1; at_path=$(printf '%s\n' "$at_expected" | jq -er '.path') || die 'at rollback state invalid'; at_identity=$(printf '%s\n' "$at_expected" | jq -er '.identity') || die 'at rollback state invalid'; at_sequence_identity=$(printf '%s\n' "$at_expected" | jq -er '.sequenceIdentity') || die 'at rollback state invalid'; [ "$at_path" = "$AT_JOB_DIR" ] && [ "$(stat -c '%d:%i:%u:%g:%a' "$at_path")" = "$at_identity:1550" ] && [ -f "$at_path/.SEQ" ] && [ ! -L "$at_path/.SEQ" ] && [ "$(stat -c '%d:%i:%u:%g:%a' "$at_path/.SEQ")" = "$at_sequence_identity:600" ] || die 'at submission quiescence drift'; cron_inventory_require_empty_at_queue || die 'queued work or an unsafe queue'; }
quiesce_at_submissions() { at_expected=$1; [ "$(at_submission_state)" = "$at_expected" ] || die 'at submission spool changed'; chmod 1550 "$AT_JOB_DIR" || die 'at submission quiescence failed'; fsync_dir "$AT_JOB_DIR"; fsync_dir "$(dirname "$AT_JOB_DIR")"; assert_at_submissions_quiesced "$at_expected"; }
reconcile_interrupted_at_quiescence() { at_pre="$RECEIPT_DIR/pre-destructive.json"; at_actions="$RECEIPT_DIR/pre-destructive.actions"; [ -e "$at_pre" ] || return 0; safe_file "$at_pre" || die 'unsafe pre-destructive receipt'; at_expected=$(jq -ce '.atSubmissionRollback | select(.path == $path and .originalMode == "1770" and .quiescedMode == "1550" and (.identity | test("^[0-9]+:[0-9]+:[0-9]+:[0-9]+$")) and (.sequenceIdentity | test("^[0-9]+:[0-9]+:[0-9]+:[0-9]+$")))' --arg path "$AT_JOB_DIR" "$at_pre") || die 'at rollback receipt invalid'; at_identity=$(printf '%s\n' "$at_expected" | jq -er '.identity') || die 'at rollback receipt invalid'; at_sequence_identity=$(printf '%s\n' "$at_expected" | jq -er '.sequenceIdentity') || die 'at rollback receipt invalid'; at_current=$(stat -c '%d:%i:%u:%g:%a' "$AT_JOB_DIR") || die 'at rollback identity failed'; [ -f "$AT_JOB_DIR/.SEQ" ] && [ ! -L "$AT_JOB_DIR/.SEQ" ] && [ "$(stat -c '%d:%i:%u:%g:%a' "$AT_JOB_DIR/.SEQ")" = "$at_sequence_identity:600" ] || die 'at rollback sequence drift'; if [ -e "$at_actions" ]; then safe_file "$at_actions" && [ "$(cat "$at_actions")" = quiesce_at_submissions ] || return 0; case "$at_current" in "$at_identity:1550") chmod 1770 "$AT_JOB_DIR" || die 'at rollback failed'; fsync_dir "$AT_JOB_DIR"; fsync_dir "$(dirname "$AT_JOB_DIR")";; "$at_identity:1770") :;; *) die 'at rollback state drift';; esac; rm -f "$at_actions" || die 'at rollback action cleanup failed'; fsync_dir "$RECEIPT_DIR"; elif [ "$at_current" != "$at_identity:1770" ]; then return 0; fi; rm -f "$at_pre" || die 'at rollback receipt cleanup failed'; fsync_dir "$RECEIPT_DIR"; }
...
apply() {
  ...
  assert_zero_consumers; jq -e '.scan.dependencies | type == "array" and length == 0' "$RECEIPT" >/dev/null || review_required 'retirement requires zero dependencies'; approved=$(approved_dependency_sha) || review_required 'independent dependency review required'; [ "$approved" = "$(dependency_sha)" ] || review_required 'independent dependency review required'; ensure_receipt_dir; reconcile_interrupted_at_quiescence
  [ ! -e "$RECEIPT_DIR/pre-destructive.json" ] && [ ! -e "$RECEIPT_DIR/pre-destructive.actions" ] && [ ! -e "$RECEIPT_DIR/completion.json" ] || die 'incomplete or completed retirement exists'; at_state=$(at_submission_state); pre_pending=$(pending_for "$RECEIPT_DIR/pre-destructive.json"); jq -S -n --argjson at "$at_state" '{phase:"pre-destructive",atSubmissionRollback:$at,rollbackNeeds:["restore at submission spool mode 01770","reinstall Ollama package","redownload models"]}' >"$pre_pending" || { discard_pending "$pre_pending"; die 'pre-destructive receipt failed'; }; publish_pending "$pre_pending" "$RECEIPT_DIR/pre-destructive.json"; pre=$(completion_metrics); record_action quiesce_at_submissions; quiesce_at_submissions "$at_state"
  revalidate_before install_crontab; record_action install_crontab; assert_at_submissions_quiesced "$at_state"; install_crontab
  revalidate_before disable_unit; record_action disable_unit; assert_at_submissions_quiesced "$at_state"; disable_unit
  revalidate_before remove_container; record_action remove_container; assert_at_submissions_quiesced "$at_state"; remove_container
  revalidate_before delete_models; record_action delete_models; assert_at_submissions_quiesced "$at_state"; delete_models
  post=$(completion_metrics)
  ensure_receipt_dir; completion_pending=$(pending_for "$RECEIPT_DIR/completion.json"); jq -S -n --arg receiptSha256 "$(canonical_receipt_digest)" --argjson at "$at_state" --argjson pre "$pre" --argjson post "$post" '{receiptSha256:$receiptSha256,atSubmissionRollback:$at,prePostDeltas:{preDestructive:$pre,postDestructive:$post,deltas:{cgroupMemoryBytes:($post.cgroupMemoryBytes-$pre.cgroupMemoryBytes),hostAvailableMemoryBytes:($post.hostAvailableMemoryBytes-$pre.hostAvailableMemoryBytes),modelStoreBytes:($post.modelStoreBytes-$pre.modelStoreBytes)}},rollbackNeeds:["restore at submission spool mode 01770","reinstall Ollama package","redownload models"]}' >"$completion_pending" || { discard_pending "$completion_pending"; die 'completion receipt failed'; }; publish_pending "$completion_pending" "$RECEIPT_DIR/completion.json"
}
