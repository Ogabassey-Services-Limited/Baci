...
retirement_lock() { if [ -n "${RETIRE_OLLAMA_TEST_BIN:-}" ]; then lock=${RETIRE_OLLAMA_LOCK:-"${TMPDIR:-/tmp}/baci-cwv-retire-ollama.lock"}; flock=${RETIRE_OLLAMA_FLOCK:-/usr/bin/true}; else lock=/run/lock/baci-cwv-retire-ollama.lock; flock=/usr/bin/flock; fi; exec 10>"$lock" || die 'retirement lock open failed'; "$flock" -n 10 || die 'another retirement operation owns the lock' 75; }; main() { case "${1:-}" in --scan) root; retirement_lock; scan;; --apply) root; retirement_lock; apply;; --recovery-scan) type recovery_scan >/dev/null 2>&1 || review_required 'recovery helper missing'; recovery_scan;; *) usage;; esac; }
