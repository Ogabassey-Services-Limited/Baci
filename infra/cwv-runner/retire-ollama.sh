...
scan_systemd_consumers() {
  ...
  while IFS=: read -r definition directive || [ -n "$definition$directive" ]; do case "$definition" in */"$UNIT"|*/"$UNIT".d/*|*/"$TIMER"|*/"$TIMER".d/*) continue;; esac; value=${directive#*=}; set -f; for item in $value; do optional=0; case "$item" in -*) optional=1; item=${item#-};; esac; item=${item#\"}; item=${item%\"}; case "$item" in /*) :;; *) set +f; return 2;; esac; if [ ! -e "$item" ]; then [ "$optional" -eq 1 ] && continue; set +f; return 2; fi; [ -f "$item" ] && [ ! -L "$item" ] || { set +f; return 2; }; [ "${RECOVERY_RECORDS+x}" = x ] && recovery_record_environment "$item" "$optional"; if grep -q -Ei 'ollama|11434' "$item"; then printf '%s:%s\n' "$definition" "$item"; else status=$?; [ "$status" -eq 1 ] || { set +f; return "$status"; }; fi; done; set +f; done <"$environment_files"; rm -f "$environment_files"; scan_systemd_runtime_consumers
}
scan_container_rows() {
