...
load_cron_inventory_helper() { if [ "$(id -u)" = 0 ]; then [ -z "${RETIRE_OLLAMA_CRON_INVENTORY_HELPER:-}" ] || die 'privileged cron inventory helper override refused'; helper="$SCRIPT_DIR/retire-ollama-cron-inventory.sh"; elif [ -n "${RETIRE_OLLAMA_CRON_INVENTORY_HELPER:-}" ]; then [ -n "${RETIRE_OLLAMA_TEST_BIN:-}" ] || die 'cron inventory helper override requires test harness'; helper=$RETIRE_OLLAMA_CRON_INVENTORY_HELPER; else helper="$SCRIPT_DIR/retire-ollama-cron-inventory.sh"; fi; [ -f "$helper" ] && [ ! -L "$helper" ] || die 'cron inventory helper missing'; . "$helper"; }
# shellcheck disable=SC1090,SC1091 # Sealed sibling or unprivileged test-injected helper.
