...
disable_unit() { revalidate_before disable_unit; systemctl stop "$TIMER" "$UNIT" || die 'unit stop failed'; systemctl disable "$TIMER" "$UNIT" || die 'unit disable failed'; assert_postcondition disable_unit; }
