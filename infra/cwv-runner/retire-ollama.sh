...
recovery_socket_snapshot() {
  ...
  raw=$(temp_path); for table in "$RECOVERY_PROC_ROOT/net/tcp" "$RECOVERY_PROC_ROOT/net/tcp6"; do family=tcp; case "$table" in *tcp6) family=tcp6;; esac; awk -v family="$family" -v clientsOnly="${RECOVERY_CLIENT_SOCKETS_ONLY:-}" 'NR > 1 { split($2,local_endpoint,":"); split($3,remote_endpoint,":"); if (clientsOnly != "yes" && $4 == "0A" && local_endpoint[2] == "2CAA") print family "|listener|" local_endpoint[1] "|" local_endpoint[2] "|" $10; else if ($4 == "01" && remote_endpoint[2] == "2CAA") print family "|client|" remote_endpoint[1] "|" remote_endpoint[2] "|" $10 }' "$table" >>"$raw" || die 'recovery socket table scan failed'; done
