...
retirement_helpers() { load_cron_inventory_helper; load_at_quiescence_helper; load_consumer_scanners; }; retirement_lock() { if [ -n "${RETIRE_OLLAMA_TEST_BIN:-}" ]; then lock=${RETIRE_OLLAMA_LOCK:-"${TMPDIR:-/tmp}/baci-cwv-retire-ollama.lock"}; flock=${RETIRE_OLLAMA_FLOCK:-/usr/bin/true}; else lock=/run/lock/baci-cwv-retire-ollama.lock; flock=/usr/bin/flock; fi; exec 9>"$lock" || die 'retirement lock open failed'; "$flock" -n 9 || die 'another retirement operation owns the lock' 75; }; retirement_prepare() { init_temp_root; trap 'cleanup_temp' EXIT HUP INT TERM; retirement_helpers; retirement_lock; }; main() { case "${1:-}" in --scan) root; retirement_prepare; scan;; --apply) root; retirement_prepare; apply;; --recovery-scan) type recovery_scan >/dev/null 2>&1 || review_required 'recovery helper missing'; recovery_scan;; *) usage;; esac; }
