...
test('discovers a custom Compose file after validated Docker global options', async () => {
  const directory = await realpath(
    await mkdtemp(join(tmpdir(), 'baci-compose-cli-global-option-'))
  );
  const composeRoot = join(directory, 'projects');
  const systemdRoot = join(directory, 'units');
  const compose = join(composeRoot, 'workload.yml');
  const unit = join(systemdRoot, 'application.service');
  try {
    await Promise.all([mkdir(composeRoot), mkdir(systemdRoot)]);
    await Promise.all([
      ...
      writeFile(
        unit,
        `[Service]\nExecStart=/usr/bin/docker --host unix:///var/run/docker.sock compose -f ${compose} up\n`
      ),
    ]);
    ...
    const { stdout } = await execFileAsync('sh', [
      '-c',
      `${prelude}. "$1"; SCRIPT_DIR=$(dirname "$1"); COMPOSE_ROOTS="$2"; SYSTEMD_ROOTS="$3"; init_temp_root; trap cleanup_temp EXIT; scan_compose_definitions`,
      'retire-ollama-compose-cli-global-option-test',
      script.pathname,
      composeRoot,
      systemdRoot,
    ]);
