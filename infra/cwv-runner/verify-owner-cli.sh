...
expect_refusal() { stem=$1 docs=$2 expected_kind=$3; shift 3; wire="$stem.wire"; body="$stem.body.json"; error="$stem.stderr"; for file in "$wire" "$body" "$error"; do [ ! -e "$file" ] && [ ! -L "$file" ] || refuse; done
  ...
  message=$(json_get "$body" message); case $expected_kind in (duplicate) case $message in ('Reference already exists'|'Repository rule violations found') :;; (*) refuse;; esac;; (update|delete) [ "$message" = 'Repository rule violations found' ] || [ "$message" = "$(/usr/bin/printf 'Repository rule violations found\n\nCannot %s this protected ref.\n\n' "$expected_kind")" ] || refuse;; (*) refuse;; esac; if [ "$(json_get "$body" documentation_url)" = "$docs" ] && /usr/bin/plutil -extract status xml1 -o - "$body" 2>/dev/null | /usr/bin/grep -q '<string>422</string>'; then :; else refuse; fi
}
