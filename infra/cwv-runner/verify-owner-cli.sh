...
verify_source_receipt() (
  ...
  case $purpose in
    (task7-provisioning)
      [ "$(json_get "$receipt" sourceHashes.dispatcherSha256)" = "$(sha256 "$dispatcher")" ] && [ "$(json_get "$receipt" sourceHashes.verifierSha256)" = "$(sha256 "$verifier")" ] || refuse;;
    (task9-exact-run)
...
verify_source() {
  ...
  [ -n "$manifest$manifest_sha$policy$dispatcher$verifier$purpose$output$output_digest" ] || refuse
  if ! is_sha256 "$manifest_sha" || ! is_purpose "$purpose"; then refuse; fi
...
prepare_task9_bootstrap_node() {
  ...
  provenance=$(/usr/bin/printf '{"artifact":"node","checksumSha256":"%s","keyringSha256":"%s","schemaVersion":1,"sha256":"%s","signatureSha256":"%s","version":"24.18.0"}' "$(sha256 "$checksums")" "$(sha256 "$keyring")" "$(sha256 "$root/prepared-node/node")" "$(sha256 "$signature")"); write_atomic "$root/prepared-node/node-provenance.json" "$provenance" 0400; /bin/rm -rf -- "$stage"; trap - EXIT HUP INT TERM; /bin/sync
}
parse_common() {
...
exec_task7() {
  ...
  case $operation in
    ...
    (read-auditor-app-registration) jwt=$(app_jwt) || refuse; node=$(verified_task7_node) || refuse; /usr/bin/printf '%s' "$jwt" | exec "$node" --input-type=module -e 'import https from "node:https";let jwt="";for await(const chunk of process.stdin){jwt+=chunk;if(jwt.length>4096)process.exit(65)}if(!/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(jwt))process.exit(65);const fail=()=>{process.exitCode=65};const request=https.request({agent:false,headers:{Accept:"application/vnd.github+json",Authorization:`Bearer ${jwt}`,"User-Agent":"baci-cwv-owner-verifier","X-GitHub-Api-Version":"2026-03-10"},hostname:"api.github.com",method:"GET",path:"/app",port:443,protocol:"https:"},response=>{let bytes=0;const chunks=[];response.on("data",chunk=>{bytes+=chunk.length;if(bytes>1048576){fail();request.destroy();return}chunks.push(chunk)});response.on("end",()=>{if(response.statusCode!==200||bytes>1048576){fail();return}const body=Buffer.concat(chunks);try{JSON.parse(body)}catch{fail();return}process.stdout.write(body)})});request.setTimeout(30000,()=>{fail();request.destroy()});request.on("error",fail);request.end();';;
