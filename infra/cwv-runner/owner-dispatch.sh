...
compose_task9_bundle() { [ "$#" -ge 14 ] && [ "$1" = --transaction-dir ] && [ "$3" = --policy ] && [ "$5" = --reviewed-policy-sha256 ] && [ "$7" = --reviewed-helper-sha256 ] && [ "$9" = --reviewed-launcher-sha256 ] && [ "${11}" = --reviewed-composer-sha256 ] && [ "${13}" = -- ] || refuse; transaction_dir=$2; policy=$4; reviewed_policy_sha=$6; reviewed_helper_sha=$8; reviewed_launcher_sha=${10}; reviewed_composer_sha=${12}; shift 13; assert_transaction "$transaction_dir"; [ "$policy" = "$transaction_dir/policy.json" ] || refuse; assert_child_file "$policy"; [ "$(file_mode "$policy")" = 400 ] && [ "$(sha256 "$policy")" = "$reviewed_policy_sha" ] || refuse; prepare_gh; helper="$SCRIPT_DIR/task9-compose-bundle.sh"; [ -f "$helper" ] && [ ! -L "$helper" ] && [ "$(sha256 "$helper")" = "$reviewed_helper_sha" ] || refuse; exec "$helper" --transaction-dir "$transaction_dir" --policy "$policy" --reviewed-policy-sha256 "$reviewed_policy_sha" --source-root "$SCRIPT_DIR" --reviewed-helper-sha256 "$reviewed_helper_sha" --reviewed-launcher-sha256 "$reviewed_launcher_sha" --reviewed-composer-sha256 "$reviewed_composer_sha" --github-sha256 "$(sha256 "$transaction_dir/tools/gh/bin/gh")" -- "$@"; }
initialize_task9() {
