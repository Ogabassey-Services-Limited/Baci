...
publish_once() { temporary=$1 output=$2; [ ! -e "$output" ] && [ ! -L "$output" ] || return 1; /bin/sync -f "$temporary" && /bin/ln "$temporary" "$output" && /bin/sync -f "$output" && /bin/sync -f "$(/usr/bin/dirname -- "$output")" && /bin/rm -f -- "$temporary"; }
...
task9_until() { operation=$1; shift; count=0; while :; do task9 "$operation" "$@"; count=$((count + 1)); [ "$count" -le 10 ] || refuse; case $operation in (list-attestation-runs) phase=$(json_get "$state" phase); [ "$phase" = QUIESCENT ] && break; case $phase in (QUEUED|RUNNING) [ "$(json_get "$state" postDispatchEvidence.run.id)" = "$(json_get "$state" run.id)" ] && [ "$(json_get "$state" postDispatchEvidence.run.attempt)" = "$(json_get "$state" run.attempt)" ] && break;; esac;; (list-runner-inventory) json_get "$state" runnerEvidence.runnerId >/dev/null 2>&1 && break;; (*) break;; esac; done; }
...
task9_exact() {
  evidence_directory="/private/tmp/baci-cwv-h0-evidence-$(json_get "$state" sourceAuthorization.transactionId)"; readonly evidence_directory; task9_root_started=0; task9_cleanup_confirmed=0; rerun_pending=0; trap 'task9_cancel; task9_manual; task9_abort' EXIT HUP INT TERM; task9_until list-attestation-runs "$@"; task9 dispatch-exact-run --state "$state" --state-sha256 "$state_sha" --source-authorization "$source_authorization" --source-authorization-sha256 "$source_authorization_sha256"; task9_until list-attestation-runs --state "$state" --state-sha256 "$state_sha" --source-authorization "$source_authorization" --source-authorization-sha256 "$source_authorization_sha256"; task9_campaign; [ "$rerun_pending" = 0 ] || { rerun_pending=0; task9_campaign; }; [ "$rerun_pending" = 0 ] || refuse; task9_root_started=0; trap - EXIT HUP INT TERM
}
...
task7_record() { probe_id=$1 response="$transaction_dir/probe-$probe_id-ref-response.json" ledger="$transaction_dir/task7-probe-$probe_id.json"; assert_child_file "$response"; [ ! -e "$ledger" ] && [ ! -L "$ledger" ] || refuse
  ref=$(json_get "$response" ref); object=$(json_get "$response" object.sha); [ -n "$ref" ] && [ "${#object}" -eq 40 ] || refuse; case $object in (*[!0-9a-f]*|'') refuse;; esac; temporary=$(/usr/bin/mktemp "$transaction_dir/task7-probe-$probe_id.XXXXXX") || refuse; /usr/bin/printf '{"objectSha":"%s","policyFileSha256":"%s","ref":"%s","schemaVersion":1,"sourceAuthorizationSha256":"%s","targetSha":"%s"}' "$object" "$(sha256 "$policy")" "$ref" "$(sha256 "$source_authorization")" "$target" >"$temporary"; /bin/chmod 0400 "$temporary"; publish_once "$temporary" "$ledger" || refuse; }
task7_cleanup() { [ "${task7_armed-0}" = 1 ] && [ "${task7_activated-0}" = 0 ] || return 0; for probe_id in 0 1 2; do [ -f "$transaction_dir/probe-$probe_id-ref-response.json" ] && task7_op rollback-owned-probe-ref "$probe_id" >/dev/null 2>&1 || :; done; }
task7_manual() { output="$transaction_dir/task7-manual-reconciliation.json"; [ ! -e "$output" ] && [ ! -L "$output" ] || return 0; temporary=$(/usr/bin/mktemp "$transaction_dir/task7-manual.XXXXXX") || return 0; /usr/bin/printf '%s' '{"reason":"post-activation-refusal-failed","schemaVersion":1}' >"$temporary"; /bin/chmod 0400 "$temporary"; publish_once "$temporary" "$output" || return 0; }
task7_probes() {
