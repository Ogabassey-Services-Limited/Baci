...
export function FloatingControls() {
    ...
    const safeFieldChange = (fieldName: string, value: any) => {
        ...
        const itemIndex = newContent.findIndex((item: any) => item.props.id === selectedItem.props.id);
    ...
    return (
        <div className="fixed right-6 top-24 z-[60] w-80 bg-background/95 backdrop-blur-md border rounded-lg shadow-xl overflow-hidden flex flex-col max-h-[calc(100vh-8rem)] animate-in slide-in-from-right-5 duration-200">
            <div className="flex items-center justify-between p-4 border-b bg-muted/30">
                <h3 className="font-semibold text-sm">Edit {componentConfig.label || selectedItem.type}</h3>
                <Button
                    variant="ghost"
                    size="icon"
                    className="h-6 w-6"
                    onClick={() => dispatch({ type: 'setUi', ui: { selectedItem: null } as any })}
                >
                    <X className="h-4 w-4" />
                </Button>
            </div>
                        ...
                        <div key={fieldName} className="space-y-2">
                            ...
                            {fieldConfig.type === 'number' && (
                                <Input
                                    type="number"
                                    value={value || ''}
                                    onChange={(e) => safeFieldChange(fieldName, Number(e.target.value))}
                                />
                            )}
