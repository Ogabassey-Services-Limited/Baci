...
import { Search, ShoppingCart, Bell, User, Loader2, Sparkles, X, LayoutGrid } from 'lucide-react';
import { searchProductsWithGemini, ProductRecommendation } from '@/services/gemini-service-mock';
                            ...
                            <form onSubmit={handleSearch} className="relative group">
                                <div className="relative flex items-center bg-white rounded-md overflow-hidden h-11 transition-all duration-300 focus-within:ring-2 focus-within:ring-red-500/50">
                                    <div className="pl-3 pr-2 text-red-600">
                                        <Sparkles size={18} className={isSearching ? "animate-pulse" : ""} fill="currentColor" />
                                    </div>
                                    <input
                                        type="text"
                                        value={query}
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setQuery('')}
                                        className={cn("p-2 text-gray-400 hover:text-gray-600", query ? 'block' : 'hidden')}
                                    >
                                        <X size={16} />
                                    </button>
                                        ...
                                        <div className="p-4">
                                            {isSearching ? (
                                                    ...
                                                    <ul className="space-y-3">
                                                        {results.map((item, idx) => (
                                                            <li key={idx} className="flex justify-between items-start group cursor-pointer hover:bg-gray-50 p-2 rounded-md transition-colors">
                                                                <div>
                                                                    <p className="font-bold text-gray-900 group-hover:text-red-600 transition-colors">{item.name}</p>
                                                ...
                                                <div className="text-center py-6 text-gray-500">
                                                    ...
                                                    <p className="text-xs mt-1">Try asking "Best laptop for gaming" or "Cheap phones"</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
