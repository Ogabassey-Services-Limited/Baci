...
export const OgabasseyV2CartPage: React.FC<OgabasseyV2CartPageProps> = ({ storeSlug }) => {
    const { cart, removeFromCart, updateQuantity, applyNegotiatedPrice, applyCartWideNegotiation, toggleAssurance } = useCart();
    const [negotiationState, setNegotiationState] = useState<NegotiationState | null>(null);
    const router = useRouter();

    // Calculate subtotal
    const subtotal = cart.reduce((acc, item) => {
        const price = item.negotiatedPrice || item.rawPrice;
            ...
            <div className="max-w-[1400px] mx-auto px-4 md:px-6 w-full flex-1 flex flex-col">
                ...
                <div className="flex items-center justify-between mb-6 shrink-0">
                    <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                        ...
                        Cart <span className="text-gray-400 text-lg font-medium">({cart.length})</span>
                    </h1>
                    <Link href="/" className="text-sm font-medium text-red-600 hover:text-red-700 hidden md:block">
                        Continue Shopping
                    </Link>
                </div>
                                ...
                                return (
                                    <div key={item.cartItemId} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden transition-all md:hover:shadow-md active:scale-[0.99] md:active:scale-100 touch-manipulation">
                                        ...
                                        <div className="flex gap-4">
                                            {/* Image */}
                                            <Link href={`/product/${item.id}`} className="w-20 h-20 md:w-28 md:h-28 bg-gray-50 rounded-xl border border-gray-100 p-2 flex-shrink-0 flex items-center justify-center">
