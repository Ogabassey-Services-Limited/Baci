...
import { CategoryFiltersSidebar, FilterState } from '../components/CategoryFiltersSidebar';
...
export const OgabasseyV2CategoryPage: React.FC<OgabasseyV2CategoryPageProps> = ({ storeSlug, categoryName }) => {
    ...
    const categoryProducts = useMemo(() => {
        // Decode category name from URL (e.g. "Phones" or "All")
        const decodedCategory = decodeURIComponent(categoryName);
        return products.filter(p => decodedCategory === 'All' || p.category === decodedCategory);
    }, [categoryName]);
    ...
    const decodedCategory = decodeURIComponent(categoryName);
    const pageTitle = decodedCategory === 'All' ? 'All Products' : decodedCategory;
