...
export const OgabasseyV2ProductDetails: React.FC<OgabasseyV2ProductDetailsProps> = ({ storeSlug, productId }) => {
    ...
    const currentCartItem = useMemo(() => {
        if (selectedColor === null && productData.colors.length > 0) return undefined;
        if (selectedStorage === null && productData.storage.length > 0) return undefined;

        return cart.find(item => {
            if (String(item.id) !== String(productData.id)) return false;

            // Check attributes
            const colorMatch = !productData.colors.length || (item.variantAttributes?.color === productData.colors[selectedColor!].name);
            const storageMatch = !productData.storage.length || (item.variantAttributes?.storage === productData.storage[selectedStorage!]);

            return colorMatch && storageMatch;
        });
    }, [cart, productData.id, selectedColor, selectedStorage, productData.colors, productData.storage]);
    ...
    const handleDecrement = () => {
        if (currentCartItem && quantityInCart > 0) {
            if (quantityInCart <= 1) {
                ...
                updateQuantity(String(productData.id), quantityInCart - 1, currentCartItem.variantId);
            }
        }
    };
        ...
        <div className="bg-white pb-32 pt-4 relative">
            ...
            <div className="max-w-[1400px] mx-auto px-4 md:px-6">
                ...
                <nav className="flex items-center text-sm text-gray-500 mb-8 overflow-x-auto whitespace-nowrap pb-2">
                    <Link href="/" className="md:hover:text-red-600 transition-colors">Home</Link>
                    <ChevronRight size={16} className="mx-2" />
                    <Link href={`/category/${productData.category}`} className="md:hover:text-red-600 transition-colors">{productData.category}</Link>
                    <ChevronRight size={16} className="mx-2" />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8">
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        <div className="relative aspect-square bg-gray-50 rounded-2xl flex items-center justify-center overflow-hidden border border-gray-100">
                            ...
                            />
                            <div className={`absolute top-4 left-4 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider ${productData.condition === 'New' ? 'bg-emerald-500' : 'bg-amber-500'
                                }`}>
                                        ...
                                        {comparableProducts.map((comp, idx) => (
                                            <div key={idx} className="space-y-4 relative group">
                                                <div className="h-44 flex flex-col items-center justify-end pb-4 pt-4 opacity-90 hover:opacity-100 transition-opacity">
                                                    ...
                                                    <Link href={`/product/${comp.id}`} className="font-bold text-gray-500 text-center text-sm line-clamp-2 px-2 hover:text-red-600 transition-colors h-10 flex items-center">{comp.name}</Link>
                                                </div>
