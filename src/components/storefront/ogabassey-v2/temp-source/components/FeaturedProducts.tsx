...
export const FeaturedProducts: React.FC<FeaturedProductsProps> = ({
  ...
  const filteredProducts = products.filter((product) => {
    ...
    // Price Filter
    if (
      product.rawPrice < minPrice ||
      (maxPrice > 0 && product.rawPrice > maxPrice)
    ) {
      return false;
    }
    return true;
    ...
    <section className="max-w-[1400px] mx-auto px-4 md:px-6 pt-6 md:pt-8 pb-20">
      <div className="flex items-center justify-between mb-8">
        ...
        {showViewAll && (
          <Link
            to={viewAllLink}
            className="text-gray-500 hover:text-red-600 font-medium transition-colors text-sm md:text-base hidden sm:block"
          >
            View all products
          </Link>
        )}
          ...
          {filteredProducts.map((product, index) => {
            ...
            return (
              <React.Fragment key={product.id}>
                ...
                {(index + 1 === 4 || index + 1 === 8) && (
                  <div
                    className={`col-span-1 ${viewMode === 'grid' ? 'col-span-2 lg:col-span-4' : 'w-full'} flex items-center justify-center my-4`}
                  >
