...
import { type ChatMessage, chatWithGemini } from '../services/gemini';
...
const SantaIcon: React.FC<{ className?: string }> = ({ className }) => (
    ...
    >
        {/* Hat Back */}
        <path
            d="M48 20C48 20 54 36 32 36C32 36 18 36 16 20C16 20 20 4 32 4C44 4 48 20 48 20Z"
            fill="#DC2626"
        />
        {/* Hat Tip Falling */}
        <path
            d="M46 12C46 12 58 16 60 28"
            stroke="#DC2626"
            strokeWidth="6"
            strokeLinecap="round"
        />
        ...
        <path
            d="M14 30C12 30 10 32 10 35C10 38 12 40 14 40C14 44 16 48 22 52C26 55 32 56 32 56C32 56 38 55 42 52C48 48 50 44 50 40C52 40 54 38 54 35C54 32 52 30 50 30"
            fill="white"
            stroke="#E5E7EB"
            strokeWidth="1"
        />
        ...
        {/* Moustache */}
        <path
            d="M32 40C32 40 28 42 24 40C24 40 26 36 32 38C38 36 40 40 40 40C36 42 32 40 32 40Z"
            fill="white"
            stroke="#E5E7EB"
            strokeWidth="0.5"
        />
        ...
        {/* Hat Brim */}
        <path
            d="M14 20C14 20 14 26 32 26C50 26 50 20 50 20C50 20 50 14 32 14C14 14 14 20 14 20Z"
            fill="white"
        />
    </svg>
);
...
export const ChatWidget: React.FC = () => {
    ...
    const handleSend = async (text: string) => {
        ...
        try {
            ...
            setMessages((prev) => [
                ...
                {
                    role: 'model',
                    text: "I'm having a little trouble connecting. Please try again or contact support.",
                },
            ]);
        } finally {
            setIsLoading(false);
    ...
    return (
        <div
            className={`fixed bottom-24 md:bottom-6 right-4 md:right-6 z-[100] font-sans flex flex-col items-end gap-4 transition-all duration-300 ${isCartOpen ? 'opacity-0 pointer-events-none translate-y-4' : 'opacity-100 translate-y-0'}`}
        >
            {/* Chat Window */}
            {isOpen && (
                <div
                ...
                >
                    {/* Header */}
                    <div
                        className={`${isSanta ? 'bg-red-50 border-red-100' : 'bg-white border-gray-100'} border-b p-4 flex items-center justify-between shrink-0 relative`}
                    >
                        <div className="flex items-center gap-3 relative z-10">
                            <div className="relative">
                                <div
                                    className={`w-10 h-10 rounded-full flex items-center justify-center border ${isSanta ? 'bg-white border-red-200' : 'bg-red-50 border-red-100'}`}
                                >
                                    {isSanta ? (
                                        <SantaIcon className="w-8 h-8" />
                                    ) : (
                            ...
                            <div>
                                <h3 className="font-bold text-gray-900 text-[15px] flex items-center gap-2">
                                    ...
                                    <span
                                        className={`${isSanta ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'} text-[10px] px-2 py-0.5 rounded-full font-medium border border-transparent`}
                                    >
            ...
            >
                {isOpen ? (
                    ...
                    <>
                        {isSanta ? (
                            <SantaIcon className="w-8 h-8 drop-shadow-sm" />
                        ) : (
                            <Sparkles
                                size={28}
