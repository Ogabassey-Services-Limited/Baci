...
export const CategoryPage: React.FC = () => {
    ...
    const availableOptions = useMemo(() => {
        ...
        categoryProducts.forEach((p) => {
            ...
            if (p.colors) p.colors.forEach((c: any) => options.colors.add(typeof c === 'string' ? c : c.name || c));
    ...
    const filteredProducts = useMemo(() => {
        return categoryProducts.filter((p) => {
            // Price
            if (p.rawPrice && (p.rawPrice < filters.minPrice || p.rawPrice > filters.maxPrice))
                return false;
            ...
            if (filters.storage.length > 0) {
                const productStorage = (Array.isArray(p.storage) ? p.storage : [p.storage]).filter((s): s is string => !!s);
                if (!productStorage.some(s => filters.storage.includes(s))) return false;
            }
            ...
            if (filters.colors.length > 0) {
                if (!p.colors || !p.colors.some((c: any) => filters.colors.includes(typeof c === 'string' ? c : c.name || c)))
                    return false;
            }
    ...
    const pageTitle = categoryName === 'All' ? 'All Products' : decodeURIComponent(categoryName);
