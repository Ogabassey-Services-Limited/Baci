...
export const Navbar: React.FC = () => {
    ...
    return (
        <>
            <div className={cn("fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 transition-transform duration-300", !isVisible && "-translate-y-full")}>
                <div className="max-w-[1400px] mx-auto px-4 md:px-6 h-20 flex items-center justify-between gap-8">
                    ...
                    <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                            ...
                            {showNotifications && (
                                <div className="absolute top-full right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                    <div className="p-4 border-b border-gray-50 flex justify-between items-center bg-white sticky top-0 z-10">
                                        ...
                                        {unreadCount > 0 && (
                                            <button onClick={markAllAsRead} className="text-xs font-bold text-red-600 hover:text-red-700">
                                                Mark all read
                                            </button>
                                        )}
                                    </div>
                                    <div className="max-h-[400px] overflow-y-auto">
                                        {notifications.length > 0 ? (
                                            notifications.map((notif: any) => (
                                                <div key={notif.id} className={`p-4 border-b border-gray-50 hover:bg-gray-50 transition-colors cursor-pointer ${!notif.read ? 'bg-red-50/30' : ''}`}>
                                                    <div className="flex gap-3">
                                                        <div className={`w-2 h-2 mt-2 rounded-full shrink-0 ${!notif.read ? 'bg-red-500' : 'bg-gray-300'}`}></div>
                                                        <div>
                        ...
                        {/* Account */}
                        <Link href="/profile" className="hidden sm:flex items-center gap-2 p-1.5 pr-3 rounded-full hover:bg-gray-100 border border-transparent hover:border-gray-200 transition-all group">
                        ...
                        <div className="flex items-center gap-6 h-12">
                            ...
                            <div className="relative" ref={categoryRef}>
                                <button
                                    ...
                                    className={cn("flex items-center gap-2 text-sm font-bold text-gray-700 hover:text-red-600 transition-colors px-1 py-1", showCategoryDropdown && "text-red-600")}
                                >
                                    <LayoutGrid size={18} />
                                    Shop by Category
                            ...
                            {/* IMEI Checker */}
                            <Link href="/imei-check" className="flex items-center gap-2 text-sm font-bold text-gray-700 hover:text-red-600 transition-colors px-1 py-1">
                                <ScanBarcode size={18} />
                                IMEI Checker
                            </Link>
                            ...
                            {/* Repairs */}
                            <Link href="/repairs" className="flex items-center gap-2 text-sm font-bold text-gray-700 hover:text-red-600 transition-colors px-1 py-1">
                                <Wrench size={18} />
                                Repairs
                            </Link>
                            ...
                            {/* Wallet */}
                            <Link href="/wallet" className="flex items-center gap-2 text-sm font-bold text-gray-700 hover:text-red-600 transition-colors px-1 py-1">
                                <Wallet size={18} />
                                Wallet
                            </Link>
                        </div>
