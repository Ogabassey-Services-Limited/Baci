...
export const InteractiveProductGrid: React.FC<InteractiveProductGridProps> = ({
    ...
    const filteredProducts = products.filter(product => {
        ...
        // Price Filter
        const productPrice = product.rawPrice ?? (typeof product.price === 'number' ? product.price : 0);
        if (productPrice < priceRange.min || (priceRange.max > 0 && productPrice > priceRange.max)) {
            return false;
        }
        return true;
    });
