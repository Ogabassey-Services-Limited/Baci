...
export const ProductDetails: React.FC = () => {
    ...
    const validateAndAddToCart = () => {
        const missing = [];
        if (productData.colors && productData.colors.length > 0 && selectedColor === null) missing.push('Color');
        if (productData.storage && productData.storage.length > 0 && selectedStorage === null) missing.push('Storage');
        ...
        addToCart(
            {
                ...
                image: productData.images ? productData.images[selectedImage] : productData.image,
            } as any,
            quantity,
            {
                ...
                variantAttributes: {
                    color: selectedColor !== null && productData.colors ? productData.colors[selectedColor].name : '',
                    storage: selectedStorage !== null && productData.storage ? productData.storage[selectedStorage] : '',
                }
            }
        );
        ...
        <div className="min-h-screen bg-gray-50 font-sans">
            ...
            <div className="max-w-[1400px] mx-auto px-4 md:px-6 py-4">
                <div className="flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <Link href={"/category/phones" as any} className="hover:text-red-600 transition-colors">Phones</Link>
                    <ChevronRight size={14} />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-12">
                        ...
                        <div className="p-6 md:p-10 bg-white">
                            <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-50 mb-6 group">
                                <img
                                    src={productData.images ? productData.images[selectedImage] : productData.image}
                            ...
                            <div className="mb-6">
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <div className="flex items-center gap-1 text-yellow-400">
                                        ...
                                        <span className="text-xs text-gray-400 font-medium">({productData.reviews} reviews)</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-end gap-4 mb-6">
                                    ...
                                    {productData.discount && (
                                        <div className="mb-2 px-2 py-1 bg-red-50 text-red-700 text-xs font-bold rounded-lg">
                                            Save ₦{((typeof productData.originalPrice === 'number' ? productData.originalPrice : 0) - (productData.rawPrice ?? (typeof productData.price === 'number' ? productData.price : 0))).toLocaleString()}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="space-y-6 mb-8">
                                    ...
                                    {productData.colors && productData.colors.length > 0 && (
                                        <div>
                                            <div className="flex justify-between mb-3">
                                                <span className="text-sm font-bold text-gray-900">Select Color</span>
                                                <span className="text-sm text-gray-500 font-medium">
                                                    {selectedColor !== null ? productData.colors[selectedColor].name : 'Required'}
                                                </span>
                                            </div>
                                                ...
                                                {productData.colors.map((color, idx) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`group relative w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 outline-none focus:ring-4 focus:ring-red-100 ${isSelected
                                                                ? 'border-[3px] border-red-600 scale-110 shadow-lg'
                                                                : 'border border-gray-200 hover:border-gray-400 hover:scale-105'
                                                                }`}
                                                            title={color.name}
                                                        >
                                                            <div className="w-9 h-9 rounded-full border border-black/5 shadow-inner" style={{ backgroundColor: color.value }}></div>
                                                            {isSelected && (
                                                                <div className="absolute inset-0 flex items-center justify-center z-10">
                                                                    <Check size={18} className={isLight ? "text-gray-900" : "text-white"} strokeWidth={3} />
                                                                </div>
                                                            )}
                                                        </button>
                                                    )
                                    ...
                                    {productData.storage && productData.storage.length > 0 && (
                                        <div>
                                            <div className="flex justify-between mb-3">
                                                <span className="text-sm font-bold text-gray-900">Storage</span>
                                                <span className="text-sm text-gray-500 font-medium">
                                                    {selectedStorage !== null ? productData.storage[selectedStorage] : 'Required'}
                                                </span>
                                            </div>
                                            <div className="flex flex-wrap gap-3">
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-8">
                                    <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-500">Official manufacturer warranty</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-500">For orders over ₦500k</p>
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="fixed inset-0 z-[80] flex items-end md:items-center justify-center px-0 md:px-4">
                    ...
                    <div className="bg-white w-full md:max-w-md md:rounded-2xl rounded-t-2xl shadow-2xl relative z-10 animate-in slide-in-from-bottom-10 duration-300">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            {(missingFields.includes('Color') || missingFields.length === 0) && productData.colors && (
                                <div>
                            ...
                            {(missingFields.includes('Storage') || missingFields.length === 0) && productData.storage && (
                                <div>
            ...
            <NegotiationModal
                ...
                currentPrice={productData.rawPrice ?? (typeof productData.price === 'number' ? productData.price : 0)}
                onSuccess={(price) => {
                    setIsNegotiationOpen(false);
                    if ((selectedColor !== null || !productData.colors?.length) && (selectedStorage !== null || !productData.storage?.length)) {
