...
export function FileUploader({
    ...
    const onDrop = useCallback((acceptedFiles: File[], fileRejections: FileRejection[]) => {
        ...
        if (acceptedFiles?.length) {
            ...
            setPreviews(prev => {
                const newPreviews = newFiles.map(f => (f as File & { preview: string }).preview);
        ...
        <div className={cn("space-y-4", className)}>
            <div
                ...
                className={cn(
                    ...
                    (files.length + (initialFiles?.length || 0)) >= maxFiles && "opacity-50 cursor-not-allowed pointer-events-none"
                )}
            >
                <input {...getInputProps()} />
