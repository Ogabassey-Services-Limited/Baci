...
export function validateDNSRecord(record: DNSRecord): ValidationResult {
    ...
    if (record.ttl && (record.ttl < 60 || record.ttl > 86400)) {
        warnings.push('TTL should be between 60 seconds and 24 hours for optimal performance');
    }
    ...
    switch (record.type) {
        ...
        case 'TXT':
            if (record.value.length > 255) {
                ...
                warnings.push('TXT record value exceeds 255 characters; ensure it is properly split if required by nameserver');
            }
...
export function validateDNSRecordBatch(records: DNSRecord[]): ValidationResult {
    ...
    records.forEach((record, index) => {
        ...
        allWarnings.push(...result.warnings.map(w => `Record ${index + 1}: ${w}`));
    });
    ...
    records.forEach(record => {
        ...
        if (seen.has(key)) {
            allWarnings.push(`Duplicate record detected: ${record.type} ${record.name}`);
        }
        seen.add(key);
    });
...
function validateSPF(spf: string): ValidationResult {
    ...
    if (lookupCount > 10) {
        errors.push(`SPF record exceeds 10 DNS lookups (${lookupCount} found). This will cause SPF validation failures.`);
    }

    if (!spf.match(/[~\-?+]all$/)) {
        warnings.push('SPF record should end with an "all" mechanism (~all, -all, ?all, or +all)');
    }
