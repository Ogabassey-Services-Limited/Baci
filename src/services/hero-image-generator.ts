...
export async function generateHeroImageBatch(
    ...
    try {
        ...
        logger.info({ message: `Generating ${count} hero images for category: ${category}` });

        // Generate images in batches of 3 (using the 3 prompts)
        const batchesNeeded = Math.ceil(count / prompts.length);

        for (let batch = 0; batch < batchesNeeded && generatedImages.length < count; batch++) {
            for (const basePrompt of prompts) {
                ...
                try {
                    ...
                    if (uploadError) {
                        logger.error({ message: 'Failed to upload hero image to storage', error: uploadError });
                        continue;
                    }
                    ...
                    logger.info({ message: `Generated hero image ${generatedImages.length}/${count}`, url: publicUrl });
                } catch (imageError) {
                    logger.error({ message: 'Failed to generate individual hero image', error: imageError });
        ...
        if (insertError) {
            logger.error({ message: 'Failed to insert hero images into database', error: insertError });
...
export async function assignHeroImagesToMerchant(
    ...
    try {
        ...
        if (fetchError || !images || images.length === 0) {
            logger.warn({ message: 'No hero images available, generating new batch', category });
        ...
        if (updateError) {
            logger.error({ message: 'Failed to update merchant with hero images', error: updateError });
            return { success: false, error: updateError.message };
        }

        logger.info({ message: 'Successfully assigned hero images to merchant', merchantId, imageIds });
    ...
    try {
        ...
        if (assignResult.success) {
            ...
            await supabase
                .from('merchants')
                .update({
                    hero_images_regeneration_count: (merchant.hero_images_regeneration_count || 0) + 1
                })
                .eq('id', merchantId);
        }
