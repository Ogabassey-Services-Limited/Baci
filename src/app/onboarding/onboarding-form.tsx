    ...
    <div className='space-y-4'>
      ...
      {brandColors && (
        <div className='mt-6 animate-fade-in'>
            <div className="mb-4">
              <div className='text-sm text-muted-foreground font-medium mb-2'>Extracted Brand Colors</div>
              <div className='flex items-center gap-2'>
                {displayedColors.map(({ role, color }) => (
                  <div key={role} className='flex flex-col items-center gap-1'>
                    <div className='w-10 h-10 rounded-full border-2' style={{ backgroundColor: color }} />
                    <span className='text-xs capitalize text-muted-foreground'>{role}</span>
                  </div>
                ))}
                <ThemedButton type="button" variant="ghost" size="icon" onClick={handleShuffleColors} disabled={isLoading} className='ml-2'>
