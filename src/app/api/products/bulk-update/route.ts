            ...
            try {
                if (change.type === 'update') {
                    ...
                    if (change.productId) {
                        ...
                        matchQuery = matchQuery.eq('sku', change.details.sku).eq('merchant_id', merchant.id);
                    } else {
                        // Fallback: match by name and merchant_id (risky but necessary for name-only sheets)
                        matchQuery = matchQuery.eq('name', change.details.name).eq('merchant_id', merchant.id);
                    }
                    ...
                    const sku = change.details.sku || generateSlug(change.details.name).toUpperCase().substring(0, 20);
                ...
                console.error(`Error processing change for ${change.details.name}:`, err);
                results.errors.push(`Failed to ${change.type} "${change.details.name}"`);
            }
        }
