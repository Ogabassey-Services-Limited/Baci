...
import type { UpdateNotificationInput, NotificationWithStats } from '@/types/notifications';
...
export async function GET(
  ...
  try {
    ...
    const { data: stats } = await supabase
      .rpc('get_notification_stats', { p_notification_id: id });
    ...
    const notificationWithStats: NotificationWithStats & { deliveries: unknown[] } = {
      ...notification,
      stats: stats?.[0] || {
        total_sent: 0,
        total_read: 0,
...
export async function PATCH(
  ...
  try {
    ...
    if (body.notification_type !== undefined) updates.notification_type = body.notification_type;
    if (body.priority !== undefined) updates.priority = body.priority;
    if (body.target_type !== undefined) updates.target_type = body.target_type;
    if (body.target_merchant_ids !== undefined) updates.target_merchant_ids = body.target_merchant_ids;
    if (body.target_segment !== undefined) updates.target_segment = body.target_segment;
