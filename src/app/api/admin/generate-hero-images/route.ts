...
export async function GET() {
    try {
        ...
        const categoryStats = stats.reduce((acc: Record<string, { total: number; avgUsage: number; totalUsage: number }>, img: { category: string; usage_count: number }) => {
            if (!acc[img.category]) {
            ...
            return acc;
        }, {} as Record<string, { total: number; avgUsage: number; totalUsage: number }>);

        // Calculate averages
        Object.keys(categoryStats).forEach(cat => {
            categoryStats[cat].avgUsage = categoryStats[cat].totalUsage / categoryStats[cat].total;
        });
