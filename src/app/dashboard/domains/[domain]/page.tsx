...
export default function DomainDetailsPage() {
    ...
    const [newRecord, setNewRecord] = useState<DNSRecord>({ type: 'A', name: '', value: '' });
    ...
    const handleToggleLock = async (checked: boolean) => {
        try {
            const res = await fetch(`/api/domains/${domain}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'update_lock', data: { lock: checked } }),
            });
        ...
        <div className="container mx-auto p-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground">Manage domain settings and configuration</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-2xl font-bold capitalize">{domainInfo?.status || 'Active'}</span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Expires On</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="flex items-center gap-2">
                            <span className="text-2xl font-bold">
                                {domainInfo?.expiryDate ? new Date(domainInfo.expiryDate).toLocaleDateString() : 'N/A'}
                            </span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Registrar Lock</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <Lock className={`w-5 h-5 ${lockStatus ? 'text-green-500' : 'text-yellow-500'}`} />
                                <span className="font-medium">{lockStatus ? 'Locked' : 'Unlocked'}</span>
                            </div>
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div>
                                ...
                                <CardDescription>Manage your domain's DNS records (A, CNAME, MX, etc.)</CardDescription>
                            </div>
                            <Dialog open={isAddRecordOpen} onOpenChange={setIsAddRecordOpen}>
                                ...
                                <DialogContent>
                                    <DialogHeader>
                                        ...
                                        <DialogDescription>Add a new DNS record for {domain}</DialogDescription>
                                    </DialogHeader>
                                            ...
                                            dnsRecords.map((record, index) => (
                                                <TableRow key={index}>
                                                    ...
                                                    <TableCell className="text-right">
                                                        <Button variant="ghost" size="icon" onClick={() => handleDeleteDnsRecord(index)}>
                ...
                <TabsContent value="email">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Forward emails from your domain to another address</CardDescription>
                        </CardHeader>
                                ...
                                <div className="flex-1 space-y-2">
                                    ...
                                    <div className="flex items-center">
                                        <Input
                                            ...
                                            onChange={(e) => setNewForward({ ...newForward, prefix: e.target.value })}
                                        />
                                        <span className="ml-2 text-muted-foreground">@{domain}</span>
                                    </div>
                                </div>
                                <div className="flex-1 space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => setNewForward({ ...newForward, forwardto: e.target.value })}
                                    />
                                </div>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {forwards.map((forward, index) => (
                                        <TableRow key={index}>
                                            <TableCell>{forward.prefix}@{domain}</TableCell>
                    ...
                    <div className="grid gap-6">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Hide your personal contact information from the public WHOIS database</CardDescription>
                            </CardHeader>
                            <CardContent className="flex items-center justify-between">
                                ...
                                <Switch checked={idProtection} onCheckedChange={handleToggleIdProtection} />
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Manage the nameservers for your domain</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-4">
                                    <div className="grid gap-2">
                                        <Label>Nameserver 1</Label>
                                        <Input defaultValue={nameservers?.ns1} placeholder="ns1.whogohost.com" />
                                    </div>
                                    <div className="grid gap-2">
                                        <Label>Nameserver 2</Label>
                                        <Input defaultValue={nameservers?.ns2} placeholder="ns2.whogohost.com" />
                                    </div>
