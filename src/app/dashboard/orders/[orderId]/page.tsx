...
import { initialOrders, type ShippingStatus, type PaymentStatus } from '../page'; // Reusing mock data from orders list
...
export default function OrderDetailsPage() {
  ...
  const [order, setOrder] = useState(initialOrders.find(o => o.orderNumber.replace('#', '') === orderId));
  ...
  const doesOrderRequireFulfillment = () => {
      ...
      return orderItem.product.fulfillmentFields && orderItem.product.fulfillmentFields.length > 0;
  }
  ...
  const handleUpdateStatus = (newStatus: ShippingStatus) => {
      ...
      setOrder(prevOrder => prevOrder ? { ...prevOrder, shippingStatus: newStatus, paymentStatus: newPaymentStatus } : prevOrder);
      toast({ 
  ...
  const getPrimaryAction = () => {
      switch (order.shippingStatus) {
          case 'Pending':
              return { text: 'Confirm Order', action: () => handleUpdateStatus('Processing'), icon: CheckCircle };
          case 'Processing':
              return { text: 'Ship Order', action: () => handleUpdateStatus('Shipped'), icon: Truck };
          case 'Shipped':
              return { text: 'Mark as Delivered', action: () => handleUpdateStatus('Delivered'), icon: PackageCheck };
          default:
              return null;
      }
  };
  ...
  const getSecondaryAction = () => {
      switch (order.shippingStatus) {
          case 'Pending':
          case 'Processing':
              return { text: 'Cancel Order', action: () => handleUpdateStatus('Canceled'), icon: XCircle, variant: 'outline' as const };
          case 'Delivered':
          case 'Shipped':
              return { text: 'Process Return', action: () => handleUpdateStatus('Returned'), icon: Undo2, variant: 'outline' as const };
          default:
              return null;
      }
  }
        ...
        <div className="grid gap-4 md:grid-cols-3">
            <div className="grid auto-rows-max items-start gap-4 md:col-span-2">
                ...
                <Card>
                    <CardHeader>
                        ...
                        {doesOrderRequireFulfillment() && (
                             <Alert variant="default" className="mt-2 bg-blue-50 border-blue-200 text-blue-800">
                                ...
                                <AlertDescription>
                                    One or more items in this order need specific details (e.g., serial numbers) before fulfillment.
                                </AlertDescription>
                            </Alert>
                        )}
                    </CardHeader>
                    <CardContent>
                        <div className="flex items-center gap-4">
                            <Image src={orderItem.product.image} alt={orderItem.name} width={64} height={64} className="rounded-md object-cover"/>
                            <div className="flex-1">
                                <p className="font-semibold">{orderItem.name}</p>
                                <p className="text-sm text-muted-foreground">{orderItem.quantity} x {formatCurrency(orderItem.price)}</p>
                            </div>
                            <p className="font-semibold">{formatCurrency(orderItem.price * orderItem.quantity)}</p>
                        </div>
                    </CardContent>
                    <CardFooter className="flex gap-2 justify-end">
                        {secondaryAction && 
                        <Button variant={secondaryAction.variant} onClick={secondaryAction.action} className="gap-1">
