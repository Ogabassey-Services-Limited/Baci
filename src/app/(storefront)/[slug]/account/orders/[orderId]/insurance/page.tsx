...
import { Loader2, ShieldCheck, AlertTriangle, FileText, ExternalLink } from 'lucide-react';
...
export default function InsurancePolicyPage() {
    ...
    const handleFileClaim = () => {
        ...
        if (!publicKey) {
            alert('Claims system is currently unavailable (Missing Configuration). Please contact support.');
            return;
        }
        ...
        mycoverai({
            action: 'claim',
            pk: publicKey,
            pid: [policy?.itemsInsured?.product_id || process.env.NEXT_PUBLIC_MYCOVER_GADGET_PRODUCT_ID],
    ...
    return (
        <div className="container max-w-3xl py-8 space-y-8">
            <div>
                <h1 className="text-2xl font-bold tracking-tight">Standard Insurance Policy</h1>
                <p className="text-muted-foreground">Managed by MyCover.ai • Underwritten by {policy.provider}</p>
            </div>

            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <div className="space-y-1">
                        <CardTitle className="text-base font-medium">Policy Status</CardTitle>
                ...
                <CardContent className="grid gap-4 pt-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="font-semibold">{new Date(policy.startDate).toLocaleDateString()}</div>
                        </div>
                        <div>
                            <div className="text-sm font-medium text-muted-foreground">Expiry Date</div>
                            <div className="font-semibold">{new Date(policy.expiryDate).toLocaleDateString()}</div>
                        </div>
                        <div>
                            <div className="text-sm font-medium text-muted-foreground">Coverage Amount</div>
                            <div className="font-semibold">₦{policy.coverage.toLocaleString()}</div>
                        </div>
                        <div>
                            <div className="text-sm font-medium text-muted-foreground">Premium Paid</div>
                            <div className="font-semibold">₦{policy.premium.toLocaleString()}</div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="text-sm font-medium text-muted-foreground mb-2">Insured Item</div>
                        <div className="flex items-center gap-2 p-3 border rounded-md bg-muted/40">
                            <ShieldCheck className="h-5 w-5 text-green-600" />
                            <span className="font-medium">{policy.itemsInsured?.product_name || 'Device'}</span>
                            <span className="text-xs text-muted-foreground ml-auto">IMEI: {policy.itemsInsured?.imei}</span>
                        </div>
                    </div>
                </CardContent>
                <CardFooter className="flex flex-col sm:flex-row gap-3 pt-4">
                    {policy.certificateUrl && (
                        <Button variant="outline" className="w-full sm:w-auto gap-2" asChild>
                            <a href={policy.certificateUrl} target="_blank" rel="noopener noreferrer">
            ...
            <div className="rounded-lg border p-4 bg-muted/20">
                ...
                <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                    ...
                    <li>Digital proof of purchase and ID may be required during the claim process.</li>
                    <li>Screen damage and liquid damage are covered under this policy.</li>
                </ul>
            </div>
        </div>
    );
