...
- **TEMPORARY B0 CACHE-INVALIDATION EXCEPTION (owner-approved 2026-07-27; expires when B0 is retired in favor of queue-owned invalidation or on 2026-09-16, whichever comes first):** only the standalone VPS entrypoint `vps-workers/jobs/drain-cache-invalidations.mjs` may read `SUPABASE_SERVICE_ROLE_KEY`, `VERCEL_TOKEN`, `CLOUDFLARE_API_TOKEN`, and their non-secret project/zone configuration for this job. Its Supabase authority is limited to `claim_cache_invalidations` and `finish_cache_invalidation`; it may not query tables or call another RPC. It must hard-delete bounded production Vercel tag batches before a bounded Cloudflare hostname purge, persist only token-fenced outcomes, and never call a Baci/Vercel web route. Secrets, tenant credentials, and provider response bodies must never enter logs, event payloads, or source. No sibling worker, route, helper, importer, generic service-role operation, table query, or additional RPC inherits authorization. The backend key inherently bypasses RLS; these controls constrain the approved edge, not that capability. Remove the exception with B0 retirement, or obtain explicit reapproval by the fixed expiry even if queue-owned invalidation is not yet active.
