...
type DateFilter = 'today' | 'yesterday' | 'this_week' | 'last_week' | 'this_month' | 'last_month' | 'this_year' | 'last_year' | 'custom';
...
export default function AnalyticsScreen() {
    ...
    const getStartDate = (filter: DateFilter, year: number, customStart: Date, customEnd: Date): Date => {
        const now = new Date();
        let startDate: Date;
    ...
    const getEndDate = (filter: DateFilter, year: number, customStart: Date, customEnd: Date): Date => {
        const now = new Date();
        let endDate: Date;
        ...
        switch (filter) {
            ...
            case 'this_week':
                ...
                break;
            case 'last_week':
                const lastWeekEnd = new Date(now.setDate(now.getDate() - now.getDay() - 7 + 6));
    ...
    const { data: analytics, isLoading } = useQuery<AnalyticsData>({
        queryKey: ['analytics', merchant?.id, dateFilter, selectedYear, customStartDate, customEndDate],
        queryFn: async () => {
            if (!merchant?.id) throw new Error('No merchant found');

            const startDate = getStartDate(dateFilter, selectedYear, customStartDate, customEndDate);
            const endDate = getEndDate(dateFilter, selectedYear, customStartDate, customEndDate);

            // Fetch orders for the period
            const { data: orders } = await supabase
                .from('orders')
                .select('id, total, tax_amount, payment_status, payment_method, created_at')
            ...
            const vatDue = paidOrders.reduce((sum, o) => sum + (o.tax_amount || 0), 0);
            ...
            // Find best months
            const maxRevenueMonth = monthlyRevenue.indexOf(Math.max(...monthlyRevenue));
            const maxSalesMonth = monthlySales.indexOf(Math.max(...monthlySales));
            const monthlyAOV = monthlyRevenue.map((rev, i) => monthlySales[i] > 0 ? rev / monthlySales[i] : 0);
            ...
            const topPaymentMethod = topMethod
                ? { method: topMethod[0], percentage: sales > 0 ? (topMethod[1] / sales) * 100 : 0 }
        ...
        `)
            ...
            topProductData?.forEach(item => {
                const product = item.products as any; // Cast to access cost_price
            ...
            const topBrand = Object.values(brandRevenue).sort((a, b) => b.revenue - a.revenue)[0] || null;
            ...
            const customerPurchases: Record<string, { name: string; purchases: number }> = {};
            customerData?.forEach(order => {
                const customer = order.customers as { first_name: string; last_name: string };
                const name = `${customer.first_name} ${customer.last_name}`.trim() || 'Guest';
                if (!customerPurchases[name]) customerPurchases[name] = { name, purchases: 0 };
                customerPurchases[name].purchases += order.total || 0;
            });
            const topCustomer = Object.values(customerPurchases).sort((a, b) => b.purchases - a.purchases)[0] || null;
    ...
    const getFilterLabel = () => {
        return DATE_FILTERS.find(f => f.value === dateFilter)?.label || 'This year';
    };
    ...
    const MetricRow = ({
        ...
        >
            <View style={styles.metricLeft}>
                ...
                <Text style={[styles.metricValue, { color: ACCENT_COLOR }]}>{value}</Text>
                <Text style={[styles.metricSubtitle, { color: colors.textMuted }]}>{subtitle}</Text>
            </View>
            <View style={styles.metricRight}>
                {sparklineData && <Sparkline data={sparklineData} color={ACCENT_COLOR} />}
                {showCircle && (
    ...
    const TopItemRow = ({
        ...
        >
            <View style={styles.metricLeft}>
                <Text style={[styles.metricLabel, { color: colors.textSecondary }]}>{label}</Text>
                <Text style={[styles.topItemName, { color: ACCENT_COLOR }]} numberOfLines={1}>{name}</Text>
                <Text style={[styles.metricSubtitle, { color: colors.textMuted }]}>{subtitle}</Text>
            </View>
            <View style={styles.metricRight}>
                <Ionicons name="chevron-forward" size={20} color={colors.textMuted} />
        ...
        <>
            ...
            <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
                <View style={[styles.header, { borderBottomColor: colors.border, borderBottomWidth: 1 }]}>
                    ...
                    >
                        ...
                        <Text style={[styles.reportButtonText, { color: colors.text }]}>Report</Text>
                    </Pressable>
                </View>
                ...
                <View style={[styles.yearSelector, { borderColor: colors.border }]}>
                    ...
                    >
                        <Ionicons
                            name="chevron-back"
                            size={24}
                            color={dateFilter === 'this_year' ? colors.text : colors.textMuted}
                        />
                    </Pressable>
                    <Pressable
                        onPress={() => setShowDateFilter(true)}
                        style={[styles.yearCenter, { backgroundColor: colors.primaryLight }]}
                    >
                        <Ionicons name="calendar-outline" size={18} color={colors.primary} />
                        <Text style={[styles.yearText, { color: colors.text }]} numberOfLines={1}>
                            {getFilterLabel()}
                        </Text>
                    ...
                    <Pressable
                        onPress={() => {
                            if (dateFilter === 'this_year' && selectedYear < new Date().getFullYear()) {
                                setSelectedYear(y => y + 1);
                            }
                        }}
                        style={styles.yearArrow}
                    >
                        <Ionicons
                            ...
                            size={24}
                            color={dateFilter === 'this_year' && selectedYear < new Date().getFullYear() ? colors.text : colors.textMuted}
                        />
                    </Pressable>
                </View>
                    ...
                    <SafeAreaView style={[styles.modalContainer, { backgroundColor: colors.background }]}>
                            ...
                            <View style={styles.dateRangeSection}>
                                <View style={styles.dateRangeHeader}>
                                    <View style={styles.dateRangeHeaderLeft}>
                                        ...
                                        <Text style={[styles.dateRangeTitle, { color: colors.textSecondary }]}>Custom date range</Text>
                                    </View>
                                    {dateFilter === 'custom' && (
                                        <Pressable
                                ...
                                <Pressable
                                    style={[
                                        ...
                                        showDatePicker === 'start' && { backgroundColor: colors.primaryLight }
                                    ]}
                                    onPress={() => setShowDatePicker(showDatePicker === 'start' ? null : 'start')}
                                >
                                    <Text style={[styles.dateInputText, { color: dateFilter === 'custom' || showDatePicker === 'start' ? colors.text : colors.textMuted }]}>
                                        {dateFilter === 'custom' || showDatePicker === 'start' ? formatDate(customStartDate) : 'Start date'}
                                    </Text>
                                </Pressable>
                                <Pressable
                                    style={[
                                        styles.dateInput,
                                        { borderBottomColor: colors.border },
                                        showDatePicker === 'end' && { backgroundColor: colors.primaryLight }
                                    ]}
                                    onPress={() => setShowDatePicker(showDatePicker === 'end' ? null : 'end')}
                                >
                                    <Text style={[styles.dateInputText, { color: dateFilter === 'custom' || showDatePicker === 'end' ? colors.text : colors.textMuted }]}>
                                        {dateFilter === 'custom' || showDatePicker === 'end' ? formatDate(customEndDate) : 'End date'}
                                    </Text>
                                </Pressable>
                            </View>

                            <View style={[styles.presetGrid, { borderTopColor: colors.border }]}>
                                {DATE_FILTERS.map((filter, index) => (
                                    <Pressable
                                        key={filter.value}
                                        style={[
                                            styles.presetButton,
                                            index % 2 === 0 && { borderRightColor: colors.border, borderRightWidth: 1 },
                                            { borderBottomColor: colors.border },
                                        ]}
                                        onPress={() => {
                                    ...
                                    >
                                        <Text style={[
                                            styles.presetText,
                                            { color: dateFilter === filter.value ? ACCENT_COLOR : colors.text }
                                        ]}>
                                            {filter.label}
                                        </Text>
                                    </Pressable>
                        ...
                        {!showDatePicker && (
                            <View style={styles.modalFooter}>
                                <Pressable
                                    style={[styles.applyButton, { backgroundColor: dateFilter === 'custom' ? colors.primary : colors.cardHover }]}
                                    onPress={() => setShowDateFilter(false)}
                                >
                                    <Text style={[styles.applyButtonText, { color: dateFilter === 'custom' ? '#FFFFFF' : colors.textMuted }]}>
                        ...
                        {showDatePicker && (
                            <View style={[styles.bottomDatePicker, { backgroundColor: colors.card, borderTopColor: colors.border }]}>
                                <DateTimePicker
            ...
            {analytics && (
                <ReportSelectionModal
                    ...
                    startDate={getStartDate(dateFilter, selectedYear, customStartDate, customEndDate)}
                    endDate={getEndDate(dateFilter, selectedYear, customStartDate, customEndDate)}
                />
            )}
        </>
    );
