...
export default function StaffScreen() {
            ...
            Alert.alert(
                ...
                [
                    { text: 'Cancel', style: 'cancel' },
                    {
                        ...
                        onPress: async () => {
                            try {
                                await updateStaff.mutateAsync({ id: member.id, status: newStatus });
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'active':
                return { bg: colors.successLight, text: colors.success, label: 'Active', icon: 'checkmark-circle' };
            case 'pending':
                return { bg: colors.warningLight, text: colors.warning, label: 'Pending', icon: 'time' };
            case 'suspended':
                return { bg: colors.errorLight, text: colors.error, label: 'Suspended', icon: 'close-circle' };
            default:
                return { bg: colors.border, text: colors.textMuted, label: status, icon: 'help-circle' };
        }
    };
        ...
        return (
            ...
            >
                ...
                <View style={styles.staffInfo}>
                    <Text style={[styles.staffName, { color: colors.text }]}>{item.name || item.email.split('@')[0]}</Text>
                    <Text style={[styles.staffEmail, { color: colors.textSecondary }]}>{item.email}</Text>

                    <View style={styles.badgeRow}>
                        <View style={[styles.roleBadge, { backgroundColor: colors.goldLight }]}>
                            <Ionicons name="shield-outline" size={10} color={colors.gold} />
                            <Text style={[styles.roleBadgeText, { color: colors.gold }]}>{ROLE_LABELS[item.role]}</Text>
                        </View>

                        <View style={[styles.statusBadge, { backgroundColor: statusBadge.bg }]}>
                            <Ionicons name={statusBadge.icon as keyof typeof Ionicons.glyphMap} size={10} color={statusBadge.text} />
        ...
        <>
            ...
            <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['bottom']}>
                ...
                <FlatList
                    ...
                    refreshControl={
                        <RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={colors.gold} colors={[colors.gold]} />
                    }
                    ListEmptyComponent={
                        !isLoading ? (
                ...
                <Modal visible={inviteModalVisible} animationType="slide" transparent>
                    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
                        <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
                            ...
                            <View style={styles.inputGroup}>
                                <Text style={[styles.inputLabel, { color: colors.textSecondary }]}>Name (optional)</Text>
                                <TextInput
                                    style={[styles.input, { backgroundColor: colors.background, color: colors.text, borderColor: colors.border }]}
                                    ...
                                    {VALID_ROLES.map((role) => (
                                        <Pressable
                                            ...
                                            style={[
                                                ...
                                                selectedRole === role && { backgroundColor: colors.primary + '10' },
                                            ]}
                                            onPress={() => setSelectedRole(role)}
                                        >
                                            <Text
                                                style={[styles.roleOptionText, { color: selectedRole === role ? colors.primary : colors.text }]}
                                            >
                                                {ROLE_LABELS[role]}
                                            </Text>
                                        </Pressable>
                        ...
                        <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
                            ...
                            <ScrollView style={styles.roleList}>
                                {VALID_ROLES.map((role) => (
                                    <Pressable
                                        key={role}
                                        style={[styles.roleListItem, selectedRole === role && { backgroundColor: colors.primary + '10' }]}
                                        onPress={() => setSelectedRole(role)}
                                    >
                                        <View>
                                            <Text style={[styles.roleListLabel, { color: colors.text }]}>{ROLE_LABELS[role]}</Text>
                                            <Text style={[styles.roleListDesc, { color: colors.textMuted }]}>{ROLE_DESCRIPTIONS[role]}</Text>
                                        </View>
                                        {selectedRole === role && <Ionicons name="checkmark-circle" size={22} color={colors.primary} />}
                                    </Pressable>
                                ))}
                            </ScrollView>
                            ...
                            <Pressable
                                style={[styles.inviteButton, { backgroundColor: colors.primary }]}
                                onPress={() => selectedStaff && handleChangeRole(selectedStaff.id, selectedRole)}
