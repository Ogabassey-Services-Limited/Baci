...
export default function NewOrderScreen() {
    ...
    const [showFinancialModal, setShowFinancialModal] = useState<{ type: 'discount' | 'shipping' | 'tax', visible: boolean }>({ type: 'discount', visible: false });
    ...
    const { data: customersData } = useCustomers({ search: debouncedCustomerSearch, sortBy: 'alpha' });
    ...
    const [duplicateCustomer, setDuplicateCustomer] = useState<any>(null); // Found existing customer

    // Delivery Details
    const [sameAsCustomer, setSameAsCustomer] = useState(true);
    const [deliveryInfo, setDeliveryInfo] = useState({ name: '', phone: '', address: '', city: '', state: '' });
        ...
        try {
            ...
            const { data: order, error: orderError } = await supabase
                ...
                .insert({
                    ...
                    amount_paid: paymentStatus === 'partially_paid' ? (parseFloat(partialAmount) || 0) : (paymentStatus === 'paid' ? total : 0),
                    payment_method: (paymentStatus === 'paid' || paymentStatus === 'partially_paid') ? paymentMethod : null,
                    total: total,
                    subtotal: subtotal,
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['bottom']}>
            ...
            >
                <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={[styles.card, { backgroundColor: colors.card }]}>
                        {/* Date Row */}
                        <Pressable
                            style={[styles.listRow, { borderBottomColor: colors.border, borderBottomWidth: 1 }]}
                        ...
                        >
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                {!customer.name && <Text style={{ color: colors.primary, fontSize: 14, fontWeight: '500' }}>Select</Text>}
                    ...
                    <View style={[styles.card, { backgroundColor: colors.card, marginTop: 16 }]}>
                        <View style={[styles.listRow, { borderBottomColor: !sameAsCustomer ? colors.border : 'transparent', borderBottomWidth: !sameAsCustomer ? 1 : 0 }]}>
                            <View style={styles.iconBox}>
                                <Ionicons name="location-outline" size={20} color={colors.primary} />
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={[styles.listLabel, { color: colors.text }]}>Delivery Details</Text>
                                <Text style={[styles.listSubValue, { color: colors.textSecondary }]}>
                                    {sameAsCustomer ? 'Deliver to same person' : 'Deliver to different person'}
                                </Text>
                            </View>
                            <Switch
                                ...
                                thumbColor={Platform.OS === 'ios' ? '#fff' : sameAsCustomer ? '#fff' : '#f4f3f4'}
                            />
                        </View>
                        ...
                        {!sameAsCustomer && (
                            <View style={{ padding: 16, gap: 12 }}>
                                <View>
                                    <Text style={[styles.label, { color: colors.textSecondary, marginBottom: 4 }]}>Recipient Name</Text>
                                    <TextInput
                                        style={[styles.input, {
                                        ...
                                        onChangeText={(text) => setDeliveryInfo(prev => ({ ...prev, name: text }))}
                                    />
                                </View>

                                <View>
                                    <Text style={[styles.label, { color: colors.textSecondary, marginBottom: 4 }]}>Recipient Phone</Text>
                                    <TextInput
                                        style={[styles.input, {
                                            backgroundColor: colors.background,
                                            color: colors.text,
                                ...
                                <View style={{ zIndex: 5 }}>
                                    <Text style={[styles.label, { color: colors.textSecondary, marginBottom: 4 }]}>Delivery Address</Text>
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={{ flexDirection: 'row', gap: 12, marginTop: 8 }}>
                            <Pressable
                                style={[styles.actionBtn, { flex: 1, backgroundColor: colors.primary + '10', borderColor: colors.primary + '20' }]}
                                onPress={() => setShowCustomItemModal(true)}
                            >
                                <Ionicons name="flash-outline" size={18} color={colors.primary} />
                                <Text style={[styles.actionBtnText, { color: colors.primary }]}>Quick Add</Text>
                            </Pressable>
                            <Pressable
                                style={[styles.actionBtn, { flex: 1.2, backgroundColor: colors.backgroundLight, borderColor: colors.border }]}
                                onPress={() => {
                                    setProductSearch('');
                            ...
                            >
                                ...
                                <Text style={[styles.actionBtnText, { color: colors.textSecondary }]}>Search Catalog</Text>
                            </Pressable>
                        </View>
                        ...
                        <View style={{ marginTop: 12, gap: 8 }}>
                            {orderItems.length === 0 ? (
                                ...
                                orderItems.map((item) => (
                                    <View key={item.product_id || item.name} style={[styles.itemCard, { backgroundColor: colors.card }]}>
                                        {/* Thumbnail Rendering */}
                                        <View style={[styles.itemThumbnail, { backgroundColor: colors.backgroundLight }]}>
                                            {item.image_url ? (
                                        ...
                                        <View style={{ flex: 1, paddingHorizontal: 12 }}>
                                            ...
                                            {item.details ? (
                                                <Text style={{ color: colors.textMuted, fontSize: 12, marginBottom: 2 }}>{item.details}</Text>
                                            ) : null}
                                            <Text style={{ color: colors.textSecondary, fontSize: 13, fontWeight: '600', marginTop: 2 }}>{formatPrice(item.price)}</Text>
                                            ...
                                            <Pressable
                                                ...
                                                style={{ alignSelf: 'flex-start', marginTop: 6, borderBottomWidth: 1.5, borderBottomColor: colors.primary }}
                                            >
                                                <Text style={{ color: colors.primary, fontSize: 13, fontWeight: '700', paddingBottom: 1 }}>Edit</Text>
                                            </Pressable>
                                        </View>

                                        <View style={styles.itemActions}>
                                            <Pressable onPress={() => handleQuantityChange(item.product_id, -1)} style={[styles.qtyBtn, { backgroundColor: colors.backgroundLight }]}>
                                                <Ionicons name={item.quantity > 1 ? "remove" : "trash-outline"} size={16} color={item.quantity > 1 ? colors.text : colors.error} />
                                            </Pressable>
                                            <Text style={{ fontWeight: 'bold', color: colors.text, minWidth: 24, textAlign: 'center' }}>{item.quantity}</Text>
                                            <Pressable onPress={() => handleQuantityChange(item.product_id, 1)} style={[styles.qtyBtn, { backgroundColor: colors.backgroundLight }]}>
                        ...
                        <View style={styles.summaryContainer}>
                            ...
                            >
                                ...
                                <Text style={[styles.summaryValue, { color: discount > 0 ? colors.error : colors.textMuted }]}>
                            ...
                            >
                                ...
                                <Text style={[styles.summaryValue, { color: shippingFee > 0 ? colors.text : colors.textMuted }]}>
                            ...
                            >
                                ...
                                <Text style={[styles.summaryValue, { color: taxesToUse > 0 ? colors.text : colors.textMuted }]}>
                                    {formatPrice(taxesToUse)}
                                </Text>
                            </Pressable>

                            <View style={[styles.summaryRow, { marginTop: 4, paddingTop: 12, borderTopWidth: 1, borderTopColor: 'rgba(0,0,0,0.05)' }]}>
                                <Text style={[styles.summaryLabel, { color: colors.text, fontWeight: '700', fontSize: 16 }]}>Total Amount</Text>
                                <Text style={[styles.summaryValue, { color: colors.text, fontWeight: '800', fontSize: 18 }]}>{formatPrice(total)}</Text>
                            </View>
                        </View>
            ...
            <View style={[styles.footer, { backgroundColor: colors.card, borderTopColor: colors.border }]}>
                ...
                {['paid', 'partially_paid'].includes(paymentStatus) && (
                    <View style={{ marginBottom: 4 }}>
                        <Text style={{ fontSize: 12, fontWeight: '600', color: colors.textSecondary, marginBottom: 8, marginLeft: 4 }}>
                            Payment Method
                        </Text>
                            ...
                            {PAYMENT_METHODS.map((method) => (
                                <Pressable
                                    ...
                                    style={{
                                        ...
                                        paddingVertical: 10,
                                        borderRadius: 12,
                                        backgroundColor: paymentMethod === method.id ? colors.primary : colors.card,
                                        borderWidth: 1,
                                        borderColor: paymentMethod === method.id ? colors.primary : colors.border
                                    }}
                                >
                                    <Ionicons name={method.icon as any} size={18} color={paymentMethod === method.id ? '#fff' : colors.text} />
                                    <Text style={{ color: paymentMethod === method.id ? '#fff' : colors.text, fontWeight: '600', fontSize: 13 }}>
                                        {method.label}
                                    </Text>
                                </Pressable>
                            ))}
                ...
                <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
                    ...
                    >
                        ...
                        <View style={{ marginLeft: 12 }}>
                            <Text style={{ color: colors.primary, fontWeight: '600', fontSize: 16 }}>Create New Product</Text>
                            <Text style={{ color: colors.textSecondary, fontSize: 12 }}>Add a new item to inventory</Text>
                        </View>
                    </Pressable>
                    <FlatList
            ...
            <Modal visible={showCustomItemModal} transparent animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={[styles.dialog, { backgroundColor: colors.card }]}>
                        <Text style={[styles.dialogTitle, { color: colors.text }]}>Quick Add Item</Text>
                        <Text style={{ color: colors.textSecondary, fontSize: 13, marginBottom: 16, fontStyle: 'italic' }}>
                        ...
                        <TextInput
                            ...
                            value={(() => {
                                ...
                                const formattedInt = !isNaN(Number(rawInt)) && rawInt !== '' ? Number(rawInt).toLocaleString('en-US') : parts[0];
                            ...
                            <ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
                                ...
                                <View style={{ gap: 8 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', gap: 12 }}>
                                        <TextInput
                                            style={[styles.sheetInput, { backgroundColor: colors.inputBg, color: colors.text, flex: 1 }]}
                                ...
                                <View style={{ gap: 8, zIndex: 10 }}>
                                    ...
                                    <GooglePlacesAutocomplete
                                        ...
                                        query={{
                                            key: process.env.EXPO_PUBLIC_GOOGLE_MAPS_API_KEY || '',
                                            language: 'en',
                                            components: selectedCountryCode ? `country:${selectedCountryCode.toLowerCase()}` : undefined,
                                        }}
                                        textInputProps={{
                                            placeholderTextColor: colors.textMuted,
                                        }}
                                        ...
                                        styles={{
                                            ...
                                            description: { color: colors.text },
                                            poweredContainer: { backgroundColor: colors.card, borderBottomLeftRadius: 12, borderBottomRightRadius: 12 },
                                        }}
                ...
                <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: 24 }}>
                    <View style={{
                        ...
                        <View style={{ width: '100%', gap: 12 }}>
                            ...
                            >
                                <Text style={{ color: '#fff', fontSize: 17, fontWeight: '700' }}>View Order Details</Text>
                            </Pressable>
                            ...
                            >
                                <Text style={{ color: colors.text, fontSize: 17, fontWeight: '600' }}>Create New Sale</Text>
                            </Pressable>
                        </View>
                    </View>
                </View>
            ...
            >
                <Pressable
                    ...
                    onPress={() => setShowFinancialModal({ ...showFinancialModal, visible: false })}
                >
                    <Pressable style={{ backgroundColor: colors.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 48 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                            <Text style={{ fontSize: 20, fontWeight: '700', color: colors.text }}>
                                {showFinancialModal.type === 'discount' ? 'Add Discount' : showFinancialModal.type === 'shipping' ? 'Shipping Fee' : 'Taxes'}
                            </Text>
                            <Pressable onPress={() => setShowFinancialModal({ ...showFinancialModal, visible: false })}>
                                <Ionicons name="close" size={24} color={colors.text} />
                            </Pressable>
                        </View>

                        {showFinancialModal.type === 'tax' && merchant?.vat_registration_status === 'registered' && (
                            <View style={{
                                ...
                                <View style={{ flex: 1, marginRight: 12 }}>
                                    <Text style={{ fontSize: 16, fontWeight: '700', color: colors.text, marginBottom: 2 }}>Apply 7.5% VAT</Text>
                                    <Text style={{ fontSize: 13, color: colors.textSecondary }}>Automatic calculation based on subtotal</Text>
                                </View>
                                <Pressable
                        ...
                        {showFinancialModal.type === 'tax' && merchant?.vat_registration_status !== 'registered' && (
                            ...
                            >
                                ...
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 15, fontWeight: '700', color: colors.info }}>Setup VAT Module</Text>
                                    <Text style={{ fontSize: 13, color: colors.textSecondary }}>Enable automatic VAT at 7.5%</Text>
                                </View>
                        ...
                        <Text style={{ fontSize: 14, color: colors.textSecondary, marginBottom: 8 }}>
                            {showFinancialModal.type === 'tax' && isVatApplied ? 'Calculated VAT Amount' : 'Amount (NGN)'}
                        </Text>
                        <TextInput
                            style={{
                                backgroundColor: isVatApplied && showFinancialModal.type === 'tax' ? colors.backgroundLight : colors.inputBg,
                                color: isVatApplied && showFinancialModal.type === 'tax' ? colors.textMuted : colors.text,
                                padding: 18,
                                borderRadius: 16,
                                fontSize: 18,
                                fontWeight: '600',
                            ...
                            placeholderTextColor={colors.textMuted}
                            keyboardType="numeric"
                            autoFocus={!(showFinancialModal.type === 'tax' && isVatApplied)}
                            value={showFinancialModal.type === 'tax' && isVatApplied
                        ...
                        <View style={{ flexDirection: 'row', gap: 12 }}>
                            ...
                            <Pressable
                                ...
                                onPress={() => {
                                    ...
                                    if (showFinancialModal.type === 'shipping') setShippingFee(val);
                                    if (showFinancialModal.type === 'tax' && !isVatApplied) setTaxes(val);
                                    setShowFinancialModal({ ...showFinancialModal, visible: false });
                                }}
                            >
                ...
                >
                    <Pressable style={{ backgroundColor: colors.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 48 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                            <Text style={{ fontSize: 22, fontWeight: '800', color: colors.text }}>Edit item</Text>
                        ...
                        <View style={{
                            ...
                            <Text style={{ color: colors.primary, fontSize: 13, fontWeight: '600', lineHeight: 18 }}>
                                Edits made here apply only to this sale and won’t update your store’s inventory
                            </Text>
                        </View>
                        ...
                        <View style={{ gap: 20, marginBottom: 32 }}>
                            ...
                            <View style={{ flexDirection: 'row', gap: 12 }}>
                                <View style={{ flex: 1.5 }}>
                                    <Text style={{ fontSize: 12, color: colors.textMuted, marginBottom: 6, marginLeft: 4, fontWeight: '600' }}>Price</Text>
                                    <View style={{
                                        flexDirection: 'row',
                                        alignItems: 'center',
                                        ...
                                        <Text style={{ fontSize: 16, color: colors.text, marginRight: 4 }}>₦</Text>
                                        <TextInput
                                            style={{
                                                flex: 1,
                                                color: colors.text,
                                ...
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 12, color: colors.textMuted, marginBottom: 6, marginLeft: 4, fontWeight: '600' }}>Quantity</Text>
                                    <TextInput
                                        style={{
                                            backgroundColor: colors.backgroundLight,
                                            borderWidth: 1,
                        ...
                        <View style={{ flexDirection: 'row', gap: 12 }}>
                            <Pressable
                                ...
                                onPress={() => {
                                    if (editingItem) {
                                        setOrderItems(prev => prev.filter(i => i.product_id !== editingItem.product_id));
                                        setShowEditItemModal(false);
                                    }
                                }}
                            >
                                <Text style={{ color: colors.error, fontSize: 16, fontWeight: '800' }}>Remove</Text>
                            </Pressable>
                            ...
                            <Pressable
                                ...
                                onPress={() => {
                                    if (editingItem) {
                                        const finalPrice = parseFloat(editPriceValue.replace(/,/g, '')) || 0;
                                        const finalQty = parseInt(editQtyValue) || 1;
                                        setOrderItems(prev => prev.map(item =>
                                            item.product_id === editingItem.product_id
                                                ? { ...item, price: finalPrice, quantity: finalQty, details: editDetails }
                                                : item
                                        ));
                                    }
                                    setShowEditItemModal(false);
                            ...
                            >
                                <Text style={{ color: '#fff', fontSize: 16, fontWeight: '800' }}>Save</Text>
                            </Pressable>
                        </View>
                    </Pressable>
                </Pressable>
