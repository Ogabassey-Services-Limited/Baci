...
export default function StaffAccountsScreen() {
    ...
    const createAccountMutation = useMutation({
        mutationFn: async ({ name, staffId, branchId }: { name: string; staffId?: string | null; branchId?: string | null }) => {
                        ...
                        >
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.pickerScroll}>
                                <Pressable
                                    style={[
                                        styles.pickerOption,
                                        !selectedBranchId && { borderColor: colors.primary, backgroundColor: `${colors.primary}10` },
                                        { borderColor: !selectedBranchId ? colors.primary : colors.border }
                                    ]}
                                    onPress={() => setSelectedBranchId(null)}
                                >
                                    <Text style={{ color: !selectedBranchId ? colors.primary : colors.text, fontSize: 12 }}>No Branch</Text>
                                </Pressable>
                                {branches?.map((branch) => (
                                    <Pressable
                                        key={branch.id}
                                        style={[
                                            styles.pickerOption,
                                            selectedBranchId === branch.id && { borderColor: colors.primary, backgroundColor: `${colors.primary}10` },
                                            { borderColor: selectedBranchId === branch.id ? colors.primary : colors.border }
                                        ]}
                                        onPress={() => setSelectedBranchId(branch.id)}
                                        accessibilityRole="radio"
                                        accessibilityState={{ selected: selectedBranchId === branch.id }}
                                        accessibilityLabel={`Branch: ${branch.name}`}
                                    >
                                        <Text style={{ color: selectedBranchId === branch.id ? colors.primary : colors.text, fontSize: 12 }}>{branch.name}</Text>
                                    </Pressable>
                                ))}
                            </ScrollView>
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.pickerScroll}>
                                <Pressable
                                    style={[
                                        styles.pickerOption,
                                        !selectedStaffId && { borderColor: colors.primary, backgroundColor: `${colors.primary}10` },
                                ...
                                >
                                    <Text style={{ color: !selectedStaffId ? colors.primary : colors.text, fontSize: 12 }}>No Staff</Text>
                                </Pressable>
                                {staffMembers?.map((staff) => (
                                    <Pressable
                                        ...
                                        style={[
                                            styles.pickerOption,
                                            selectedStaffId === staff.id && { borderColor: colors.primary, backgroundColor: `${colors.primary}10` },
                                            { borderColor: selectedStaffId === staff.id ? colors.primary : colors.border }
                                        ]}
                                        onPress={() => setSelectedStaffId(staff.id)}
                                        accessibilityRole="radio"
                                        accessibilityState={{ selected: selectedStaffId === staff.id }}
                                        accessibilityLabel={`Staff: ${staff.name || staff.email}`}
                                    >
                                        <Text style={{ color: selectedStaffId === staff.id ? colors.primary : colors.text, fontSize: 12 }}>{staff.name || staff.email}</Text>
                                    </Pressable>
                                ))}
                            </ScrollView>
