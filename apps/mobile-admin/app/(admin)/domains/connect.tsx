...
export default function ConnectDomainScreen() {
    ...
    const [verificationInfo, setVerificationInfo] = useState<{ domain: string; token: string } | null>(null);
    ...
    if (verificationInfo) {
        return (
            <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.content}>
                <View style={[styles.successHeader, { backgroundColor: colors.success + '15' }]}>
                    <Ionicons name="checkmark-circle" size={48} color={colors.success} />
                    <Text style={[styles.title, { color: colors.text }]}>Domain Added!</Text>
                    <Text style={[styles.subtitle, { color: colors.textSecondary, textAlign: 'center' }]}>
                        To verify ownership of {verificationInfo.domain}, please add the following TXT record to your DNS settings.
                    </Text>
                </View>

                <View style={[styles.recordCard, { backgroundColor: colors.card, borderColor: colors.border }, shadows.sm]}>
                    ...
                    <View style={styles.recordRow}>
                        ...
                        >
                            <Text style={[styles.tokenText, { color: colors.text }]} numberOfLines={1} ellipsizeMode="middle">
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
            <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.content}>
                <Text style={[styles.title, { color: colors.text }]}>Connect Existing Domain</Text>
                <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
                    Enter the domain name you own (e.g. example.com) to connect it to your store.
                </Text>

                <View style={{ marginTop: SPACING.xl }}>
                    <Text style={[styles.inputLabel, { color: colors.text }]}>Domain Name</Text>
                    <TextInput
