...
export default function DomainsDashboard() {
    ...
    const { user } = useAuth();
    const { merchant, storeUrl, primaryDomain: merchantPrimaryDomain } = useMerchant();
    ...
    const handleSetPrimary = async (domain: Domain) => {
        if (domain.status !== 'active') {
            Alert.alert('Action Failed', 'Only active domains can be set as primary.');
            return;
        }
                ...
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        ...
                        try {
                            ...
                            const response = await fetch(`${API_URL}/domains/${encodeURIComponent(domain.domain)}`, {
                                method: 'DELETE',
                                headers: {
                            ...
                            Alert.alert('Error', error instanceof Error ? error.message : 'Failed to delete domain');
                        } finally {
                            setActionLoading(false);
                        }
                    }
    ...
    const handleVerify = async (domain: Domain) => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/domains/${encodeURIComponent(domain.domain)}/verify`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${session.access_token}`,
                },
            ...
            if (response.ok && data.success) {
                ...
                Alert.alert('Verification Failed', data.error || 'Could not verify domain.');
            }
        } catch (error) {
    ...
    const handleOptionAction = (action: 'visit' | 'verify' | 'set_primary' | 'delete', domain: Domain) => {
    ...
    const renderDomainItem = (domain: Domain) => (
        <View key={domain.id} style={[styles.domainCard, { backgroundColor: colors.card }, shadows.sm]}>
            <View style={styles.domainInfo}>
                <View style={styles.domainHeader}>
                    <View style={[
                        styles.iconContainer,
                        { backgroundColor: domain.status === 'active' ? colors.successLight : domain.status === 'failed' ? colors.errorLight : colors.warningLight }
                    ]}>
                        <Ionicons name="globe-outline" size={24} color={
                            domain.status === 'active' ? colors.success :
                                domain.status === 'failed' ? colors.error : colors.warning
                        } />
                    </View>
                    ...
                    <View>
                        ...
                        <View style={styles.badgesRow}>
                            ...
                            { backgroundColor: domain.status === 'active' ? colors.successLight : domain.status === 'failed' ? colors.errorLight : colors.warningLight }
                                ...
                                { color: domain.status === 'active' ? colors.success : domain.status === 'failed' ? colors.error : colors.warning }
                                ]}>
                ...
                >
                    ...
                    <View style={[styles.storeCard, { backgroundColor: colors.card }, shadows.sm]}>
                        ...
                        <View style={styles.storeInfo}>
                            <Text style={[styles.storeLabel, { color: colors.text }]}>Baci Store Link</Text>
                            <Text style={[styles.storeUrl, { color: colors.textSecondary }]}>
                                {merchantPrimaryDomain?.domain || (merchant?.slug ? `${merchant.slug}.usebaci.com` : 'Loading...')}
                            </Text>
                        </View>
                    </View>

                    <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>CUSTOM DOMAINS</Text>

                    {loading ? (
                        <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Loading domains...</Text>
                    ) : domains.filter(d => d.domain_type !== 'subdomain').length === 0 ? (
                        <View style={styles.promoContainer}>
                            <View style={[styles.promoCard, { backgroundColor: colors.card }]}>
                                <View style={[styles.promoIconCircle, { backgroundColor: colors.primary + '15' }]}>
                                    <Ionicons name="rocket" size={32} color={colors.primary} />
                                </View>
                                <Text style={[styles.promoTitle, { color: colors.text }]}>Claim your brand online</Text>
                                <Text style={[styles.promoText, { color: colors.textSecondary }]}>
                                    Merchants with custom domains (e.g. .com) establish more trust and look more professional to customers.
                                </Text>
                                ...
                                >
                                    <Text style={[styles.connectLinkText, { color: colors.primary }]}>I already own a domain</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    ) : (
                        <View style={styles.domainsList}>
                            {domains.filter(d => d.domain_type !== 'subdomain').map(renderDomainItem)}
                        </View>
                    )}
