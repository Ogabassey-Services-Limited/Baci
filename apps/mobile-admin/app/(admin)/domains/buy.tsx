...
import { DOMAIN_PRICING, calculateDomainPrice } from '@/constants/domain-pricing';
...
export default function BuyDomainScreen() {
    ...
    const handleSearch = async () => {
        if (!query.includes('.')) {
            // Simple validation
            Alert.alert('Invalid Domain', 'Please enter a valid domain (e.g. mystore.com)');
            return;
        }
    ...
    const handleBuy = async (domain: string, price: number) => {
    ...
    const renderItem = ({ item }: { item: SearchResult }) => (
        <View style={[styles.resultCard, { backgroundColor: colors.card, borderColor: colors.border }, shadows.sm]}>
            <View style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <Text style={[styles.domainName, { color: colors.text }]}>{item.domain}</Text>
                    {item.popular && (
                        <View style={{ backgroundColor: colors.primary, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 }}>
                            <Text style={{ color: 'white', fontSize: 10, fontWeight: 'bold' }}>POPULAR</Text>
                        </View>
                    )}
                </View>

                {item.available ? (
                    <Text style={{ color: colors.success, fontSize: 13, marginTop: 4, fontFamily: TYPOGRAPHY.fontFamily.medium }}>Available</Text>
                ) : (
                    <Text style={{ color: colors.textSecondary, fontSize: 13, marginTop: 4 }}>Unavailable</Text>
                )}
            </View>
            ...
            <View style={{ alignItems: 'flex-end' }}>
                ...
                {item.available && (
                    <Pressable
                        style={[styles.buyButton, { backgroundColor: colors.primary, opacity: purchasing ? 0.5 : 1 }]}
            ...
            <View style={[styles.container, { backgroundColor: colors.background }]}>
                ...
                {loading ? (
                    <View style={{ padding: 40 }}>
                        ...
                        <Text style={{ textAlign: 'center', marginTop: 20, color: colors.textSecondary }}>Checking availability...</Text>
                    </View>
                ) : (
                    <FlatList
                        data={results}
                        ...
                        ListEmptyComponent={
                            results.length === 0 && query.length > 0 ? (
                                <Text style={{ textAlign: 'center', marginTop: 40, color: colors.textSecondary }}>No results found.</Text>
                            ) : null
                        }
                    />
                )}
