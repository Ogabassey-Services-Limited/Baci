...
export default function StoreSettingsScreen() {
    ...
    useEffect(() => {
        if (merchant) {
            ...
            const defaultCurrencyForCountry = COUNTRIES.find(c => c.name === initialCountry)?.currency;
            setCurrency(merchant.payout_currency || defaultCurrencyForCountry || COUNTRIES[0].currency);
        ...
        <>
            ...
            <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['bottom']}>
                ...
                <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
                    ...
                    <View style={[styles.card, { backgroundColor: colors.card }, shadows.sm]}>
                        <Pressable
                            style={[styles.navRow, { borderBottomColor: colors.border, borderBottomWidth: 1 }]}
                            onPress={() => router.push('/social-media')}
                        >
                            <View style={styles.navRowLeft}>
                                <Ionicons name="share-social-outline" size={20} color={colors.primary} />
                                <Text style={[styles.navRowText, { color: colors.text }]}>Social Media Connections</Text>
                            </View>
                        ...
                        >
                            <View style={styles.navRowLeft}>
                                ...
                                <Text style={[styles.navRowText, { color: colors.text }]}>Analytics & Tracking</Text>
                            </View>
                            <Ionicons name="chevron-forward" size={20} color={colors.textSecondary} />
                        </Pressable>
                    </View>
                    ...
                    {/* Region Settings */}
                    <View style={[styles.card, { backgroundColor: colors.card }, shadows.sm]}>
                        <Text style={[styles.label, { color: colors.textSecondary }]}>Region Settings</Text>
                        <View style={{ gap: SPACING.md }}>
                            {/* Country Select */}
                            <View>
                                <Text style={[styles.sublabel, { color: colors.textSecondary, marginBottom: 4, fontSize: 12 }]}>Country</Text>
                                <Pressable
                                    style={[styles.readOnlyInput, { backgroundColor: colors.background, borderColor: colors.border, opacity: 1 }]}
                                    onPress={() => setShowCountryModal(true)}
                                >
                                    <Text style={{ color: colors.text }}>
                                        {COUNTRIES.find(c => c.code === country || c.name === country)?.name || country}
                                    </Text>
                            ...
                            <View>
                                <Text style={[styles.sublabel, { color: colors.textSecondary, marginBottom: 4, fontSize: 12 }]}>Currency</Text>
                                <View style={[styles.readOnlyInput, { backgroundColor: colors.background, borderColor: colors.border, opacity: 0.6 }]}>
                    ...
                    <View style={[styles.card, { backgroundColor: colors.card }, shadows.sm]}>
                        ...
                        <View style={[styles.urlContainer, { backgroundColor: colors.cardHover, flexDirection: 'row', alignItems: 'center' }]}>
                            <TextInput
                                style={[styles.urlText, { color: colors.text, flex: 1, padding: 0 }]}
                                value={slug}
                                ...
                                autoCapitalize="none"
                            />
                            <Text style={[styles.urlText, { color: colors.textSecondary }]}>.usebaci.com</Text>
                        </View>
                ...
                >
                    <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
                        <View style={[styles.modalHeader, { borderBottomColor: colors.border }]}>
                            ...
                            <Pressable onPress={() => setShowCountryModal(false)} style={styles.closeButton}>
                            ...
                            {filteredCountries.map((item) => (
                                <Pressable
                                    ...
                                    style={[
                                        styles.countryItem,
                                        {
                                            backgroundColor: country === item.name ? colors.primaryLight : colors.card,
                                ...
                                >
                                    <View>
                                        <Text style={[styles.countryName, { color: colors.text, fontWeight: country === item.name ? 'bold' : 'normal' }]}>
                                            {item.name}
                                        </Text>
                ...
                >
                    <View style={styles.statusModalOverlay}>
                        <View style={[styles.statusModalCard, { backgroundColor: colors.surface }]}>
                            <View style={[styles.iconContainer, { backgroundColor: statusModal.type === 'success' ? '#E8F5E9' : '#FFEBEE' }]}>
                                <Ionicons
                            ...
                            <Text style={[styles.statusMessage, { color: colors.textSecondary }]}>{statusModal.message}</Text>
                            <Pressable
                                style={[styles.statusButton, { backgroundColor: statusModal.type === 'success' ? colors.primary : '#C62828' }]}
...
const styles = StyleSheet.create({
    ...
    cardTitle: { fontSize: TYPOGRAPHY.size.lg, fontFamily: TYPOGRAPHY.fontFamily.semiBold, marginBottom: SPACING.md },
    ...
    modalTitle: { fontSize: TYPOGRAPHY.size.lg, fontFamily: TYPOGRAPHY.fontFamily.bold },
    ...
    countryName: { fontSize: TYPOGRAPHY.size.md, fontFamily: TYPOGRAPHY.fontFamily.semiBold },
