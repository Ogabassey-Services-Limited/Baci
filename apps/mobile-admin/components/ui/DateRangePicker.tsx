...
export default function DateRangePicker({ visible, onClose, onSelect, currentFilter }: DateRangePickerProps) {
        ...
        >
            <Pressable style={styles.overlay} onPress={onClose}>
                ...
                >
                    ...
                    {/* Mode Switcher */}
                    <View style={[styles.modeSwitcher, { backgroundColor: colors.background }]}>
                        <Pressable
                            style={[
                                styles.modeButton,
                                mode === 'presets' && { backgroundColor: colors.card, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 2, elevation: 1 }
                            ]}
                        ...
                        <Pressable
                            style={[
                                styles.modeButton,
                                mode === 'calendar' && { backgroundColor: colors.card, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 2, elevation: 1 }
                            ]}
                        ...
                        {mode === 'presets' ? (
                                ...
                                {PRESETS.map((preset) => (
                                    <Pressable
                                        ...
                                        style={[
                                            ...
                                            activePreset === preset && { borderColor: colors.primary, backgroundColor: colors.primaryLight }
                                        ]}
                                        onPress={() => {
                                            onSelect(preset);
                                            onClose();
                            ...
                            <View style={styles.calendarContainer}>
                                ...
                                <View style={styles.monthNav}>
                                    <Pressable onPress={() => setViewDate(subMonths(viewDate, 1))} style={styles.navButton}>
                                        <Ionicons name="chevron-back" size={20} color={colors.text} />
                                    </Pressable>
                                    <Text style={[styles.monthTitle, { color: colors.text }]}>
                                        {format(viewDate, 'MMMM yyyy')}
                                    </Text>
                                    <Pressable onPress={() => setViewDate(addMonths(viewDate, 1))} style={styles.navButton}>
                                ...
                                <View style={styles.daysGrid}>
                                    {days.map((day, index) => {
                                        ...
                                        // Styling logic
                                        let bg = 'transparent';
                                        let textColor = isCurrentMonth ? colors.text : colors.textMuted;
                                        ...
                                        return (
                                            <Pressable
                                                ...
                                                style={[
                                                    ...
                                                    { backgroundColor: bg },
                                                    // Start/End rounding
                                                    selection.start && isSameDay(day, selection.start) && { borderTopLeftRadius: 8, borderBottomLeftRadius: 8 },
                                                    selection.end && isSameDay(day, selection.end) && { borderTopRightRadius: 8, borderBottomRightRadius: 8 },
                                ...
                                <View style={styles.footer}>
                                    ...
                                    <Pressable
                                        style={[
                                            styles.applyButton,
                                            { backgroundColor: colors.primary, opacity: (selection.start && selection.end) ? 1 : 0.5 }
                                        ]}
