...
function createFixture(files: Record<string, string>) {
  const root = path.join(
    os.tmpdir(),
    `admin-module-size-${Date.now()}-${Math.random().toString(36).slice(2)}`
  );
  tempDirs.add(root);
...
function createBaseline(entries: Array<{ lineCount: number; path: string }>) {
  return JSON.stringify({
    ...
    roots: ['components', 'hooks', 'lib', 'services', 'stores', 'utils'],
  });
}
...
describe('check-module-size', () => {
  ...
  it('fails when a new module exceeds the max line budget', () => {
    const root = createFixture({
      'hooks/new-module.ts': createLines(301),
      'config/module-size-baseline.json': createBaseline([]),
    });
    ...
    expect(result.stderr).toContain('hooks/new-module.ts: 301 lines');
  });

  it('fails when a baselined module grows beyond its recorded line count', () => {
    const root = createFixture({
      'services/push.ts': createLines(321),
      'config/module-size-baseline.json': createBaseline([
        { path: 'services/push.ts', lineCount: 320 },
      ]),
    });
    ...
    expect(result.stderr).toContain('services/push.ts: 321 lines > 320 baseline');
  });

  it('ignores test files in monitored roots', () => {
    const root = createFixture({
      'utils/ignored.test.ts': createLines(800),
      'config/module-size-baseline.json': createBaseline([]),
    });
  ...
  it('fails when baseline entry file is missing', () => {
    const root = createFixture({
      'hooks/existing.ts': createLines(20),
      'config/module-size-baseline.json': createBaseline([
        { path: 'hooks/deleted.ts', lineCount: 320 },
      ]),
    });
    ...
    expect(result.stderr).toContain('hooks/deleted.ts: file is missing');
  });
});
