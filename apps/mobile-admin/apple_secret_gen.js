...
async function generate() {
  ...
  if (TEAM_ID.includes('YOUR_') || KEY_ID.includes('YOUR_')) {
    console.error(
      '❌ ERROR: You must open this file and fill in TEAM_ID and KEY_ID first!'
    );
    process.exit(1);
  }
  ...
  if (!fs.existsSync(PRIVATE_KEY_PATH)) {
    console.error(
      `❌ ERROR: Could not find the .p8 file at: ${PRIVATE_KEY_PATH}`
    );
    console.error(
      "   Please make sure the file is in this folder and update 'PRIVATE_KEY_PATH' in this script."
    );
    process.exit(1);
  }
