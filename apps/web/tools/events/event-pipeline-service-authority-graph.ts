...
function serviceConstructionEdges(path: string, source: string, sources: ReadonlyMap<string, string>): AuthorityEdge[] {
  if (allowedFactoryImporter(path, 'service') || !source.includes('createServiceClient')) return []; const referenced = moduleGraph.moduleReferences(path, source).some((specifier) => factoryReference(path, specifier, sources)?.kind === 'service'); if (!referenced) return [];
  ...
  function seedMember(key: ts.Node, name: string, expression: ts.Expression) { markKnown(key, name); const valueForms = factoryForms(expression); markMember(factoryMembers, key, name, [...valueForms].map(() => 'container')); if (valueForms.size === 0 && isForwarded(expression)) markMember(factoryMembers, key, name, ['container']); markMember(callableMembers, key, name, callable(expression)); }
