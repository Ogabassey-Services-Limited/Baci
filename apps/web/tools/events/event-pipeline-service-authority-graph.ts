...
function serviceConstructionEdges(path: string, source: string, sources: ReadonlyMap<string, string>): AuthorityEdge[] {
  if (allowedFactoryImporter(path, 'service')) return []; const referenced = moduleGraph.moduleReferences(path, source).some((specifier) => factoryReference(path, specifier, sources)?.kind === 'service'); if (!referenced) return [];
  type Fn = ts.ArrowFunction | ts.FunctionDeclaration | ts.FunctionExpression | ts.MethodDeclaration;
  const file = parseEventPipelineTypeScriptSource(path, source); const lexical = createEventPipelineLexicalFlow(file, new Map([['createServiceClient', file]])); let changed = false;
  ...
  const objectAssignCall = (expression: ts.Expression): expression is ts.CallExpression => ts.isCallExpression(expression) && objectAssignReference(expression.expression);
  function serviceNamespace(expression: ts.Expression): boolean { const value = unwrap(expression); if (ts.isIdentifier(value)) return lexical.definitionKeys(value).some((key) => namespaces.has(key)); if (ts.isConditionalExpression(value)) return serviceNamespace(value.whenTrue) || serviceNamespace(value.whenFalse); if (ts.isBinaryExpression(value)) return value.operatorToken.kind === ts.SyntaxKind.CommaToken || value.operatorToken.kind === ts.SyntaxKind.EqualsToken ? serviceNamespace(value.right) : branching(value.operatorToken) && (serviceNamespace(value.left) || serviceNamespace(value.right)); return ts.isCallExpression(value) && value.arguments.length > 0 && ts.isStringLiteralLike(value.arguments[0]) && (value.expression.kind === ts.SyntaxKind.ImportKeyword || ts.isIdentifier(value.expression) && value.expression.text === 'require' && !lexical.bindingOf(value.expression)) && factoryReference(path, value.arguments[0].text, sources)?.kind === 'service'; }
  ...
  function seedValue(key: ts.Node | undefined, expression: ts.Expression) { const valueForms = factoryForms(expression); mark(forms, key, [...valueForms].map((form) => form === 'direct' ? 'forwarded' : form)); const structured = seedMembers(key, expression); if (valueForms.size === 0 && isForwarded(expression) && !structured) flag(containers, key); mark(callableValues, key, callable(expression)); if (serviceNamespace(expression)) flag(namespaces, key); if (objectAssignReference(expression)) flag(assigners, key); }
  function bindPattern(name: ts.BindingName, expression: ts.Expression) { if (ts.isIdentifier(name)) { seedValue(lexical.bindingOf(name), expression); return; } const value = unwrap(expression); for (const [index, element] of name.elements.entries()) { if (ts.isOmittedExpression(element)) continue; let part: ts.Expression | undefined; if (ts.isObjectBindingPattern(name) && ts.isObjectLiteralExpression(value)) { const wanted = element.propertyName && (ts.isIdentifier(element.propertyName) || ts.isStringLiteralLike(element.propertyName)) ? element.propertyName.text : ts.isIdentifier(element.name) ? element.name.text : undefined; const property = value.properties.find((candidate) => !ts.isSpreadAssignment(candidate) && propertyKey(candidate.name) === wanted); if (property) part = ts.isPropertyAssignment(property) ? property.initializer : ts.isShorthandPropertyAssignment(property) ? property.name : undefined; } else if (ts.isArrayBindingPattern(name) && ts.isArrayLiteralExpression(value)) { const item = value.elements[index]; if (item && !ts.isOmittedExpression(item)) part = ts.isSpreadElement(item) ? item.expression : item; } part ??= element.initializer; if (part) bindPattern(element.name, part); else if (isForwarded(expression)) addFactories(element.name, 'forwarded'); } }
  ...
  function seedCall(call: ts.CallExpression) { const arguments_ = effectiveArguments(call); for (const fn of callable(call.expression)) for (const [index, parameter] of fn.parameters.entries()) { if (parameter.dotDotDotToken) { const rest = arguments_.slice(index); if (rest.some((argument) => isForwarded(argument) || factoryForms(argument).size > 0)) for (const key of lexical.bindingKeys(parameter.name)) flag(containers, key); continue; } const argument = arguments_[index] ?? parameter.initializer; if (argument) bindPattern(parameter.name, argument); } }
  function flow(node: ts.Node) { if (ts.isFunctionDeclaration(node) && node.name) mark(callableValues, lexical.bindingOf(node.name), [node]); if (ts.isFunctionExpression(node) && node.name) mark(callableValues, lexical.bindingOf(node.name), [node]);
    if (ts.isImportDeclaration(node) && !node.importClause?.isTypeOnly && ts.isStringLiteral(node.moduleSpecifier) && factoryReference(path, node.moduleSpecifier.text, sources)?.kind === 'service') { const named = node.importClause?.namedBindings; if (named && ts.isNamedImports(named)) for (const element of named.elements) if (!element.isTypeOnly && (element.propertyName?.text ?? element.name.text) === 'createServiceClient') mark(forms, lexical.bindingOf(element.name), ['direct']); if (named && ts.isNamespaceImport(named)) flag(namespaces, lexical.bindingOf(named.name)); }
  ...
  const signature = (expression: ts.Expression, form: string): string => { const value = unwrap(expression); if (ts.isPropertyAccessExpression(value) || ts.isElementAccessExpression(value)) { const name = memberName(value); if (name === 'call' || name === 'apply') return name; if (name === 'createServiceClient') return 'direct-member'; return ts.isElementAccessExpression(value) ? 'container-element' : 'container-member'; } return ts.isCallExpression(value) || ts.isNewExpression(value) ? 'returned' : form; };
  ...
  const counts = new Map<string, number>(); const emit = (node: ts.CallExpression | ts.NewExpression, form: string) => { const semantic = `${lexical.semanticContext(node)}|${ts.isNewExpression(node) ? 'new-' : ''}${form === 'helper-invoke' ? form : signature(node.expression, form)}`; const occurrence = counts.get(semantic) ?? 0; counts.set(semantic, occurrence + 1); edges.push({ key: JSON.stringify(['construction', path, semantic, occurrence]), message: `${path}: unauthorized service factory importer` }); };
  ...
  const readCounts = new Map<string, number>(); const readUse = (node: ts.Expression): ts.Expression => { let value = node; while ((ts.isParenthesizedExpression(value.parent) || ts.isAsExpression(value.parent) || ts.isTypeAssertionExpression(value.parent) || ts.isNonNullExpression(value.parent) || ts.isAwaitExpression(value.parent) || ts.isSatisfiesExpression(value.parent)) && value.parent.expression === value) value = value.parent; return value; }; const capabilityKind = (node: ts.Expression): string | undefined => serviceNamespace(node) ? 'namespace' : factoryForms(node).size > 0 ? 'factory' : isForwarded(node) ? 'container' : undefined; const readRole = (node: ts.Expression): string => { const value = readUse(node); const parent = value.parent; if (ts.isCallExpression(parent)) return parent.expression === value ? 'invoke' : `argument:${ts.isIdentifier(parent.expression) ? parent.expression.text : memberName(parent.expression) ?? 'call'}`; if ((ts.isPropertyAccessExpression(parent) || ts.isElementAccessExpression(parent)) && parent.expression === value) return `member:${memberName(parent) ?? 'computed'}`; if (ts.isArrayLiteralExpression(parent)) return 'array-value'; if (ts.isShorthandPropertyAssignment(parent)) return `object:${parent.name.text}`; if (ts.isPropertyAssignment(parent) && parent.initializer === value) return `object:${propertyKey(parent.name) ?? '*'}`; if ((ts.isVariableDeclaration(parent) || ts.isParameter(parent) || ts.isBindingElement(parent)) && parent.initializer === value) return ts.isBindingElement(parent) ? 'default' : 'initializer'; if (ts.isBinaryExpression(parent) && parent.right === value) return 'assignment'; if (ts.isReturnStatement(parent)) return 'return'; return ts.SyntaxKind[parent.kind]; };
  const runtimeRead = (node: ts.Expression): boolean => { if (ts.findAncestor(node, (ancestor) => ts.isImportDeclaration(ancestor) || ts.isExportDeclaration(ancestor) || ts.isTypeNode(ancestor))) return false; const value = readUse(node); const parent = value.parent; if ((ts.isPropertyAccessExpression(parent) || ts.isElementAccessExpression(parent)) && parent.expression === value && capabilityKind(parent)) return false; if (ts.isIdentifier(node)) { if (lexical.bindingOf(node) === node) return false; const directParent = node.parent; if ((ts.isPropertyAccessExpression(directParent) || ts.isElementAccessExpression(directParent)) && directParent.expression === node && memberKnown(rootKeys(node), memberName(directParent)) && factoryForms(directParent).size === 0) return false; if (ts.isPropertyAccessExpression(directParent) && directParent.name === node || ts.isPropertyAssignment(directParent) && directParent.name === node || ts.isMethodDeclaration(directParent) && directParent.name === node || ts.isBindingElement(directParent) && directParent.propertyName === node) return false; } return !(ts.isBinaryExpression(parent) && parent.left === value && parent.operatorToken.kind === ts.SyntaxKind.EqualsToken); }; function collectReads(node: ts.Node) { if ((ts.isIdentifier(node) || ts.isCallExpression(node) || ts.isPropertyAccessExpression(node) || ts.isElementAccessExpression(node)) && runtimeRead(node)) { const kind = capabilityKind(node); if (kind) { const semantic = `${lexical.semanticContext(node)}|${kind}:${readRole(node)}`; const occurrence = readCounts.get(semantic) ?? 0; readCounts.set(semantic, occurrence + 1); edges.push({ key: JSON.stringify(['capability', path, semantic, occurrence]), message: `${path}: unauthorized service factory importer` }); } } ts.forEachChild(node, collectReads); } collectReads(file); collect(file); return edges;
}
