...
const MERCHANT_ANON_COLUMNS =
  'about_page bank_account_name bank_account_number bank_code bank_name brand_colors business_address business_name business_type cac_rc_number country created_at email email_domain email_domain_verified email_logo_url email_sender_name endpoint_id endpoint_scheme_id facebook_pixel_id faq_items favicon_apple_touch_url favicon_png_192_url favicon_png_32_url favicon_svg_url favicon_uploaded_at feature_settings firs_business_id firs_service_id gmc_variants_enabled google_analytics_id hero_image_ids hero_images_generated_at hero_images_regeneration_count hero_slides id is_published kyc_status legal_entity_name lga_code logo_url mobile_hero_slides multi_currency_enabled offline_conversions_enabled order_prefix pages payout_currency phone plan_expires_at plan_started_at plan_tier premium_features published_at published_config registered_address rider_phone_number self_fulfillment_enabled signup_source site_description site_tagline site_title slug snapchat_pixel_id social_media state_code support_email support_phone tax_exempt tax_identification_number template_id tiktok_pixel_id trust_profile twitter_pixel_id updated_at user_id vat_rate vat_registration_status'.split(
    ' '
  );

describe('supabaseHistoryEffectScope', () => {
  it('binds the exact P0 event relations and internal functions', () => {
    expect(supabaseHistoryEffectScope.scopeVersion).toBe('baci-p0-effects-v3');
    expect(supabaseHistoryEffectScope.eventPipeline.tables).toEqual([
    ...
    expect(
      ...
      ).map(([qualifiedName, identityArguments]) => [
        qualifiedName.replace('eventing.', ''),
        identityArguments,
      ])
    ).toEqual([
      ['capture_order_domain_event_v1', ''],
      ['capture_product_domain_event_v1', ''],
      ['capture_transaction_domain_event_v1', ''],
      [
      ...
      ['is_event_pipeline_operator_v1', ''],
      [
        'resolve_domain_event_duplicate_v1',
  ...
  it('binds all nineteen public event RPC overloads exactly', () => {
    expect(
      Object.entries(supabaseHistoryEffectScope.eventPipeline.publicRpcs).map(
        ([qualifiedName, identityArguments]) => [
  ...
  it('binds external, cancellation, and duplicate-history contracts', () => {
    expect(supabaseHistoryEffectScope.eventPipeline.externalContracts).toEqual({
    ...
    expect(supabaseHistoryEffectScope.fulfillmentCancellation).toEqual({
      columns: [
    ...
    expect(supabaseHistoryEffectScope.duplicateHistoryFunctions).toEqual({
      ...
      'public.storefront_merchant_has_paystack_subaccount':
        'p_merchant_id uuid',
    });
  });

  it('binds merchant containment plus required extension and PGMQ boundaries', () => {
    expect(supabaseHistoryEffectScope.merchantContainment.relation).toBe(
      'public.merchants'
    );
    expect(supabaseHistoryEffectScope.merchantContainment.anonPolicy).toBe(
      'Anon can view merchants'
    );
    expect(
      supabaseHistoryEffectScope.merchantContainment.rowLevelSecurity
    ).toEqual({ enabled: true, forced: false });
    expect(
      supabaseHistoryEffectScope.merchantContainment.tablePrivileges
    ).toEqual({ PUBLIC: [], anon: [] });
    expect(
      supabaseHistoryEffectScope.merchantContainment.publicSelectableColumns
    ).toEqual([]);
    expect(
      supabaseHistoryEffectScope.merchantContainment.anonSelectableColumns
    ).toEqual(MERCHANT_ANON_COLUMNS);
    expect(
      supabaseHistoryEffectScope.merchantContainment.anonForbiddenColumns
    ).toEqual([
      'bvn',
      'cac_number',
    ...
    expect(
      supabaseHistoryEffectScope.merchantContainment
        .forbiddenFeatureSettingsReadRoles
    ).toEqual(['PUBLIC', 'anon']);
    expect(supabaseHistoryEffectScope.requiredExtensions).toEqual([
      { name: 'pgcrypto', schema: 'extensions' },
      { name: 'pgmq', schema: 'pgmq' },
    ]);
    expect(supabaseHistoryEffectScope.pgmq).toEqual({
  ...
  it('is deeply immutable', () => {
    expect(Object.isFrozen(supabaseHistoryEffectScope)).toBe(true);
    expect(Object.isFrozen(supabaseHistoryEffectScope.eventPipeline)).toBe(
      true
    );
    expect(
      Object.isFrozen(supabaseHistoryEffectScope.eventPipeline.publicRpcs)
    ).toBe(true);
    expect(
      Object.isFrozen(supabaseHistoryEffectScope.requiredExtensions.at(0))
    ).toBe(true);
  });
});
