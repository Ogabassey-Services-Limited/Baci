...
async function process() {
  ...
  try {
    ...
    await execAsync(
      `scp -o BatchMode=yes -o ConnectTimeout=5 ${OUTPUT_FILE} ${REMOTE_HOST}:${tmpRemote}`
    );

    console.log('Step 2: Moving file');
    await execAsync(
      `ssh ${REMOTE_HOST} "sudo mv ${tmpRemote} ${TARGET_PATH}${OUTPUT_FILE}"`
    );

    console.log('Step 3: Fixing permissions');
    await execAsync(
      `ssh ${REMOTE_HOST} "sudo chown www-data:www-data ${TARGET_PATH}${OUTPUT_FILE}"`
    );

    console.log(
      `✅ Upload successful: https://cdn.ogabassey.com/products/laptops/${OUTPUT_FILE}`
    );
  } catch (err) {
