...
describe('isPaypalMerchantCountry', () => {
  it.each(['GB', 'US', 'IE', 'DE', 'CA', 'AU'])(
    'allows %s — PayPal pays merchants there',
    (country) => {
      expect(isPaypalMerchantCountry(country)).toBe(true);
    }
  );

  it.each(['NG', 'GH', 'UG', 'TZ', 'RW'])(
    'refuses %s — PayPal is effectively send-only, so a merchant there can never be paid',
    (country) => {
      expect(isPaypalMerchantCountry(country)).toBe(false);
    }
  );

  it.each(['ZA', 'KE', 'BW', 'LS', 'MU', 'MA', 'MZ', 'SN'])(
    'allows %s — PayPal does pay out there',
    (country) => {
