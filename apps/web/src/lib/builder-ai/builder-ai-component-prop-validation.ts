...
export const builderAiBooleanProps = new Set([
...
export const builderAiStringProps = new Set([
  'Features.subtitle',
  'Features.title',
...
function hasEditableProperty(
  componentType: AiEditableComponentType,
  property: string
): boolean {
  return getAiComponentDefinition(componentType).editableProps.includes(
    property
  );
}
...
export function getBuilderAiPropShape(
  ...
  if (
    ...
    !hasEditableProperty(componentType, property)
  ) {
    return undefined;
  }
...
export function isBuilderAiPropValue(
  ...
  if (builderAiBooleanProps.has(key)) return typeof value === 'boolean';
  if (builderAiStringProps.has(key)) return typeof value === 'string';
