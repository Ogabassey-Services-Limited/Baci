...
export async function getStorefrontAccountDocumentData({
  ...
  const [
    ...
    supabase
      .from('order_items')
      .select(
        'id, product_id, variant_id, condition, variant_name, name, quantity, price, line_extension_amount, unit_code, vat_category_code, vat_rate, vat_amount, sellers_item_id, fulfillment_data'
      )
      .eq('order_id', orderId),
    supabase
      .from('transactions')
