...
export async function getOrCreatePaystackCustomer(
    ...
    try {
        ...
        if (createData.message?.toLowerCase().includes('duplicate') || createData.status === false) {
            // Only fetch if it failed.
            // Note: Paystack unfortunately doesn't always return the code in error. 
            // We must query.
            const secretKey = PAYSTACK_SECRET_KEY || 'sk_test_placeholder';
            const getRes = await fetch(`https://api.paystack.co/customer/${customer.email}`, {
                method: 'GET',
        ...
        throw new Error(`Could not create or retrieve customer: ${createData.message}`);
