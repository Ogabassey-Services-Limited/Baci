...
/**
 * Format amount as NGN currency (e.g. ₦5,000)
 */
...
export async function notifyNegotiationRequest(
  ...
  const priceStr = formatCurrency(offeredPrice);
  const currentPriceStr = currentPrice ? formatCurrency(currentPrice) : null;
