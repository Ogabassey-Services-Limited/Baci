...
describe('ogabasseyHomeHeroContract', () => {
  ...
  it('fails closed when the request tenant, rendered slide, or preload source drifts from the projection', () => {
    const projection = ogabasseyHomeHeroContract.project(
      createPublishedShell()
    );
    if (!projection) {
    ...
    ).toEqual({ reason: 'merchant_mismatch', valid: false });
    expect(
      ogabasseyHomeHeroContract.assessRenderer({
        preload: {
          ...preload,
          imageUrl: 'https://cdn.ogabassey.com/other.avif',
        },
        projection,
        renderedSlides: [projection.candidate],
        requestPublication: {
    ...
    expect(
      ogabasseyHomeHeroContract.assessRenderer({
        preload,
        projection,
        renderedSlides: [{ ...projection.candidate, id: 'other' }],
        requestPublication: {
          merchantId: projection.merchantId,
          status: 'published',
        },
    ...
    expect(
      ogabasseyHomeHeroContract.assessRenderer({
        ...
        projection,
        renderedSlides: [
          { ...projection.candidate, imageAlt: 'Wrong alternative text' },
        ],
        requestPublication: {
          merchantId: projection.merchantId,
          status: 'published',
