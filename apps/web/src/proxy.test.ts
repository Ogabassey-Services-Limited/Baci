...
describe('Middleware Proxy', () => {
  ...
  it.each([
    ...
    'does not flatten blog post %s metadata routes as legacy category URLs',
    async (metadataRoute) => {
      const req = new NextRequest(
  ...
  it.each([
    ...
    'redirects legacy category paths to canonical post URLs for HTML navigation when slug is %s',
    async (postSlug) => {
      const req = new NextRequest(
