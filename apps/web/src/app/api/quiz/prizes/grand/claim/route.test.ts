...
function jsonRequest(body: unknown) {
  return new NextRequest('http://localhost/api/quiz/prizes/grand/claim', {
...
describe('grand prize claim route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    rpc.mockResolvedValue({
      data: { event_id: EVENT_ID, status: 'claimed' },
      error: null,
    });
  ...
  it('returns invalid JSON responses before guard or RPC work', async () => {
    ...
    const response = await POST(
      new NextRequest('http://localhost/api/quiz/prizes/grand/claim', {
        body: '{bad-json',
        method: 'POST',
      })
    );
  ...
  it('returns validation errors before guard or RPC work', async () => {
    const { POST } = await import('./route');
    const response = await POST(jsonRequest({ eventId: 'not-a-uuid' }));
  ...
  it('returns proof creation failures before RPC calls', async () => {
    const proofResponse = NextResponse.json(
    ...
    expect(response).toBe(proofResponse);
    expect(enforceEventPrizeGuard).toHaveBeenCalledWith(supabase, EVENT_ID);
    expect(rpc).not.toHaveBeenCalled();
  });

  it('returns 500 when the Supabase RPC returns an error', async () => {
    rpc.mockResolvedValue({
      data: null,
  ...
  it('claims grand prizes with the signed server proof', async () => {
    ...
    expect(await readJson(response)).toEqual({
      claim: { event_id: EVENT_ID, status: 'claimed' },
    });
    expect(enforceEventPrizeGuard).toHaveBeenCalledWith(supabase, EVENT_ID);
    expect(createRouteProof).toHaveBeenCalledWith({
      action: 'claim_grand_prize',
    ...
    expect(rpc).toHaveBeenCalledWith('claim_quiz_grand_prize', {
      p_event_id: EVENT_ID,
