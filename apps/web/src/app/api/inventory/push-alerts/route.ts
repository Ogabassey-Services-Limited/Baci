      ...
      `)
        ...
        for (const alert of alerts) {
            try {
                ...
                const products = alert.products as unknown as { id: string; name: string } | { id: string; name: string }[] | null;
                ...
                console.error(`[Inventory Alerts] Failed to notify for alert ${alert.id}:`, pushError);
                failedCount++;
            }
        }

        console.log(`[Inventory Alerts] Sent ${sentCount} push notifications, ${failedCount} failed`);
