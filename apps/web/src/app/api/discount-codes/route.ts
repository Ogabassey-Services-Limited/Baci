...
export async function GET() {
  try {
    ...
    const { data: discountCodes, error } = await supabase
      .from('discount_codes')
      .select(
        'id, merchant_id, code, description, discount_type, discount_value, minimum_purchase_amount, maximum_discount_amount, usage_limit, usage_count, usage_limit_per_customer, starts_at, expires_at, is_active, applies_to, product_ids, category_ids, created_at, updated_at'
      )
