...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: discountCode, error } = await supabase
      .from('discount_codes')
      .select(
        'id, code, description, discount_type, discount_value, starts_at, expires_at, usage_limit, usage_count, minimum_purchase_amount, applies_to, product_ids, category_ids, usage_limit_per_customer, maximum_discount_amount'
      )
