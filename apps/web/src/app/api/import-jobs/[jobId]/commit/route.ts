...
export async function POST(
  ...
  try {
    ...
    const validRows = Number(job.summary?.validRows || 0);
    if (job.status !== 'preview_ready') {
      return NextResponse.json(
        { error: 'Import job is not ready to commit', code: 'invalid_status' },
        { status: 409 }
      );
    }
    ...
    if (validRows < 1) {
      return NextResponse.json(
        {
          error: 'Import job has no valid rows to commit',
          code: 'empty_commit',
        },
        { status: 409 }
      );
    ...
    logger.error({
      message: 'Import job commit route failed',
      error,
    });
    return NextResponse.json(
