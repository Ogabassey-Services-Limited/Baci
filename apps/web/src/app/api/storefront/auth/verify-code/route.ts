...
export async function POST(request: Request) {
  try {
    ...
    if (verifyError || !authData.user) {
      console.error('OTP verification error:', JSON.stringify(verifyError).replace(/[\r\n]/g, ' '));
    ...
    if (customerError) {
      console.error('Customer upsert error:', JSON.stringify(customerError).replace(/[\r\n]/g, ' '));
    ...
    console.error('Verify code error:', JSON.stringify(error).replace(/[\r\n]/g, ' '));
    return NextResponse.json(
