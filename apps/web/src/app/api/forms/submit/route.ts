  ...
  try {
    ...
    if (merchantEmail) {
      ...
      const formDataRows = Object.entries(formData)
        .map(
          ([key, value]) =>
            `<tr><td style="padding: 8px; border: 1px solid #eee; font-weight: 500;">${escapeHtmlText(String(key))}</td><td style="padding: 8px; border: 1px solid #eee;">${escapeHtmlText(String(value ?? ''))}</td></tr>`
        )
        .join('');
