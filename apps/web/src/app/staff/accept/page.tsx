...
export default async function StaffAcceptPage({ searchParams }: AcceptPageProps) {
    ...
    const { data: invitation, error: inviteError } = await supabaseAdmin
        .from('staff_members')
        .select('id, email, role, status, invitation_expires_at, merchants(business_name, slug)')
    ...
    // Get merchant info
    const merchantInfo = invitation.merchants as { business_name: string; slug: string } | null;
            ...
            <ThemedCard className="w-full max-w-md p-8 shadow-xl">
                ...
                <div className="text-center mb-6">
                    ...
                    <p className="text-muted-foreground">
                        <strong>{merchantName}</strong> has invited you to join their team as a <strong>{role}</strong>.
                    </p>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-xs text-center text-muted-foreground">
                        ...
                        <Link href={`/signup?email=${encodeURIComponent(inviteEmail)}&redirect=/staff/accept?token=${token}`} className="text-primary hover:underline">
                            Create one
                        </Link>
                    </p>
                </div>
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-500/5 to-orange-500/5 p-4">
            <ThemedCard className="w-full max-w-md p-8 shadow-xl">
                ...
                <div className="text-center mb-6">
                    ...
                    <h1 className="text-2xl font-bold mb-2 text-red-600 dark:text-red-400">{title}</h1>
                ...
                {currentEmail && (
                    <div className="bg-muted/50 rounded-lg p-4 mb-6">
                        <p className="text-sm text-muted-foreground">
                            <strong>Currently signed in as:</strong><br />
                            {currentEmail}
                        </p>
                    </div>
                )}
