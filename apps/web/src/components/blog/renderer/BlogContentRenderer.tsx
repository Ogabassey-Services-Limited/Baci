        ...
        for (const mark of node.marks) {
            switch (mark.type) {
                ...
                case 'strike':
                    ...
                    break;
                case 'code':
                    content = <code key={mark.type} className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">{content}</code>;
                    break;
                case 'link':
                    content = (
                        <Link
                ...
                case 'textStyle':
                    if (mark.attrs?.color) {
                        content = <span key={mark.type} style={{ color: mark.attrs.color }}>{content}</span>;
                    }
                    break;
            }
        }
...
const NodeRenderer = ({ node, index }: NodeRendererProps): React.ReactNode => {
    ...
    switch (node.type) {
        ...
        case 'paragraph':
            ...
            return <p className={cn('leading-relaxed', textAlignClass)}>{children}</p>;
        ...
        case 'tableRow':
            return <tr className="border-b border-border/50 hover:bg-muted/30 transition-colors">{children}</tr>;

        case 'tableHeader':
            return <th className="bg-muted/50 px-4 py-3 text-left font-semibold text-foreground border-r border-border/50">{children}</th>;

        case 'tableCell':
            return <td className="px-4 py-3 border-r border-border/50 text-muted-foreground">{children}</td>;
