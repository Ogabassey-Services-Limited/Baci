...
describe('getCheckoutOrderErrorMessage', () => {
  ...
  it('maps a multi-voucher rejection to redeem-one-at-a-time guidance', () => {
    const message = getCheckoutOrderErrorMessage({
    ...
    expect(message).toBe(
      'You can redeem only one prize voucher per order. Remove the extra voucher and check out one prize at a time.'
    );
  });
...
describe('isQuizVoucherRejectionCode', () => {
  ...
  it('does NOT prune a multi-voucher rejection (both vouchers stay valid)', () => {
    // Two individually valid prizes: the shopper redeems one at a time, so the
    // cart lines must survive rather than both being discarded.
    expect(isQuizVoucherRejectionCode('QUIZ_VOUCHER_MULTIPLE')).toBe(false);
    expect(isQuizVoucherRejectionCode('quiz_voucher_multiple')).toBe(false);
  });
