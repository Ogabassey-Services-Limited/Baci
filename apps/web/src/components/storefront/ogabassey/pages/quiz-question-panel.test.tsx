...
describe('QuizQuestionPanel', () => {
  ...
  it('tells the countdown whether an answer is selected so a forfeit waits for the deadline', () => {
    renderPanel({ selectedAnswer: null });
    expect(mockUseQuizCountdown).toHaveBeenLastCalledWith(
      expect.objectContaining({ hasSelectedAnswer: false })
    );

    mockUseQuizCountdown.mockClear();

    renderPanel({ selectedAnswer: 'b' });
    expect(mockUseQuizCountdown).toHaveBeenLastCalledWith(
      expect.objectContaining({ hasSelectedAnswer: true })
    );
  });
