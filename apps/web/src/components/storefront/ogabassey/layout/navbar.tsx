...
export const OgabasseyNavbar: React.FC<NavbarProps> = ({
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (isBlogPage) {
      router.push(`${storeSlug || ''}/blog?search=${encodeURIComponent(trimmedQuery)}` as `/${string}`);
    } else {
      router.push(`${storeSlug || ''}/search?q=${encodeURIComponent(trimmedQuery)}` as `/${string}`);
    }
  };
  ...
  const handleBlogSearch = () => {
    ...
    router.push(`${storeSlug || ''}/blog?search=${encodeURIComponent(searchQuery)}` as `/${string}`);
  };
        ...
        <div className="bg-white border-b border-gray-200 relative z-10 hidden md:block">
          ...
          <div className="max-w-[1400px] mx-auto px-4 md:px-6 relative z-10">
            <div className="flex items-center gap-6 h-12">
                  ...
                  <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 py-2 animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                    ...
                    {categories.length > 0 ? (
                      categories.map((cat) => (
                        <Link
                          key={cat.slug}
                          href={`${storeSlug || ''}/${cat.slug}` as `/${string}`}
              ...
              {/* IMEI Checker */}
              <Link
                href={`${storeSlug || ''}/imei-check` as `/${string}`}
              ...
              {/* Repairs */}
              <Link
                href={`${storeSlug || ''}/repairs` as `/${string}`}
                className="flex items-center gap-2 text-sm font-medium text-gray-700 hover:text-primary transition-colors px-1 py-1"
              >
                <Wrench size={18} />
                Repairs
              ...
              {/* Wallet */}
              <Link
                href={`${storeSlug || ''}/wallet` as `/${string}`}
                className="flex items-center gap-2 text-sm font-medium text-gray-700 hover:text-primary transition-colors px-1 py-1"
              >
                <Wallet size={18} />
                Wallet
