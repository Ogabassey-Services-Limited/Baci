...
export function InformationalClusterPanel({
  ...
  return (
    <section
      ...
      className="mt-8 rounded-3xl border border-[color-mix(in_srgb,var(--store-background-text,#111827)_10%,transparent)] bg-(--store-background,#ffffff) p-5 md:p-6"
    >
      <h2
        id="informational-cluster-panel"
        className="text-xl font-semibold text-(--store-background-text,#111827)"
      >
        {model.heading}
      </h2>
      ...
      {model.featuredProducts.length > 0 ? (
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {model.featuredProducts.map((product) => (
            <article
              key={product.href}
              className="rounded-2xl border border-[color-mix(in_srgb,var(--store-background-text,#111827)_10%,transparent)] p-4"
            >
              <a
                className="text-base font-semibold text-(--store-background-text,#111827) underline-offset-4 hover:underline"
                href={product.href}
              >
                {product.title}
              </a>
              <p className="mt-2 text-sm text-(--store-background-text,#111827)/70">
                {product.description}
              </p>
            </article>
          ))}
