...
export function InformationalClusterIndex({
  ...
  return (
    ...
    >
      <div className="rounded-3xl border border-[color-mix(in_srgb,var(--store-background-text,#111827)_10%,transparent)] bg-(--store-background,#ffffff) p-5 md:p-6">
        <h2
          id="guide-collections"
          className="text-2xl font-semibold text-(--store-background-text,#111827)"
        >
          Guide collections
        </h2>
              ...
              <ul className="mt-4 space-y-3">
                {collection.guides.map((guide) => (
                  <li key={guide.href}>
                    ...
                    <p className="mt-1 text-sm text-(--store-background-text,#111827)/70">
                      {guide.description}
                    </p>
                  </li>
                ))}
