...
export const COMPONENT_SCHEMA = {
  Header: {
    ...
    props: {
      ...
      glassEffect: { type: 'boolean', description: 'Enable frosted glass background' }
    }
  },
  Hero: {
    description: 'Large banner section with text and call-to-action. usually the first item below header.',
    props: {
      ...
      overlay: { type: 'boolean', description: 'Dark overlay for text readability' }
    }
  },
  HeroCarousel: {
  ...
  Features: {
    description: 'Grid of 2-4 key selling points or benefits (e.g., Fast Shipping, 24/7 Support).',
    props: {
      ...
      features: {
        ...
        items: {
          ...
          icon: { type: 'string', description: 'Lucide icon name (e.g., truck, shield, star)' }
        }
      }
    }
  },
  ProductGrid: {
    ...
    props: {
      title: { type: 'string', description: 'Section heading (e.g., New Arrivals)' },
      columns: { type: 'number', enum: [2, 3, 4] },
