...
const ToastViewport = ({
  ...
  <ToastPrimitives.Viewport
    ...
    style={{
      // Keep the empty viewport out of document flow before Tailwind's
      // render-blocking CSS applies; otherwise its user-agent <ol> margins
      // can push storefront shells down and create a root-level CLS.
      margin: 0,
      ...style,
      position: 'fixed',
    }}
...
const ToastClose = ({
  ...
  <ToastPrimitives.Close
    ...
    className={cn(
      'absolute right-2 top-2 rounded-md p-1 text-red-500 opacity-0 transition-opacity hover:text-red-600 focus:opacity-100 focus:outline-hidden focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-red-300 hover:group-[.destructive]:text-red-50 focus:group-[.destructive]:ring-red-400 focus:group-[.destructive]:ring-offset-red-600',
      className
    )}
    toast-close=""
    {...props}
