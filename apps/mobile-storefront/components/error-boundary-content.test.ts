...
describe('error boundary content helpers', () => {
  it('classifies typed network, auth, and server errors before message matching', () => {
    expect(classifyError(Object.assign(new Error('ignored'), { code: 'NETWORK_ERROR' }))).toBe(
      'network'
    );
    expect(classifyError(Object.assign(new Error('ignored'), { code: 'AUTH_ERROR' }))).toBe(
      'auth'
    );
    expect(classifyError(Object.assign(new Error('ignored'), { code: 'SERVER_ERROR' }))).toBe(
      'supabase'
    );
  });
  ...
  it('falls back to message-based classification', () => {
    ...
    expect(classifyError(new Error('Unexpected render error'))).toBe(
      'general'
    );
  });
