...
const mockAuth: { state: AuthSnapshot } = {
  state: { customer: null, isInitialized: false },
};
...
describe('resolveActiveCustomerId', () => {
  beforeEach(() => {
    jest.useRealTimers();
    mockListeners.clear();
    mockAuth.state = { customer: null, isInitialized: false };
  });

  it('returns the customer id immediately when auth is already initialised', async () => {
    mockAuth.state = { customer: { id: 'customer-1' }, isInitialized: true };
  ...
  it('returns undefined immediately when initialised with nobody signed in', async () => {
    mockAuth.state = { customer: null, isInitialized: true };
  ...
  it('waits for hydration on a cold start from a notification tap', async () => {
    ...
    emit({ customer: { id: 'customer-2' }, isInitialized: true });
  ...
  it('resolves only once even if auth emits repeatedly', async () => {
    ...
    emit({ customer: { id: 'customer-3' }, isInitialized: true });
    emit({ customer: { id: 'customer-4' }, isInitialized: true });
