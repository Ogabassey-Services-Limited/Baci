...
LOG_FILE="${BACI_ANDROID_EMULATOR_LOG:-/tmp/baci-mobile-storefront-emulator.log}"
...
if [[ "$GPU_MODE" == "swiftshader_indirect" ]]; then
  echo "Refusing -gpu swiftshader_indirect for mobile-storefront QA; use host or auto." >&2
  exit 1
fi
...
if [[ -z "$emulator_build" || "$emulator_build" -lt "$MIN_EMULATOR_BUILD" ]]; then
  echo "Android Emulator is too old for mobile-storefront QA: ${emulator_version_output:-unknown}." >&2
...
if ! "$EMULATOR" -list-avds | grep -Fxq -- "$AVD_NAME"; then
  ...
  echo "Then launch only with: pnpm --filter @baci/mobile-storefront android:emulator" >&2
  exit 1
fi
