...
import { palette, SEMANTIC_COLORS } from './palette';
...
describe('palette', () => {
  ...
  it('keeps semantic colors derived from palette constants', () => {
    ...
    expect(SEMANTIC_COLORS.overlay).toBe('rgba(0, 0, 0, 0.5)');
  });
  ...
  it('keeps primary foreground/background palette pairs readable', () => {
    expect(palette.white).toMatch(HEX_COLOR_PATTERN);
    expect(palette.gray[900]).toMatch(HEX_COLOR_PATTERN);
    expect(palette.black).toMatch(HEX_COLOR_PATTERN);
    expect(palette.amber[500]).toMatch(HEX_COLOR_PATTERN);
  });
});
