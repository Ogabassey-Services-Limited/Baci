  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        ...
        // Transform variants
        const productVariants = (data as any).product_variants || [];
        // Use either the explicit flag or presence of variants
        const hasVariants = (data as any).has_variants || productVariants.length > 0;

        let variantAttributes: Record<string, string[]> = {};

        if (productVariants.length > 0) {
          // Synthesize attributes from variant rows
          ...
          attrMap.forEach((values, key) => {
            variantAttributes[key] = Array.from(values);
          });
        } else {
          // Fallback to existing column
          variantAttributes = (data as any).variant_attributes || {};
        }
