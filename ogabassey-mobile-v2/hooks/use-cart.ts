...
async function checkStock(productId: string, requestedQuantity: number): Promise<StockCheckResult> {
  ...
  // Treat null or 0 as "not tracking stock" = unlimited
  const currentStock = (data?.stock_quantity === null || data?.stock_quantity === 0)
    ? 999
    : data.stock_quantity;
  return {
...
export function useCart() {
  ...
  // Get negotiation/assurance methods from store
