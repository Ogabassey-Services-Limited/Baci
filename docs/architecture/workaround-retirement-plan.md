...
> **Discovery artifacts (read-only census + research, 2026-07-11, `docs/architecture/discovery/`):** `web-merchants-census.md`, `mobile-storefront-merchants-census.md`, `mobile-admin-merchants-census.md` (S0/S1 inputs), `B0-drainer-runtime-brief.md`, `C0-route-classification-brief.md`, `A1-route-decision-matrix.md`. Headline outputs: B0 → reuse the existing VPS signed-trigger + cron-recovery-sweep worker pattern (already operated; `pg_net` live, `pg_cron` available-but-unused); C0 → hybrid route-group-default + co-located `route-class-overrides.ts` generator; A1 → 3 GO (category listing, products index, global compare hub — page-1 canonical shells), 3 NO-GO (category compare hub, flat PDP, search).
...
- **B3 must subtract every reserved/non-cacheable segment and test collision cases.** It must consume the **typed route-classification artifact produced by C0/C** (the private-route denylist) — so **B3 is blocked on C0/C**, not independent (rev-3 mislabeled it so).
