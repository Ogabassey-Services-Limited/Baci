...
- [ ] Timed prototype demonstrating: claim → simulated crash → recover after lease expiry; retry failure → not reclaimable before `next_attempt_at` → reclaim after the delay; Next invalidation → mocked confirmed Vercel tag deletion → mocked strict Cloudflare call → generation-checked completion, with each stage failure preventing completion.
