...
Sensitive-column flag 🔴 = selects/writes any of `nin, bvn, *_access_token, ga4_api_secret, *_capi_token, stripe_customer_id, bank_account_number, cac_*, kyc_*, firs_*`. Proposed-replacement shorthand: **SNAP** = public snapshot RPC `resolve_storefront_public_snapshot_v2`; **PRIV-RPC** = permission-scoped private RPC (SECURITY DEFINER, staff/owner gated); **ROUTE** = move behind an authenticated Route Handler; **KEEP** = service-role/server-only and unaffected by the anon/authenticated ACL reset. An app-gated Server Component, action, or Route Handler that still queries through an anon/authenticated Supabase client is **not** KEEP: it must move to SNAP/PRIV-RPC/ROUTE before cutover.
...
| `app/(storefront)/[slug]/(commerce)/cart/page.tsx:40` | `CartContent` | SSR-cookie → **anon/auth** (customer) | select `.eq('id').single()` | vat_registration_status, vat_rate | Server Component | public | **SNAP** (public presentation) |
| `app/(storefront)/[slug]/(catalog)/(pdp)/[category]/[productSlug]/opengraph-image.tsx:31` | `Image` (OG) | `createAdminClient()` → **service_role** | select `.eq('slug').single()` | id, business_name, logo_url, brand_colors, country | generateMetadata/OG (edge) | public image | **SNAP** — drop service_role for public data |
| `app/(storefront)/[slug]/(catalog)/(pdp)/products/[productSlug]/opengraph-image.tsx:31` | `Image` (OG) | `createAdminClient()` → **service_role** | select `.eq('slug').single()` | id, business_name, logo_url, brand_colors, country | OG image | public image | **SNAP** |
| `app/sitemap.ts:11` | `sitemap` | SSR-cookie → **anon/auth** | select `.not('slug', is, null).order()` | slug, updated_at | sitemap | public | **SNAP** / purpose-built public sitemap resolver |
...
| `app/api/analytics/conversion/route.ts:36,46,111` | `resolveLegacyMerchant` / pipeline fallback | `createServerClient()` → **anon/authenticated** depending on request cookies | select `.eq('id'|'slug').maybeSingle()` | id | public conversion/event ingress | tenant inferred from merchant id/origin/host | **SNAP** / purpose-built public id resolver before ACL reset |
| `lib/events/event-ingress-context.ts:58` | `resolveSlug` | injected conversion/event client → **anon/authenticated** | select `.eq('slug').maybeSingle()` | id | public event tenant verification | host/path-verified tenant | **SNAP** / same public id resolver |
...
| `app/actions.ts:34` | landing stats count | SSR-cookie (unconfirmed) → anon/auth | select `id` `head:true count:'exact'` | count only | Server Action (platform stats) | public | purpose-built public aggregate RPC before ACL reset |
...
| file:line | symbol | client → role | op | columns | context | authz | replacement |
|---|---|---|---|---|---|---|---|
| `app/(platform)/onboarding/onboarding-page-content-server.tsx:16` | onboarding gate | SSR-cookie → **authenticated** | select `.eq('user_id').maybeSingle()` | id, business_name | Server Component | user | **PRIV-RPC/ROUTE before ACL reset** |
...
| `app/dashboard/settings/tax/page.tsx:33` | tax settings load | SSR-cookie → **authenticated** | select `MERCHANT_TAX_SETTINGS_COLUMNS` | vat_registration_status, vat_rate, tax_identification_number, legal_entity_name, registered_address, state_code | Server Component | owner | PRIV-RPC |
...
| `app/dashboard/staff/actions.ts:229` | VT legacy sync | SSR-cookie/admin (dual-client file) → auth/service | **update** `.eq('id').is('virtual_terminal_code', null)` | virtual_terminal_code | Server Action | owner | confirm the exact variable; **PRIV-RPC** if authenticated, KEEP only if service-role after a preceding owner/permission check |
| `app/dashboard/wallet/actions.ts:30` | ownership verify | SSR-cookie → **authenticated** | select `.eq('id').eq('user_id').single()` | id | Server Action | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/dashboard/wallet/page.tsx:32` | wallet load | SSR-cookie → **authenticated** | select `.eq('user_id').single()` | id, payout_currency | Server Component | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/dashboard/orders/actions.ts:700` | 🔴 resend-notif merchant | SSR-cookie → **authenticated** | select `.eq('id').single()` | id, business_name, slug, support_email, email_sender_name, email, tax_identification_number, **cac_rc_number** | Server Action | owner | PRIV-RPC |
...
| `app/api/merchant/blog/posts/route.ts:194` | blog author | `auth.supabase` → **authenticated** | select `.eq('id').single()` | business_name, slug | Route Handler | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
...
| `app/api/merchant/regenerate-hero-images/route.ts:41` | hero regen read | SSR-cookie → **authenticated** | select `.eq('owner_id').single()` | id, business_type, hero_images_regeneration_count | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/merchant/regenerate-hero-images/route.ts:121` | hero regen read | SSR-cookie → **authenticated** | select `.eq('owner_id').single()` | id, hero_image_ids, hero_images_regeneration_count, hero_images_generated_at | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/merchant/rename-slug/route.ts:128` | current slug read | `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | slug | Route Handler | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
...
| `app/api/merchant/verify-bvn/route.ts:78` | verify-bvn read | `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | country, phone | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/merchant/verify-cac/route.ts:101` | verify-cac read | `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | country | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/merchant/verify-nin/route.ts:60` | verify-nin read | `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | country | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/merchant/verify-tax-id/route.ts:107` | 🔴 tax identity read | `auth.supabase` → **authenticated** | select `MERCHANT_TAX_IDENTITY_COLUMNS` | id, business_name, legal_entity_name, **cac_rc_number** | Route Handler | owner | PRIV-RPC |
| `app/api/merchant/verify-tax-id/route.ts:181` | tax id write | `auth.supabase` → **authenticated** | **update** `.eq('id').select(MERCHANT_SETTINGS_COLUMNS)` | writes tax_identification_number, updated_at; returns id, social_media, vat_registration_status, tax_identification_number, legal_entity_name, registered_address, state_code, updated_at | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/merchant/quiz/generate/quiz-generate-helpers.ts:114` | quiz ctx | injected `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | business_name, slug | helper (Route Handler) | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/merchant/quiz/prize-products/route.ts:48` | prize slug | `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | slug | Route Handler | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/quiz/events/route.ts:151` | quiz events merchant | `auth.supabase` → **authenticated** | select `.eq('id'|'slug').maybeSingle()` | id | Route Handler | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/marketplace/jumia/products/update/route.ts:183` | merchant id | SSR-cookie → **authenticated** | select `.eq('user_id').single()` | id | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/payouts/request/route.ts:76` | payout email | SSR-cookie → **authenticated** | select `.eq('id').single()` | email | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/paystack/subaccount/route.ts:159` | subaccount read | `auth.supabase` → **authenticated** | select `.eq('id').single()` | paystack_subaccount_code, business_name, country, email, phone | Route Handler | owner/staff prechecked | **PRIV-RPC** — return only the scoped configuration needed by this route before ACL reset |
| `app/api/paystack/subaccount/route.ts:220` | 🔴 manual bank write | `auth.supabase` → **authenticated** | **update** `.eq('id')` | paystack_subaccount_code, **bank_account_number**, bank_account_name, bank_code, bank_name | Route Handler | owner/staff permission checked | **PRIV-RPC** — same scoped mutation boundary; direct DML cannot survive the ACL reset |
| `app/api/paystack/subaccount/route.ts:344` | 🔴 bank write | `auth.supabase` → **authenticated** | **update** `.eq('id')` | paystack_subaccount_code, **bank_account_number**, bank_account_name, bank_code, bank_name | Route Handler | owner/staff permission checked | **PRIV-RPC** — same scoped mutation boundary |
| `app/api/paystack/virtual-terminal/route.ts:158` | VT legacy read | SSR-cookie → **authenticated** | select `.eq('id').single()` | virtual_terminal_code | Route Handler | owner/staff prechecked | **PRIV-RPC** before ACL reset |
| `app/api/paystack/virtual-terminal/route.ts:177` | VT legacy write | SSR-cookie → **authenticated** | **update** `.eq('id').select('id')` | virtual_terminal_code | Route Handler | owner/staff prechecked | **PRIV-RPC** before ACL reset |
| `app/api/paystack/virtual-terminal/[code]/virtual-terminal-local-sync.ts:96` | `clearLegacyTerminalCode` | injected SSR-cookie client → **authenticated** | **update** `.eq('id').eq('virtual_terminal_code')` | virtual_terminal_code=null | authenticated VT Route Handler | owner/staff terminal ownership prechecked | **PRIV-RPC** — fold into the existing scoped terminal sync/cleanup boundary before ACL reset |
| `app/api/products/[id]/archive/route.ts:110` | slug for purge | `auth.supabase` → **authenticated** | select `.eq('id').single()` | slug | Route Handler | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/products/[id]/route.ts:640` | product schema ctx | SSR-cookie → **authenticated** | select `.eq('id').single()` | business_name, country | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/products/bulk-update/route.ts:94` | product ctx | SSR-cookie → **authenticated** | select `.eq('id').maybeSingle()` | business_name, country, payout_currency | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/products/route.ts:442` | product country | SSR-cookie → **authenticated** | select `.eq('id').single()` | country | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/shipping/book/route.ts:126` | sender info | SSR-cookie → **authenticated** | select `.eq('id').single()` | business_name, business_address, phone | Route Handler | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/cache/revalidate/route.ts:200` | slug for purge | `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | slug | Route Handler | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/builder/builder-route-utils.ts:147` | template data | injected `auth.supabase` → **authenticated** | select `.eq('id').single()` | id, business_name, business_type, brand_colors, logo_url, hero_image_ids | Route Handler | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `lib/get-merchant-for-api-request.ts:49` | core resolver | injected `auth.supabase` → **authenticated** | select `.eq('user_id')` (+ or/limit) | id, slug, business_name | shared helper | user | **PRIV-RPC/ROUTE before ACL reset** |
| `lib/merchant-feature-gates.ts:73` | feature gate | injected `auth.supabase` → **authenticated** | select `.eq('id').single()` | id, plan_tier, plan_expires_at, premium_features | shared helper | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `lib/merchant-email-domain-access.ts:50` | email-domain gate | injected `auth.supabase` → **authenticated** | select `.eq('id').single()` | plan_tier, slug, plan_expires_at, premium_features | shared helper | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `lib/analytics/analytics-platform-config.ts:122` | entitlement | SSR-cookie → **authenticated** | select `.eq('id').maybeSingle()` | plan_tier, plan_expires_at, premium_features | analytics Route Handlers | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `lib/analytics/analytics-platform-config.ts:136` | 🔴 platform creds | SSR-cookie → **authenticated** | select `.eq('id').maybeSingle()` | offline_conversions_enabled, facebook_pixel_id, **facebook_capi_token**, tiktok_pixel_id, **tiktok_access_token**, google_analytics_id, **ga4_api_secret**, snapchat_pixel_id, **snapchat_capi_token** | analytics Route Handlers | owner | PRIV-RPC (server-only credential read) |
| `lib/get-merchant-blog-cache-identifiers.ts:37` | blog cache id | injected → **authenticated** | select `.eq('id').maybeSingle()` | slug | shared helper | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `lib/shipping/book-order-shipment.ts:320` | sender info | injected `auth.supabase` → **authenticated** | select `.eq('id').single()` | business_name, business_address, phone | order Route Handler | owner/staff | **PRIV-RPC/ROUTE before ACL reset** |
| `lib/admin-merchant-users.ts:45` | admin directory | SSR-cookie → **authenticated** (platform admin) | select `MERCHANT_COLUMNS` `.eq('id').maybeSingle()` | id, user_id, business_name, email, phone, slug, signup_source, plan_tier, is_published, created_at, updated_at | Route Handler | is_platform_admin gated | **PRIV-RPC/ROUTE before ACL reset** |
| `services/hero-image-generator.ts:284` | hero write | SSR-cookie → **authenticated** | **update** `.eq('id')` | hero_image_ids, hero_images_generated_at | service | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `services/hero-image-generator.ts:304` | hero write | SSR-cookie → **authenticated** | **update** `.eq('id')` | hero_image_ids, hero_images_generated_at | service | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `services/hero-image-generator.ts:347` | hero read | SSR-cookie → **authenticated** | select `.eq('id').single()` | business_type, hero_image_ids, hero_images_regeneration_count | service | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `services/hero-image-generator.ts:386` | hero count write | SSR-cookie → **authenticated** | **update** `.eq('id')` | hero_images_regeneration_count | service | owner | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/mobile-onboarding/route.ts:331` | existing merchant | `scopedSupabase` (bearer) → **authenticated** | select `.eq('user_id').maybeSingle()` | id, business_name, slug | Route Handler | user | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/mobile-onboarding/route.ts:378` | update merchant | `scopedSupabase` → **authenticated** | **update** `.eq('id').select('id, slug')` | merchantUpdate payload | Route Handler | user | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/mobile-onboarding/route.ts:413` | insert merchant | `scopedSupabase` → **authenticated** | **insert** | user_id, email, business_name, business_type, country, payout_currency, logo_url, favicon_png_192_url, brand_colors, slug | Route Handler | user | **PRIV-RPC/ROUTE before ACL reset** |

---

## D. Platform-admin authorization checks (`is_platform_admin`)

All are `select('is_platform_admin').eq('id'|'user_id')` gating platform-admin ops through an **authenticated** client. They must move to a dedicated authorization RPC/claim before the authenticated table ACL reset; app-level gating does not preserve direct `merchants` SELECT after revocation:
...
| `app/api/admin/migrate/route.ts:71` | select `.limit(1)` | social_media (column-exists probe) | SSR-cookie → **authenticated** | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/admin/notifications/route.ts:379` | select `.not('user_id', is, null)` | id | SSR-cookie → **authenticated** | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/admin/notifications/route.ts:470` | select (segment builder) | id | SSR-cookie → **authenticated** | **PRIV-RPC/ROUTE before ACL reset** |
...
| `app/api/payments/webhook/route.ts:2994` | user-scoped merchant | `createClient(cookieStore)` → **authenticated** | select `.eq('user_id').single()` | id | authenticated handler in same file | **PRIV-RPC/ROUTE before ACL reset** |
...
| `app/api/orders/reuse/restamp-merchant-rate.ts:148` | reuse-rate verification context | injected admin client → **service_role** | select `.eq('id').maybeSingle()` | country, payout_currency | authenticated order-reuse helper | KEEP |
| `app/api/payments/juicyway/webhook/route.ts:902` | 🔴 email ctx | `createAdminClient()` → **service_role** | select `.eq('id').single()` | business_name, slug, support_email, email_sender_name, email, tax_identification_number, **cac_rc_number** | webhook | KEEP |
| `app/api/orders/route.ts:1446` | 🔴 order-create merchant | SSR-cookie/admin dual → **authenticated** | select `.eq('id').single()` | id, phone, rider_phone_number, business_name, business_address, slug, support_email, email_sender_name, email, tax_identification_number, **cac_rc_number**, plan_tier, vat_registration_status, vat_rate, registered_address, support_phone, logo_url, legal_entity_name, brand_colors, bank_code, **bank_account_number**, bank_name, bank_account_name, social_media, pages, payout_currency, country | Route Handler | PRIV-RPC |
| `app/api/orders/[id]/cancelled/route.ts:80` | 🔴 email ctx | `auth.supabase` → **authenticated** (merchant) | select `.eq('id').single()` | id, business_name, slug, support_email, email_sender_name, email, tax_identification_number, **cac_rc_number** | Route Handler | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/orders/[id]/delivered/route.ts:78` | 🔴 email ctx | `auth.supabase` → **authenticated** | select `.eq('id').single()` | (same 8 incl. **cac_rc_number**) | Route Handler | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/orders/[id]/generate-dva/route.ts:109` | phone | `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | phone | Route Handler | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/orders/[id]/record-payment/route.ts:162` | 🔴 email ctx | `auth.supabase` → **authenticated** | select `.eq('id').single()` | (same 8 incl. **cac_rc_number**) | Route Handler | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/orders/[id]/reminder/route.ts:76` | 🔴 email ctx | `auth.supabase` → **authenticated** | select `.eq('id').single()` | business_name, slug, support_email, email_sender_name, tax_identification_number, **cac_rc_number** | Route Handler | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/orders/[id]/ship-on-credit/route.ts:79` | merchant | `auth.supabase` → **authenticated** | select `.eq('id').single()` | id, business_name | Route Handler | **PRIV-RPC/ROUTE before ACL reset** |
| `app/api/orders/[id]/shipped/route.ts:146` | 🔴 email ctx | `auth.supabase` → **authenticated** | select `.eq('id').single()` | (same 8 incl. **cac_rc_number**) | Route Handler | **PRIV-RPC/ROUTE before ACL reset** |
...
| `lib/order-update-email.ts:60` | 🔴 email ctx | injected → **authenticated** | select `.eq('id').single()` | (same 8 incl. **cac_rc_number**) | shared helper | **PRIV-RPC/ROUTE before ACL reset** |
...
| `lib/payments/schedule-manual-paid-order-side-effects.ts:37` | manual-payment email ctx | `createServiceClient()` → **service_role** | select `.eq('id').single()` | business_name, slug, support_email, email_sender_name, email, tax_identification_number, cac_rc_number, website_url | deferred payment side-effect | KEEP |
| `lib/trigger-purchase-conversion.ts:152` | conversion currency | injected `auth.supabase` → **authenticated** | select `.eq('id').maybeSingle()` | country, payout_currency | record-payment | **PRIV-RPC/ROUTE before ACL reset** |
| `lib/merchant-zoho-campaign-settings.ts:155` | email brand | injected → service/auth | select `.eq('id').maybeSingle()` | business_name, brand_colors | background | split/confirm callers; KEEP for service-role only, **PRIV-RPC/SNAP** for authenticated callers |
...
| `lib/imei-remediation/petrock-remediation-notifications.ts:65` | terminal IMEI notification brand | injected admin client → **service_role** | select `.eq('id').maybeSingle()` | business_name, slug | background notification | KEEP |
...
| `lib/agentic/checkout-order-tax.ts:139` | VAT status | injected → service/auth | select `.eq('id').maybeSingle()` | vat_registration_status | agentic/order | split/confirm callers; KEEP for service-role only, **PRIV-RPC/SNAP** for authenticated callers |
