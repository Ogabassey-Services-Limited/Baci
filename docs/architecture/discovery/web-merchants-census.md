...
Source: `origin/main` @ `c8108a052dfccfb0c99f4c5e6cac96a56dad9587` (fetched; head was `cff335b0fd`). READ-ONLY. Every non-test `.from('merchants')` occurrence plus merchants-reading/-writing RPCs. Subsumes the S0 anon-web read set.
...
| `app/dashboard/settings/actions.ts:53` | favicon write | SSR-cookie → **authenticated** | **update** `.eq('id')` | favicon_svg_url, favicon_png_32_url, favicon_png_192_url, favicon_apple_touch_url, favicon_uploaded_at | Server Action | owner | KEEP |
...
| `app/api/merchant/favicon/route.ts:90` | favicon write | `auth.supabase` → **authenticated** | **update** `.eq('id')` | favicon_svg_url, favicon_png_32_url, favicon_png_192_url, favicon_apple_touch_url, favicon_uploaded_at | Route Handler | owner/staff | KEEP |
