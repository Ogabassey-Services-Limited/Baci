...
Original census read against `origin/main` @ `c8108a052dfccfb0c99f4c5e6cac96a56dad9587`; revalidated after rebase at `origin/main@6758e4db3f`. Current code now reads receipt identity through `get_storefront_receipt_merchant_info`; pre-#3083 mobile binaries still require the temporary raw-table compatibility bridge documented below.

Exhaustive tracked-source grep (`.from('merchants')` + merchants-backed RPCs) across `apps/mobile-storefront` returns **exactly 4 current call sites**: 2 raw `merchants` selects and 2 bounded `SECURITY DEFINER` RPCs. No other `.from("merchants")`/schema-qualified variants exist. The pre-#3083 raw receipt select is a **legacy binary compatibility requirement, not a current source call site**, and is separated below.
...
| 2 | `apps/mobile-storefront/stores/auth-store-initialize.ts:53` | `createInitializeAction()` init sequence (runs once at app boot, before `supabase.auth.getSession()` is awaited) | **anon** for a fresh install/never-signed-in device. **Restored-authenticated-session case**: supabase-js loads any persisted session (AsyncStorage) into the client synchronously at client construction, independent of when app code calls `getSession()`. Since this query is issued *before* `auth.getSession()` is awaited in this same function, a device with a valid persisted session may already have its JWT attached to this specific request — i.e. this call can run as `authenticated` role rather than `anon`, non-deterministically from the app-code's point of view. Either way it is **not gated on auth** and is the very first DB call the app makes. | `id` only | **No.** Minimal single-column projection — already the correct pattern. |

### Current merchants-backed RPCs (audit fixed projections and EXECUTE grants)

| File:Line | Symbol | Mechanism | Sensitive? |
|---|---|---|---|
| `apps/mobile-storefront/hooks/use-receipts.ts:258` | `useMerchantReceiptInfo()` → `rpc('get_storefront_receipt_merchant_info', { p_slug })` | `SECURITY DEFINER`; `PUBLIC` revoked; `EXECUTE` granted to `anon, authenticated, service_role`; published-merchant-only fixed 20-column projection. The hook is unconditional on its receipt/order surfaces, so signed-out callers may execute it. | **Yes, deliberately bounded:** bank account fields plus CAC/tax/legal identity are returned for receipt/invoice rendering. S0 must preserve the fixed projection and review these grants; arbitrary `merchants` columns remain unreachable. |
| `apps/mobile-storefront/hooks/useMerchantPaymentSettings.ts:84` | `fetchMerchantPaymentSettings()` → `rpc('get_storefront_payment_settings', { p_merchant_id })` | `SECURITY DEFINER`; `PUBLIC` revoked; `EXECUTE` granted to `anon, authenticated, service_role`; fixed narrow return shape (`paystack_enabled, korapay_enabled, juicyway_enabled, credpal_enabled, credit_direct_enabled, klump_enabled, klump_min_amount, klump_max_amount, pay_on_delivery_enabled, wallet_order_auto_debit_enabled, wallet_paystack_dva_enabled, vat_registration_status, vat_rate`) | No. Audit its grants and projection alongside the receipt RPC; neither requires a raw table grant. |

### Legacy mobile receipt compatibility (not a current source call site)

Pre-#3083 binaries call the raw `merchants` table from `useMerchantReceiptInfo()` as anon/authenticated and request the receipt projection listed in S0-A below. That shipped behavior is why the dated compatibility grant cannot be removed until the mandatory-version gate excludes those binaries. Current source uses `get_storefront_receipt_merchant_info` instead; do not count the legacy raw select as a third current table call.
