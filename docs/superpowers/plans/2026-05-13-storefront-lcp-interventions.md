...
**Problem.** After PR #1729, mobile PDP still misses the target at 3226 ms LCP, while desktop PDP LCP remains good. The mobile LCP node is now the primary product image, not the desktop-only Flash Sale banner. Its request is discovered early through the existing high-priority preload, but the rendered image still has Next's default `decoding="async"`. Apply that low-risk image decode fix first; if the route remains above target, continue with the larger bundle/client-boundary work. The broader bundle evidence still matters: earlier post-#1634 PSI runs showed ~156 KiB unused JS and ~38 KiB unused CSS, plus local production chunks containing heavy libraries (`moment`, `lodash`, `three`, `puck`, `tiptap`, `prosemirror`, `recharts`) that should not be needed on the PDP.

**Current implementation slice.** Set `decoding="sync"` only on the primary PDP gallery image in `ProductMediaGallery`. Keep the existing `loading="eager"`, `fetchPriority="high"`, `sizes`, and quality settings unchanged. Add a colocated regression assertion so the primary product image cannot silently fall back to async decode.

**Current validation target.** After merge and production deploy, confirm the canonical PDP HTML exposes `decoding="sync"` on the primary product image, then rerun PSI mobile and desktop on `https://ogabassey.com/laptops/lenovo-legion-pro-9-16irx9-rtx-4090`.
