...
- [ ] **B3 third-layer guard (deferred, low priority)** — *RPC-level discrimination between legitimate no-shipping (pickup/airport) and broken door-no-quote (both look identical at the wire: `shipping_provider=null`, `selected_quote_id=null`).* Captured during PR #1611 review (Codex P1 + a follow-up correctness finding). Current defense:
  - **Layer 1 (client)** — `place-order.ts` and legacy `apps/web/src/app/checkout/page.tsx` block submission when `deliveryMethod === 'door' && !selectedQuoteId` (PR #1611 commit `0249941831`). Covers every storefront caller.
  - **Layer 2 (RPC)** — `IF p_shipping_provider IS NOT NULL AND p_selected_quote_id IS NULL THEN RAISE 'shipping_quote_required'` catches fabricated-provider callers (the original `|| 'GIGL'` failure mode).
  - **The gap**: a direct PostgREST caller (the RPC is `GRANT ALL TO anon` per Δ-42, by design) writing a non-storefront client could submit `{ shipping_provider: null, selected_quote_id: null, shipping_fee: 25000 }` and silently persist a zero-shipping-rate order. We have no such caller today.
  - **Why deferred**: every viable third predicate has real edge cases. `shipping_fee > 0 ⇒ selected_quote_id NOT NULL` breaks free-shipping promos, $0 quoted rates, and merchant manual overrides — we'd spend more time carving exceptions than we'd recover from theoretical breaches. Pickup/airport intent is client-side state; the RPC cannot derive it from the wire without a new `delivery_method` param, which is its own design discussion.
  - **Reconsider if**: a non-storefront caller (e.g., mobile-admin, a B2B integration) starts using `create_storefront_order` directly. The first such caller is the right forcing function for the design.
