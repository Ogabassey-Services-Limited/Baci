...
Treat non-Vercel provenance, missing/mismatched signature, unknown merchant, `not_running_on_vercel`, timeout, network failure, missing receipt, or invalid/mismatched receipt as cache-specific retryable/fail-closed. Because every barrier stage is idempotent, retry reruns the whole barrier; only an exact matching typed receipt succeeds. Existing retry ceiling/DLQ/replay/audit remain authoritative. Keep legacy `purgeCloudflareUrls()` unchanged.
