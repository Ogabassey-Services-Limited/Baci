...
### Task 8 completion receipt (active)

Completed on 2026-07-17 from exact deployed recovery release evidence:

- [x] PR `#3131` head
  `71f262a8254bf0087cb8f630e82b421feb7dfc0f` merged as
  `bb55d407e01b719a9014c87fb8a8253861b7005d`; deployment run
  `29561460438`, database job `87824630957`, and deploy job `87824674429`
  succeeded. The migration semantic-v1 digest was
  `400990a8ee41f6550b609795b02c6e8090d9c056941ab488d5cee0a2fdfc8af1`
  with `3 applied, 424 skipped`; `25501`, `25502`, and `25503` were log
  ordinals `1`, `2`, and `3` respectively.
- [x] Exceptional record 31 now records exact applied evidence for `25501`.
  `25502` and `25503` remain outside historical production mappings and are
  bound by the separate two-entry forward-repair deployment receipt.
- [x] Read-only post-deploy capture recorded `442` linked rows through
  `20260714225503` and exact production effect
  `71cba5629959c75352726e26cafcbfec8de99b1b52d10e6ad70fd85f07e4d253`.
  The manifest byte-binds the linked fixture
  (`0d8b54ecdae67d99da4e806276310e80992bda73ee94efaaf7a91fd16c3d8885`),
  production-effects fixture
  ...
  and semantic fixture
  (`1d550b33b8f681cdd2f1751279e6d93c1110457834d8743969aa6047d7e33eca`).
- [x] Exact deployed main was merged normally, without rebase or force, as
  local merge commit `1d5dbc073e13c96ee7bbc118f00bdf0b49ccd26e`.
- [x] Chronological and production-effect replays both passed five SQL checks
  in `enforce`, required and verified the immutable old-cancellation proof,
  reported zero changed components, and converged to the exact production
  effect above. The chronological receipt has 427 ordered sources and SHA-256
  `bd49803cad805fb80c626347652153941dfd98243a4babe6244cb68f9aa89f21`;
  the production-effect receipt has 426 ordered sources and SHA-256
  `225f0c4018a816eec6d10096104a564d34f737cf42b993a4672cd29238de3fef`.
  Both prove the cancellation function changed from production digest
  `6155b28720d0f4a8a20746aa1a2365e631249e940fa7339e0e19b66c28fa1e62`
  to repaired digest
  `b21dc2134c1aa3df7aed6c8b7a57173b1fed910a04730f901e56622862503556`.
- [x] Supabase types were regenerated by a third required-proof chronological
  replay and mechanically formatted with the repository formatter;
  `apps/web/src/types/supabase.ts` SHA-256 is
  `5de7d0dc52b8c8c871eba5cea138fd016f74eeb4f81e0a5fcbc62095efa4ca32`.
- [x] Actual deployed registry counts are `427` SQL files, `425` unique
  versions, tail `20260714225503`; the historical frozen base remains
  `424/422` through `25500`. Provenance is schema v5 with 25 evidence sources,
  31 exceptional records, and 9 ordering relations.
- [x] The V4 contract and immutable old-cancellation proof remain byte-identical,
  and every replay cleaned up to zero `baci_replay_*` Docker resources.
- [x] The original P0 plan is resumed at Task 5; its old Task 4 procedure is
  retained only as superseded audit history.
- [x] The Task 8 tooling was hardened after local review: stale semantic
  fixtures can be refreshed without circularly verifying their old bytes;
  forward-deployment provenance is cross-bound to the exact successful head
  and semantic digest; replay constraint identities are unique; all 25 primary
  evidence sources and 25 job groups form an exact bijection; self-referential
  record and job-group edges fail closed; paired fixture rollback attempts both
  restorations; and capture argument plus dispatch failures share one
  secret-safe diagnostic boundary. Every changed non-generated TypeScript file
  remains at or below 300 lines and each new runtime entrypoint has a focused
  regression suite.
- [x] `coderabbit review --agent -t uncommitted` completed once for the Task 8
  diff. Every valid finding was fixed. The broader endpoint/cycle finding was
  rechecked independently: existing endpoint and non-self-cycle layers were
  sound, while the newly proven self-edge and pipeline-evidence gaps were fixed
  with red/green regressions. A fresh independent combined audit of the final
  diff returned clean.
- [x] Final local gates passed after all review fixes: the complete DB-tool
  suite passed 81 files and 468 tests; `pnpm turbo test` passed all five tasks
  with web reporting 2,876 passing files, 23,120 passing tests, one skipped
  file, and one todo; `pnpm turbo lint` passed with known unrelated warnings;
  `pnpm turbo typecheck` passed all five tasks; and `git diff --check` passed.
  Nothing is staged, both pre-existing protected `cli-latest` files remain
  untouched and unstaged, and no `baci_replay_*` Docker resource remains.
...
replay, and production are identical. The Task 8 receipt above satisfies that
condition; active implementation resumes at original P0 Task 5.
