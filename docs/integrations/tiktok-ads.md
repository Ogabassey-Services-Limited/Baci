...
Normal syncs are inclusive 30-day account-local chunks, one selected advertiser at a time. TikTok's regular metrics can lag roughly 30 minutes to two hours; reach can be delayed around 16 hours and TikTok applies a daily correction at 12:00 UTC. `40100`, HTTP throttles, and `X-Tt-Ads-Throttle` cause bounded exponential backoff; a truncated result is not recorded as success. Async task status is parsed only for a future exceptional-volume workflow; dashboard requests never poll a task synchronously.
