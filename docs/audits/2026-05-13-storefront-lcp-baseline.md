...
| 2026-05-25 | Root-head CDN preconnect candidate (`codex/ogabassey-pdp-early-cdn-preconnect`) | OgaBassey PDP | mobile | - | - | - | - | pending deploy | - | - | - | Chrome DevTools production trace before the candidate measured LCP 4547 ms with the 1080w product AVIF queued at 784 ms, sent at 1437 ms, and complete at 3717 ms. The product image is correctly sized and only about 23 KiB, so this candidate targets the 653 ms request-start gap by moving `cdn.ogabassey.com` DNS-prefetch/preconnect into the physical root document head. Local optimized HTML with `Host: ogabassey.com` now shows the CDN hints before `</head>` at bytes 2370/2429, while the product image hint remains later in the stream at byte 16437. Post-deploy PSI/CDP must verify whether the request-start gap actually falls. |
