...
| 2026-05-25 | OgaBassey custom-domain CDN preconnect + single-image thumbnail guard candidate (`codex/ogabassey-pdp-early-cdn-preconnect`) | OgaBassey PDP | mobile | - | - | - | - | pending deploy | - | - | - | Chrome DevTools production trace before the candidate measured LCP 4547 ms with the 1080w product AVIF queued at 784 ms, sent at 1437 ms, and complete at 3717 ms. The product image is correctly sized and only about 23 KiB, so this candidate targets the 653 ms request-start gap by adding a tenant-scoped `Link: <https://cdn.ogabassey.com>; rel=preconnect` header on the `/ogabassey.com/:path*` rewrite path. Local optimized `next start` with `Host: ogabassey.com` emits that CDN preconnect in the response header before any HTML bytes. Chrome DevTools then showed the one-image Lenovo PDP no longer makes the deferred `width=384,quality=50` duplicate thumbnail request; only the 1080w LCP product image is requested in the mobile viewport. Post-deploy PSI/CDP must verify whether the request-start gap actually falls. |
| 2026-05-26 | DebugBear API live baseline before mobile image-size candidate | OgaBassey PDP | mobile, us-east | 74 | 100 | 100 | 92 | 4360 ms | 2156 ms | 345 ms | 0.000 | DebugBear paid Quick Test `50` used Moto G Power emulation, 150 ms RTT, 1.6 Mbps throughput, and 4x CPU. LCP was the Lenovo product image. Breakdown: 702 ms TTFB, 145 ms resource-load delay, 3423 ms resource-load duration, 90 ms render delay. The image request was high priority but selected `width=1080,quality=35`, transferred 23.8 KB, and took 3426 ms. Render-blocking CSS still cost an estimated 330 ms. |
...
## 2026-05-26 DebugBear mobile PDP follow-up evidence

Source captures:

- DebugBear project API key was used only for one-off project Quick Tests after confirming the admin key could list project `100906`. Rotate the shared keys after this audit because they were pasted into the chat.
- DebugBear paid Quick Test `50` report: `https://www.debugbear.com/project/100906/quickTest/50/overview`; local JSON saved at `/tmp/debugbear-quicktest-50.json`, `/tmp/debugbear-quicktest-50-requests.json`, and `/tmp/debugbear-quicktest-50-lhr.json`.
- DebugBear paid Quick Test `51` report: `https://www.debugbear.com/project/100906/quickTest/51/overview`; local JSON saved at `/tmp/debugbear-quicktest-51.json`, `/tmp/debugbear-quicktest-51-requests.json`, and `/tmp/debugbear-quicktest-51-lhr.json`.
- DebugBear's CrUX origin data for phone traffic remains poor: LCP p75 4962 ms, FCP p75 4039 ms, TTFB p75 1405 ms, INP p75 270 ms, CLS p75 0.14, image LCP resource type share 89.63%.
- DebugBear's field LCP image subparts show the real-user bottleneck is distributed across 1416 ms image TTFB, 1629 ms resource-load delay, 330 ms resource-load duration, and 2171 ms element-render delay. The lab bottleneck varies by region, so fixes must keep attacking discovery, resource size, and shell/render delay.
- Local optimized `next start` after the mobile image-size candidate was verified with Chrome DevTools at `http://localhost:3035/ogabassey.com/laptops/lenovo-legion-pro-9-16irx9-rtx-4090`, Moto G Power viewport `412x660x2.625`, and DebugBear-like UA. The actual LCP product image `currentSrc` became `https://cdn.ogabassey.com/image/width=750,quality=35,format=auto/core-assets/products/lenovo-legion.avif` instead of the live `1080w` URL. Screenshot saved at `/tmp/ogabassey-local-pdp-750-mobile.png`; trace saved at `/tmp/ogabassey-local-pdp-lcp-experiment-trace.json.json.gz`.
- Local Chrome DevTools render-blocking insight still reports the storefront CSS chunk as render-blocking. The candidate build reduced the storefront CSS artifact from the live 444 KB decoded chunk to about 299 KB decoded, but a large root/not-found stylesheet is still preloaded as a non-render-blocking route fallback. This is an architectural CSS split follow-up, not proof that image optimization alone will hit 2500 ms.

Decision:

- The current candidate changes the mobile PDP image selection from `1080w` to `750w` by capping the mobile `sizes` hint while keeping the rendered product image layout unchanged. This directly targets DebugBear's `image-delivery-insight` overfetch warning and should cut the selected LCP image transfer from about 23.8 KB to about 14.4 KB on the Moto profile.
- This candidate is unlikely to solve the whole CrUX problem alone. Post-merge DebugBear/PSI must verify whether mobile lab LCP falls below 2500 ms; if it does not, the next experiment should move PDP product-image discovery out of the late RSC stream path, likely through a route-segment/head or edge-generated hint, and then split storefront CSS so PDPs do not inherit all storefront route CSS.
