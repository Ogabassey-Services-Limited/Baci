...
- `/api/cron/storefront-update-nudge`, scheduled daily at 10:00 (server time / UTC). Pushes the in-app "update available" prompt to storefront installs on an older native build than `MOBILE_STOREFRONT_<PLATFORM>_LATEST_BUILD`; throttled per device server-side, so a daily cadence is safe and idempotent.
