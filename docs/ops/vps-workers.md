...
- `/api/cron/storefront-update-nudge`, scheduled daily at 10:00 (server time / UTC). Pushes the "update available" notification to storefront installs on an older native build than `MOBILE_STOREFRONT_<PLATFORM>_LATEST_BUILD`; throttled per device server-side, so a daily cadence is safe and idempotent. **Config lives in the WEB (Vercel) env, not the worker `.env`:** `MOBILE_STOREFRONT_UPDATES_ENABLED`, `MOBILE_STOREFRONT_{ANDROID,IOS}_LATEST_BUILD`, and `_STORE_URL`. A missing/blank `LATEST_BUILD` silently skips that platform (`skipped: 'no_latest_build'`, still HTTP 200) — set both per release. The route returns non-2xx (so `run-web-cron.mjs` exits non-zero and the schedule alerts) only when **every** attempted platform's send fails; a single-platform failure stays 2xx so the healthy platform still nudges.
