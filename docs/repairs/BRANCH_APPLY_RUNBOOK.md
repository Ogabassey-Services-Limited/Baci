...
- **`get_repair_status` stays anon-`EXECUTE` (matches the canonical tracking pattern).** CodeRabbit flagged that the anon-granted RPC is directly callable at `/rest/v1/rpc`, bypassing the route's `repair-status-lookup` (10/min) limiter. Decision: **keep as-is.** The repo's established public-lookup pattern is `get_order_tracking` — a SECURITY-DEFINER, anon-`EXECUTE`, enumeration-guarded RPC called via the anon client (`api/storefront/orders/track-order/route.ts`) with no DB-side throttle, exposing full customer name/email/phone/shipping address. `get_repair_status` follows that pattern and is *more* conservative: it returns only status + device type/model + tracking number behind an exact merchant+ticket+email match, and adds a route-level rate limit the tracking RPC lacks. Closing the direct-RPC path via service-role would violate the "no service-role for user-facing ops" rule and diverge from the tracking precedent; a SQL-keyed throttle on a read cannot rate-limit by client identity without harming legitimate traffic. The route limiter + enumeration guard + minimal exposure is the correct posture.
